	...
	*/
    ...
    public abstract class BaseWebUISecureObject : IDisposable
    {
        ...
        private bool Authenticate()
        {
            ...
            if (HttpContext.Current.Request.Url.AbsolutePath.ToLower().EndsWith("/services/autocompleteservice.svc/autocompletedatasource") || HttpContext.Current.Request.Url.AbsolutePath.ToLower().Contains("login") || HttpContext.Current.Request.Url.AbsolutePath.ToLower().Contains("default.cshtml"))
            {
                Authenticated = true;
            }
            else
            {
                ...
                string sessionToken = string.Empty, clientToken = string.Empty;

                if (HttpContext.Current.Session["UserDetail"] != null)
                    sessionToken = ((CargoFlash.Cargo.Model.UserLogin)HttpContext.Current.Session["UserDetail"]).Signature;
                ...
                if (WebOperationContext.Current != null)
                {
                    ...
                    {
                        if (headerName == "Token")
                            clientToken = headers[headerName].ToString();
                    }
                }
                else
                {
        ...
        private string _ContentType = "application/xml;encode='utf-8'";
        ...
        /// Description: Get domain name from current request
        /// </summary>
        /// <returns></returns>
        public string GetDomainName()
        {
            bool isSecureConnection = String.Equals(HttpContext.Current.Request.ServerVariables["HTTP_X_FORWARDED_PROTO"], "https", StringComparison.OrdinalIgnoreCase);
            string dmName = HttpContext.Current.Request.Url.GetLeftPart(UriPartial.Authority) + HttpContext.Current.Request.ApplicationPath;
            if (isSecureConnection)
                return dmName.Replace("http", "https");
            else
                return dmName;
        ...
        /// <summary>
        /// Created By: Brajraj Singh Tomar
        /// Desc: insert exceptions in db
        /// </summary>
        /// <param name="error"></param>
        /// <param name="methodName"></param>
        /// <param name="moduleName"></param>
        /// <param name="userId"></param>
        public void insertAppException(Exception error, string methodName = null, string moduleName = null, string userId = null)
        {
            ...
            try
            {
                ...
                SqlParameter[] Parameters = { new SqlParameter("@Name",method.DeclaringType.FullName),
                                                new SqlParameter("@MethodName", methodName+method.Name),
                                                ...
                                                new SqlParameter("@MachineName",Environment.MachineName.ToString()+moduleName),
                                                new SqlParameter("@Message", error.Message.ToString()+methodName),
        ...
        public void DataOperationService(string OperatioMode, object dataList, string ModuleName = "", string AppsName = "")
        {
            ...
            try
            {
                ...
                string domainName = GetDomainName(); // Gets domain name from current rquest
                var url = new Uri(domainName + "/Services/" + ServiceName);
                var request = (HttpWebRequest)WebRequest.Create(url);

                request.Method = ServiceMethod;
                request.ContentType = ContentType;
                request.Headers.Add("Token", ((CargoFlash.Cargo.Model.UserLogin)(System.Web.HttpContext.Current.Session["UserDetail"])).Signature);
            ...
            {
                ErrorMessageList = new List<string> { (ex.Message) };
                insertAppException(ex);
            }

            if (ErrorMessageList.Count > 0)
            {
                if (ErrorMessageList[0].Contains("Foreign Key constraint violations while attempting a delete"))
                    ErrorMessageList[0] = "This data is already used in other modules and can not be deleted.";
            ...
            try
            {
                var hiddenField = System.Web.HttpContext.Current.Request.Form["hdnAuditLog"].ToString();
                if (ErrorMessageList.Count == 0 && OperatioMode == "FORMACTION.UPDATE" && !string.IsNullOrEmpty(hiddenField))
                {
                    JavaScriptSerializer json_serializer = new JavaScriptSerializer();
                    var res = JsonConvert.DeserializeObject<DataTable>(hiddenField);
                    ...
                    var KeyValue = "";
                    var KeySNo = "";
                    if (!string.IsNullOrEmpty(HttpContext.Current.Session["AuditLogKeyValue"].ToString()))
                    {
                        var ss = JsonConvert.DeserializeObject<DataTable>(HttpContext.Current.Session["AuditLogKeyValue"].ToString());
                        KeyColumn = ss.Rows[0]["key"].ToString();
                        KeyValue = ss.Rows[0]["value"].ToString();
                        KeySNo = ss.Rows[0]["sno"].ToString();
                    }
        ...
        public void DataOperationAPI(string OperatioMode, object dataList, string ModuleName = "", string AppsName = "")
        {
            try
            {
                ...
                string domainName = GetDomainName(); // Gets domain name from current rquest
                var url = new Uri(domainName + "/api/" + ServiceName);
        ...
        public object DataGetRecordService(string recordID, object classType, string ModuleName = "", string AppsName = "", string ServiceName = "", IDictionary<string, string> QueryString = null)
        {
            try
            {
                //Added By CS: you can add multiple query string in GetRecord service
                string qString = string.Empty;
                if (QueryString != null)
                    foreach (KeyValuePair<string, string> str in QueryString)
                        qString += "&" + str.Key + "=" + str.Value;

                string Apps = ServiceName == "" ? System.Web.HttpContext.Current.Request.QueryString["Apps"] : ServiceName;
                object gp = null;
                //if (string.IsNullOrEmpty(ServiceName))
                ServiceName = (ModuleName == "" ? "" : ModuleName + "/") + Apps + "Service.svc/Get" + (AppsName == "" ? System.Web.HttpContext.Current.Request.QueryString["Apps"] : AppsName) + "Record?recid=" + recordID.ToString() + "&UserID=" + this.MyUserID + qString;
                //var url = new Uri(ConfigurationManager.AppSettings["ServiceURL"] + "/" + ServiceName);
                string domainName = GetDomainName(); // Gets domain name from current rquest
                var url = new Uri(domainName + "/Services/" + ServiceName);
                var request = (HttpWebRequest)WebRequest.Create(url);
