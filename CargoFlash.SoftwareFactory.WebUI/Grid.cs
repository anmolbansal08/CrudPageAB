...
namespace CargoFlash.SoftwareFactory.WebUI.Controls
{
    public class Grid : IDisposable
    {
        ...
        public bool IsShowGridHeader { get; set; }
        ...
        public string AddSlashInColumns { get; set; }
        public string ExcludeColumnsFromExcel { get; set; }
        /// <summary>
        ...
        public Grid()
        {
            ...
            this.AddSlashInColumns = "";
            this.ExcludeColumnsFromExcel = "";
            this.IsAutoBind = true;
            this.IsToolBar = false;
            this.IsAllExport = true;
            this.IsCurrentExport = true;
        ...
        /// <summary>
        /// This method used for render grid from mvc controller or view
        /// </summary>
        /// <param name="gridString"></param>
        /// <param name="readUrl"></param>
        /// <param name="editUrl"></param>
        /// <param name="deleteUrl"></param>
        public void InstantiateIn(StringBuilder gridString, string readUrl, string editUrl, string deleteUrl)
        {
            this.PageRights = ((CargoFlash.Cargo.Model.UserLogin)HttpContext.Current.Session["UserDetail"]).PageRights.Where(e => e.Apps.ToUpper() == this.AppsName.ToUpper()).Select(e => new PageRights { Apps = e.Apps, PageRight = e.PageRight }).ToList(); //HttpContext.Current.Session["PageRights"] == null ? "ALL" : HttpContext.Current.Session["PageRights"].ToString();
            string url = PageName + "?Module=" + this.ModuleName + "&Apps=" + this.AppsName + "&FormAction=";
            string qString = string.Empty;
            foreach (string str in QueryString)
                qString += "&" + str + "=";

            string targetParamRead = url + "Read&UserID=" + this.UserID + "&RecID=" + qString;
            string targetParamEdit = url + "Edit&UserID=" + this.UserID + "&RecID=" + qString;
            string targetParamDelete = url + "Delete&UserID=" + this.UserID + "&RecID=" + qString;
            ...
            if (IsFormHeader)
            {
                if (this.IsShowGridHeader)
                    gridString.Append("<table class='WebFormTable'><tr><td class='formSection' colspan='2'>" + this.FormCaptionText + "</td></tr>");
                else
                    gridString.Append("<table class='WebFormTable'><tr><td class='' colspan='2'></td></tr>");

                if (!string.IsNullOrWhiteSpace(this.CommandButtonNewText))
                {
                    if (this.PageRights.Where(e => e.PageRight.ToUpper() == "NEW").Any())
                        gridString.Append("<tr align='right'><td class='form2buttonrow'><input type='button' value='" + this.CommandButtonNewText + "'  onclick=navigateUrl('" + this.NewURL + "');  class='btn btn-info' /></td></tr>");
                }
                ...
                if (IsActionRequired)
                {
                    ...
                    {
                        string rwidth = "style=width:70px'";
                        string ewidth = "style='width:70px'";
                        if (ReadName.Length > 10)
                            rwidth = "style='width:140px'";
                        if (EditName.Length > 10)
                            ewidth = "style='width:140px'";

                        if (this.PageRights.Where(e => e.PageRight.ToUpper() == "READ").Any())
                            gridString.Append("<a class='tool-item gradient' href=\"" + readUrl + "\" " + rwidth + "><i class=\"icon-read\"><span class=\"actionSpan\">" + ReadName + "</span></i></a>");
                        if (this.PageRights.Where(e => e.PageRight.ToUpper() == "EDIT").Any())
                            gridString.Append("<a class='tool-item gradient' href=\"" + editUrl + "\" " + ewidth + "><i class=\"icon-edit\"><span class=\"actionSpan\">" + EditName + "</span></i></a>");
                        if (IsShowDelete && (this.PageRights.Where(e => e.PageRight.ToUpper() == "DELETE").Any()))
                            gridString.Append("<a class='tool-item gradient' href=\"" + deleteUrl + "\"><i class=\"icon-trash\"><span class=\"actionSpan\">Delete</span></i></a>");
                    }
                    else
                    {
                        gridString.Append(String.Join(" ", Action.Select(e => e.ToString(PageName, this.PrimaryID, this.UserID, this.IsToolBarRatioButton, true, this.PageRights, QueryString)).ToArray()));
                    }
            ...
            else
            {
                ...
                else
                {
                    if (this.IsShowGridHeader)
                        ...
                        gridString.Append("<table class='WebFormTable'><tr><td class='' colspan='2'></td></tr><tr><td colspan='2'>");
                }
            ...
            if (this.IsCurrentExport)
            {
                ...
                gridString.Append("cssClass: 'fa fa-file-excel-o',");
                gridString.Append("createUrl: '" + this.DataSoruceUrl + "',");
                gridString.Append("fileName: '" + (string.IsNullOrEmpty(this.FormCaptionText) == true ? this.AppsName : this.FormCaptionText) + "'");
                gridString.Append("},");
            }
            ...
            if (this.IsAllExport)
            {
                ...
                gridString.Append("cssClass: 'fa fa-file-excel-o',");
                gridString.Append("createUrl: '" + this.DataSoruceUrl + "',");
                gridString.Append("fileName: 'All_" + (string.IsNullOrEmpty(this.FormCaptionText) == true ? this.AppsName : this.FormCaptionText) + "'");
                gridString.Append("},");
            }
            ...
            //Added By Amit Yadav
            gridString.Append("AddSlashInColumns : '" + this.AddSlashInColumns + "',");
            gridString.Append("ExcludeColumnsFromExcel : '" + this.ExcludeColumnsFromExcel + "',");
            ...
            if (!ShowActionOnRight && IsActionRequired)
            {
                ...
                if (IsModule == false)
                {
                    if (IsToolBarRatioButton == true)
                    {
                        ...
                        {
                            gridString.Append(String.Join(" ", Action.Select(e => e.ToString(PageName, this.PrimaryID, this.UserID, this.IsToolBarRatioButton, false, PageRights, QueryString)).ToArray()));
                        }
                        gridString.Append("</div>");
            ...
            if (ShowActionOnRight && IsActionRequired)
            {
                ...
                if (IsModule == false)
                {
                    if (Action == null)
                        ...
                        if (Action.Count > 0)
                            gridString.Append("template: ' " + String.Join(" ", Action.Select(e => e.ToString(PageName, this.PrimaryID, 0, false, false, PageRights, QueryString)).ToArray()) + "', title: \"" + this.ActionTitle + "\", width: \"120px\", filterable: false, sortable: false");
                        else
                            gridString.Append("template: ' <a class=\"actionButton actionView\" href=\"" + targetParamRead + "\">Read</a> <a class=\"actionButton actionEdit\" href=\"" + targetParamEdit + "\">Edit</a> <a  class=\"actionButton actionDelte\" href=\"" + targetParamDelete + "\"> Delete </a>', title: \"" + this.ActionTitle + "\", width: \"120px\", filterable: false, sortable: false");
                }
                else
        ...
        /// 
        /// </summary>
        public void InstantiateIn(StringBuilder gridString, bool isV2 = false)
        {
            this.PageRights = ((CargoFlash.Cargo.Model.UserLogin)HttpContext.Current.Session["UserDetail"]).PageRights.Where(e => e.Apps.ToUpper() == this.AppsName.ToUpper()).Select(e => new PageRights { Apps = e.Apps, PageRight = e.PageRight }).ToList(); //HttpContext.Current.Session["PageRights"] == null ? "ALL" : HttpContext.Current.Session["PageRights"].ToString();
            string url = PageName + "?Module=" + this.ModuleName + "&Apps=" + this.AppsName + "&FormAction=";
            string qString = string.Empty;
            ...
            if (IsFormHeader)
            {
                ...
                if (IsActionRequired)
                {
                    ...
                    if (Action == null || Action.Count == 0)
                    {
                        ...
                        if (this.PageRights.Where(e => e.PageRight.ToUpper() == "READ").Any())
                            gridString.Append("<a class='tool-item gradient' href=\"" + targetParamRead + "\" " + rwidth + "><i class=\"icon-read\"><span class=\"actionSpan\">" + ReadName + "</span></i></a>");
                        if (this.PageRights.Where(e => e.PageRight.ToUpper() == "EDIT").Any())
                            gridString.Append("<a class='tool-item gradient' href=\"" + targetParamEdit + "\" " + ewidth + "><i class=\"icon-edit\"><span class=\"actionSpan\">" + EditName + "</span></i></a>");
                        if (IsShowDelete && (this.PageRights.Where(e => e.PageRight.ToUpper() == "DELETE").Any()))
            ...
            if (isV2)
                gridString.Append("data: {model:{" + extraParam + "}}");
            else
                gridString.Append("data: {" + extraParam + "}");
    ...
    public class GridColumn
    {
        ...
        public string ToString(bool isSchema, bool IsSortable, bool IsFilterable, bool IsCelleditable, bool IsDefaulteditable, List<GridAggregate> Aggregate = null)
        {
            ...
            else
            {
                ...
                else if (String.IsNullOrWhiteSpace(Template) && !IsHidden)
                {
                    if (commarequired) stb.Append(", " + Environment.NewLine);
                    ...
                    {
                        stb.Append("template :'<span  title= \"#= " + Field + " #\">#= " + Field + " #</span>' ");
                    }
                    commarequired = true;
                }
        ...
        public string ToProcessString(bool isSchema)
        {
            ...
            if (HttpContext.Current.Session["_ProcessSetting"] != null)
            {
                ...
                if (isSchema)
                {
                    ...
                    {
                        if (drProcessSetting["PROCESSNAME"].ToString() == "SLIBooking")
                            stb.Append("," + drProcessSetting["SUBPROCESSDISPLAYNAME"].ToString().Replace("SLI", ((CargoFlash.Cargo.Model.UserLogin)(System.Web.HttpContext.Current.Session["UserDetail"])).SysSetting["SLICaption"].ToString()) + ": {type : \"string\" }");
                        else
                ...
                else
                {
                    ...
                    foreach (DataRow drProcessSetting in _ProcessSetting.Rows)
                    {
                        if (!Convert.ToBoolean(drProcessSetting["IsBlocked"]))
                        {
                            var pr = "";
                            if (drProcessSetting["PROCESSNAME"].ToString() == "SLIBooking")
                                pr = drProcessSetting["SUBPROCESSDISPLAYNAME"].ToString().Replace("SLI", ((CargoFlash.Cargo.Model.UserLogin)(System.Web.HttpContext.Current.Session["UserDetail"])).SysSetting["SLICaption"].ToString());
                            else
                                pr = drProcessSetting["SUBPROCESSDISPLAYNAME"].ToString();

                            stb.Append(",");
                            stb.Append("{");
                            stb.Append("field :\"SNo\", ");
                            stb.Append("title :\"" + pr + "\", ");
                            stb.Append("tooltip :\"" + pr + "\", ");
                            ...
                            if (drProcessSetting["ProgressCheck"].ToString().ToLower() == "true")
                                stb.Append("template :'<input rel=\"Process\" type=\"button\" currentProcess=\"" + drProcessSetting["PROCESSNAME"].ToString() + "\" process=\"" + drProcessSetting["SUBPROCESSNAME"].ToString().ToUpper() + "\" subProcessSNo=\"" + drProcessSetting["SubProcessSNo"].ToString() + "\" onclick=\"HighLightGridButton(this,event);BindEvents(this,event);return false;\"  title=\"" + drProcessSetting["SUBPROCESSDISPLAYNAME"].ToString() + "\"   value=\"" + drProcessSetting["DisplayCaption"].ToString() + "\" class= #=checkProgrss(ProcessStatus,\"" + drProcessSetting["SUBPROCESSNAME"].ToString().ToUpper() + "\",\"" + drProcessSetting["DisplayCaption"].ToString() + "\")# ondblclick=\"HighLightGridButton(this,event);BindEvents(this,event,true); \"/>', ");
                            else
                                stb.Append("template :'<input rel=\"Process\" type=\"button\" currentProcess=\"" + drProcessSetting["PROCESSNAME"].ToString() + "\" process=\"" + drProcessSetting["SUBPROCESSNAME"].ToString().ToUpper() + "\" subProcessSNo=\"" + drProcessSetting["SubProcessSNo"].ToString() + "\" onclick=\"HighLightGridButton(this,event);BindEvents(this,event);return false;\"  title=\"" + drProcessSetting["SUBPROCESSDISPLAYNAME"].ToString() + "\"  value=\"" + drProcessSetting["DisplayCaption"].ToString() + "\" ondblclick=\"HighLightGridButton(this,event);BindEvents(this,event,true); \"/>', ");
                            stb.Append("width:40, ");
    ...
    public class GridAction
    {
        ...
        public string ToString(string pageName, string recordID, Int64 UserID = 0, bool IsToolBarRatioButton = false, bool IsHeaderToolBar = false, List<PageRights> PageRights = null, List<string> QueryString = null)
        {
            string qString = string.Empty;
            foreach (string str in QueryString)
                qString += "&" + str + "=";
            ...
            if (IsToolBarRatioButton)
            {
                ...
                string urlToolBarRatio = pageName + "?Module=" + this.ModuleName + "&Apps=" + this.AppsName + "&FormAction=" + this.ActionName + recordID + qString;

                if (this.CssClassName != null && this.ActionName != null)
                {
                    string bwidth = " style=\"width:70px\"";
                    if (!string.IsNullOrEmpty(ButtonCaption) && ButtonCaption.Length > 10)
                        bwidth = " style=\"width:140px\"";



                    if (string.IsNullOrWhiteSpace(this.ClientAction) && string.IsNullOrWhiteSpace(this.NewUrl))
                    {
                        if (PageRights.Where(e => e.PageRight.ToUpper() == this.ActionName.ToUpper()).Any())
                            strAction = "<a " + (IsHeaderToolBar ? "class='tool-item gradient' " : "") + "href=\"" + urlToolBarRatio + "\" \"" + bwidth + "\"><i class=\"icon-" + this.CssClassName + "\"><span class=\"actionSpan\">" + (string.IsNullOrWhiteSpace(this.ButtonCaption) == true ? this.ActionName : this.ButtonCaption) + "</span></i></a>";
                    }
                    else if (!string.IsNullOrWhiteSpace(this.ClientAction) && string.IsNullOrWhiteSpace(this.NewUrl))
                    {
                        if (PageRights.Where(e => e.PageRight.ToUpper() == this.ActionName.ToUpper()).Any())
                            strAction = "<a " + (IsHeaderToolBar ? "class='tool-item gradient' " : "") + "href=\"javascript:void(0);&RecID=\" \"" + bwidth + "\" onclick=\"return " + this.ClientAction + "(this);\"><i class=\"icon-" + this.CssClassName + "\"><span class=\"actionSpan\">" + (string.IsNullOrWhiteSpace(this.ButtonCaption) == true ? this.ActionName : this.ButtonCaption) + "</span></i></a>";
                    }
                    else
            ...
            else if (IsLink)
            {
                ...
                string url = pageName + "?Module=" + this.ModuleName + "&Apps=" + this.AppsName + "&FormAction=" + ActionName + recordID + qString;
