    ...
    if (getQueryStringValue("FormAction").toUpperCase() == "NEW" || getQueryStringValue("FormAction").toUpperCase() == "DUPLICATE") {
        ...
        $('#tbl' + dbTableName).appendGrid({
            ...
            columns: [
                ...
                { name: 'SpecialHandlingCodeSNo', display: 'SPHC Code', type: 'text', ctrlAttr: { maxlength: 100, controltype: 'autocomplete' }, ctrlCss: { width: '140px', height: '20px' }, isRequired: true, tableName: 'SPHC', textColumn: 'Code', keyColumn: 'SNo' },
    ...
    if (getQueryStringValue("FormAction").toUpperCase() == "NEW" || getQueryStringValue("FormAction").toUpperCase() == "DUPLICATE") {
        ...
        $('#tbl' + dbTableName).appendGrid({
            ...
            columns: [
                { name: 'SNo', type: 'hidden', value: 0 },
                { name: 'DueCarrierSNo', type: 'hidden', value: $('#hdnDueCarrierSNo').val() },
                { name: 'CommoditySNo', display: 'Commodity Code', type: 'text', ctrlAttr: { maxlength: 100, controltype: 'autocomplete' }, ctrlCss: { width: '140px', height: '20px' }, isRequired: true, tableName: 'Commodity', textColumn: 'CommodityCode', keyColumn: 'SNo' },
                { name: pageType == 'Edit' ? 'IsActive' : 'Active', display: 'Active', type: 'radiolist', ctrlOptions: { 0: 'No', 1: 'Yes' }, selectedIndex: 1, onClick: function (evt, rowIndex) { } },
                { name: 'CreatedBy', type: 'hidden', value: $('#hdnCreatedBy').val() },
                { name: 'UpdatedBy', type: 'hidden', value: $('#hdnUpdatedBy').val() }
            ],
