...
$(document).ready(function () {
    ...
    //$('#AirCraftInventoryPaxFactorPaxStart').attr('onblur', 'StartEnd();');
    //$('#AirCraftInventoryPaxFactorPaxEnd').attr('onblur', 'StartEnd();');
    cfi.ValidateForm();
    ...
    cfi.AutoComplete("ABBRCodeSNo", "AbbrCode,Description", "vwAircraftPossibility", "SNo", "AbbrCode", ["AbbrCode", "Description"], null, "contains");
    ...
    $("#AircraftVersion").keydown(function (event) {
        ...
        $('#tbl' + dbTableName).appendGrid({
            ...
            columns: [
                ...
                { name: 'AirCraftSNo', type: 'hidden', value: $('#hdnAirCraftSNo').val() },
                 ...
                 { name: 'SPHCSNo', display: 'SHC Code', type: 'text', ctrlAttr: { maxlength: 100, controltype: 'autocomplete', onSelect: "return Validate(this.id);" }, ctrlCss: { width: '140px', height: '20px' }, isRequired: true, tableName: 'SPHC', textColumn: 'Code', keyColumn: 'SNo' },
                {
    ...
    if (getQueryStringValue("FormAction").toUpperCase() == "NEW" || getQueryStringValue("FormAction").toUpperCase() == "DUPLICATE") {
        ...
        $('#tbl' + dbTableName).appendGrid({
            ...
            columns: [
                { name: 'SNo', type: 'hidden', value: 0 },
                { name: 'AirCraftSNo', type: 'hidden', value: $('#hdnAirCraftSNo').val() },
                { name: 'RegistrationNo', display: 'Registration No', type: 'text', ctrlAttr: { maxlength: 48, controltype: 'alphanumericupper' }, ctrlCss: { width: '150px' }, isRequired: false },
...
function AirCraftInventoryPaxFactorGrid() {
    ...
    if (getQueryStringValue("FormAction").toUpperCase() == "NEW" || getQueryStringValue("FormAction").toUpperCase() == "DUPLICATE") {
        ...
        $('#tbl' + dbTableName).appendGrid({
            ...
            columns: [
                { name: 'SNo', type: 'hidden', value: 0 },
                { name: 'AirCraftInventorySNo', display: 'Registration No', type: 'text', ctrlAttr: { maxlength: 100, controltype: 'autocomplete', onclick: "return ChangeAircraft(this.id);" }, ctrlCss: { width: '140px', height: '20px' }, isRequired: true, tableName: 'AirCraftInventory', textColumn: 'RegistrationNo', keyColumn: 'SNo', filterCriteria: "contains" },
    ...
    if (getQueryStringValue("FormAction").toUpperCase() == "NEW" || getQueryStringValue("FormAction").toUpperCase() == "DUPLICATE") {
        ...
        $('#tbl' + dbTableName).appendGrid({
            ...
            columns: [
                ...
                { name: 'ULDTypeSNo', display: 'ULD Type', type: 'text', ctrlAttr: { maxlength: 100, controltype: 'autocomplete' }, ctrlCss: { width: '140px', height: '20px' }, isRequired: true, tableName: 'vwULD', textColumn: 'Text_ULD', keyColumn: 'SNo' },//templateColumn: ["ULDName", "ContainerType"],
                { name: 'ContourType', display: 'Contour Type', type: 'text', ctrlAttr: { maxlength: 100, controltype: 'autocomplete' }, ctrlCss: { width: '140px', height: '20px' }, tableName: 'vwULDContour', textColumn: 'AbbrCode', keyColumn: 'SNo' },
...
function ExtraCondition(textId) {
    ...
    if (textId.indexOf("tblAirCraftInventoryPaxFactor_AirCraftInventorySNo") >= 0) {
...
$('#GrossWeight').blur(function () {
    ...
    if ($("#GrossWeight").val() < 1) {
        alert("Gross Weight Cannot be Zero.")
        $("#GrossWeight").val("");
        return false;
    }
...
$('#VolumeWeight').blur(function () {
    ...
    if ($("#VolumeWeight").val()<  1) {
        alert("Volume Weight Cannot be Zero.")
        $("#VolumeWeight").val("");
        return false;
    }
...
$('#MaxGrossWtPiece').blur(function () {
    ...
    if ($("#MaxGrossWtPiece").val() < 1) {
        alert("Max Weight Per Piece Cannot be Zero.");
        $("#MaxGrossWtPiece").val("");
        return false;
    }
