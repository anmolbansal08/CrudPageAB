...
$(document).ready(function () {
    ...
    cfi.AutoComplete("SPHCCode", "Code", "SPHC", "SNo", "Code", ["Code"], SPHCFunction, "contains");
    cfi.AutoComplete("CopyFrom", "SPHCCode", "VChecklisttypeSPHC", "SPHCSNo", "SPHCCode", ["SPHCCode", "Name"], getExistRecord, "contains");
    ...
    //$("#tbl tr:nth-child(6) td:nth-child(2) span").hide();
    $("input[name='Text_AirlineName']").attr('disabled', true);
    //  $("#spnFOR").text("Checklist Type");
    ...
    //                AirportSNo: 112,// $("#AirportName").val() == "" ? 0 : parseInt($("#AirportName").val() == ""),
    //                SNo: $("#SNo").val(),
    //                SHC: $("#SHC").val(),
    //                SPHCCode: $("#Text_SPHCCode").val(),
    //                SPHCSNo: $('input[name=SHC]:checked').val() == "0" ? $("#SPHCCode").val() : 0,
    //                SPHCGroupSNo: $("#SPHCCode").val() == "0" ? $("#SPHCCode").val() : 0,
    ...
    //                EnteredBy: 1//((CargoFlash.Cargo.Model.UserLogin)(System.Web.HttpContext.Current.Session["UserDetail"])).UserSNo
    //            });

    //            $.ajax({
    //                url: "Services/Master/CheckListTypeService.svc/UpdateCheckListType",
    //                async: false,
    //                type: "POST",
    //                cache: false,
    ...
    //                    if ((response.indexOf('CheckList Type already exists')) > 0)
    //                    {
    //                        ShowMessage('warning', 'Warning - CheckList', "CheckList Type already exists");
    //                    }
    //                    else
    //                    {
    //                        ShowMessage('success', 'Need your Kind Attention!', "CheckList Saved Successfully");
    //                        $("#CheckListTypeTabs").kendoTabStrip().data("kendoTabStrip").activateTab($('#liCheckListHeader'));
    ...
    //$('#tblCheckListType input:text[id^="tblCheckListType_Name"]').live("keyup", function () {
    ...
    //$('#ColumnName1,#ColumnName2,#ColumnName3').on('keydown keyup', function () {
    ...
    //$('#GeneralHeader').on('input keydown keyup', function () {
    //    var text = $(this).val();
    //    var iKeyCode = (event.which) ? event.which : event.keyCode
    //    if (iKeyCode == 13) {

    //        for (var i = 0; i < lines.indexOf('\n'); i++) {
    //            if (lines[i].length == maxLength - 2) {
    //                lines[i] = lines[i] + " \n";
    //            }
    ...
    $('#tbl' + CheckListType).appendGrid({
        ...
        columns: [{ name: 'SNo', type: 'hidden', value: 0 },
            { CLTSNo: 'CLTSNo', type: 'hidden', value: CUrrentCTLSNo },
                  { name: "Name", display: "Checklist Header Name", type: "text", ctrlAttr: { maxlength: 100, controltype: "autocomplete" }, ctrlCss: { width: "100px" }, isRequired: true, tableName: "CheckListHeader", textColumn: "Name", keyColumn: "SNo" },
                  {
...
function ViewDetailCheckListType() {
    ...
    $('#tbl' + CheckListType).appendGrid({
        ...
        rowNumColumnName: 'SNo',
        columns: [{ name: 'SNo', type: 'hidden', value: 0 },
                  { name: "Name", display: "Checklist Header Name", type: "text", ctrlAttr: { maxlength: 100, controltype: "autocomplete" }, ctrlCss: { width: "60px" }, isRequired: true, tableName: "vCheckListHeader", textColumn: "Name", keyColumn: "SNo" },
...
function BindingAutocompleteOnRadioButtonAction() {
    ...
    $("input[name^=SHC]").live("click", function () {
        if ($("input[name^=SHC]:checked").val() == 0) {
            var data = GetDataSource("SPHCCode", "SPHC", "SNo", "Code", ["Code"], null);
        ...
        if ($("input[name^=SHC]:checked").val() == 1) {
            var data = GetDataSource("SPHCCode", "SPHCGroup", "SNo", "Name", ["Name"], null);
