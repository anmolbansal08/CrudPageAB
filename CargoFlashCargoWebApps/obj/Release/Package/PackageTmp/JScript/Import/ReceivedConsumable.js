...
function MasterDeliveryOrder() {
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            cfi.AutoComplete("searchAirline", "CarrierCode,AirlineName", "Airline", "CarrierCode", "AirlineName", ["CarrierCode", "AirlineName"], OnchangeFlightnew, "contains");
            //   cfi.AutoComplete("OwnerSno", "AirlineName", "Airline", "CarrierCode", "AirlineName", ["AirlineName"], null, "contains");            
...
function ShowProcessDetailsNew(subprocess, divID, isdblclick) {
    if (subprocess == "FWBRATE") {
        $.ajax({
            url: "Services/Import/DeliveryOrderService.svc/GetWebForm/" + _CURR_PRO_ + "/Import/" + subprocess + "/New/1", async: true, type: "get", cache: false, contentType: "application/json; charset=utf-8",
            success: function (result) {
    ...
    $("#tbl" + dbtableName).appendGrid({
        ...
        columns: [{ name: 'SNo', type: 'hidden', value: 0 },
                { name: 'ConsumableSno', type: 'hidden', value: 0 },
                 ...
                 {
                     name: "IssueTo", display: "Forwarder(Agent)/Airline/Self", type: "text", ctrlAttr: { maxlength: 80, controltype: "autocomplete" }, ctrlCss: { width: "90px" }, isRequired: true, tableName: "VAgentAirline", textColumn: "Name", templateColumn: ["Name"], keyColumn: "Sno", addOnFunction: function (id, text, hdnid, key) {
                         debugger
                ...
                {
                    name: "Item", display: "Item", type: "text", ctrlAttr: { maxlength: 80, controltype: "autocomplete" }, ctrlCss: { width: "90px" }, isRequired: false, tableName: "VConsumableForself", textColumn: "item", templateColumn: ["item"], keyColumn: "Sno", addOnFunction: function (id, text, hdnid, key) {
                        debugger
                        var ind = $("#" + id).attr("id").split('_')[2];
...
function BindIssueConsumable() {
    ...
    $("#tbl" + dbtableName).appendGrid({
        ...
        columns: [{ name: 'SNo', type: 'hidden', value: 0 },
                 ...
                 {
                     name: "IssueTo", display: "Issue To", type: "text", ctrlAttr: { maxlength: 80, controltype: "autocomplete" }, ctrlCss: { width: "90px" }, isRequired: true, tableName: "VAgentAirline", textColumn: "Name", templateColumn: ["Name"], keyColumn: "Sno"
                 },
                 {
