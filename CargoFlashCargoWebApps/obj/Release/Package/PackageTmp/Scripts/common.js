...
function navigateUrl(currenturl) {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            location.href = SiteUrl + 'Account/GarudaLogin.cshtml?islogout=true';
        }
    });
    return res;
}
...
function CargoFlashApplication() {
    ...
    this.GetAWBLockedEvent = function (UserSNo, AWBSNo, DailyFlightSNo, FlightNo, FlightDate, ULDNo) {
        var message = "";
        $.ajax({
...
//Added By Amit Yadav
function AuditLogSaveNewValue(id, isAjax, subprocess, KeyColumn, KeyValue, keySNo) {
    try {
        isAjax = isAjax || false;
        ...
        $("#" + id).find("input[type!='hidden'],textarea,select").each(function (i, e) {
            ...
            } else if ($(this).attr("controltype") == "autocomplete") {
        ...
        $("#" + id).find("[newvalue]").each(function (i, e) {
            if ($(e).attr("newvalue") != "" && $(e).attr("newvalue") != $(e).attr("oldvalue")) {
                var clName = ($(e).attr("name") || $(e).attr("id")) || "";
        ...
        if ($("#hdnAuditLog").length == 0) {
            if ($("form").length > 0)
                $("<input>", { type: 'hidden', id: 'hdnAuditLog', name: 'hdnAuditLog', value: '' }).appendTo("form");
            else
                $("<input>", { type: 'hidden', id: 'hdnAuditLog', name: 'hdnAuditLog', value: '' }).appendTo("body");
        }
        if (arrVal.length > 0)
            $("#hdnAuditLog").val(JSON.stringify(arrVal));

        if (isAjax)
            SaveAppendGridAuditLog(KeyColumn, KeyValue, keySNo);
    } catch (e) {

    }
}
//Added By Amit Yadav
function SaveAppendGridAuditLog(KeyColumn, KeyValue, keySNo) {
    try {
        if ($("#hdnAuditLog").length > 0 && $("#hdnAuditLog").val() != "") {
            ...
            $.ajax({
                ...
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                url: SiteUrl + "Services/Common/CommonService.svc/SaveAppendGridAuditLog?ModuleName=" + getQueryStringValue("Module").toUpperCase() + "&AppsName=" + getQueryStringValue("Apps").toUpperCase() + "&KeyColumn=" + KeyColumn + "&KeyValue=" + KeyValue + "&KeySNo=" + keySNo,
