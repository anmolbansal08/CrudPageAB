...
(function ($) {
    ...
    var _methods = {
                ...
                if (settings.contentEditable) {
                    ...
                    if (settings.hideButtons.append && settings.hideButtons.removeLast && settings.hideButtons.updateAll
                        ...
                        if (!settings.hideButtons.updateAll) {
                            $('<button/>').addClass('updateAll', settings._buttonClasses.updateAll).css('float', 'left').attr({ type: 'button', title: settings._i18n.updateAll, title: 'Update All Record.', id: settings.idPrefix + '_btnUpdateAll', name: settings.idPrefix + '_btnUpdateAll', onclick: (!isEmpty(settings.allUpdateExtraFunction)) ? settings.allUpdateExtraFunction : '' })
                            .button({ icons: { primary: 'ui-icon-refresh' }, label: 'Update All' }).click(function () {
                        ...
                        if (!settings.hideButtons.append) {
                            ...
                            .button({ icons: { primary: 'ui-icon-plusthick' }, text: false }).click(function () {
                                ...
                                //Added Amit
                                if (!validateTableData(settings.tableID, settings._uniqueIndex)) {
                                    return false;
                                }
                                //Added by Amit Yadav
        ...
        getStringJson: function () {
            ...
            if (validateTableData(settings.tableID, settings.updatedRows)) {
    ...
    function createHTMLControl(tWhole, tRow, tCell, col, colIndex, uIndex, settings) {
        ...
        if (col.type != 'custom' && settings.contentEditable) {
            ...
            if (decimalPosition >= -1) {
                ...
                cfi.GridAutoComplete(settings.idPrefix + '_' + col.name + '_' + uIndex, col.basedOn, col.tableName, col.keyColumn, col.textColumn, col.templateColumn, col.addOnFunction, col.filterCriteria, col.separator, col.newAllowed, col.confirmOnAdd, col.procName, col.onSelect);
