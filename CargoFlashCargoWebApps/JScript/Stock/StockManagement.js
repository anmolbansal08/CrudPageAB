...
$(document).ready(function () {
    ...
    if (getQueryStringValue("FormAction").toUpperCase() == "NEW") {
        ...
        cfi.AutoCompleteV2("AWBPrefix", "AirlineCode", "StockManagement_AirlineCode", null, "contains", null, null, null, null, AWBNumber);
        cfi.AutoCompleteV2("IsAutoAWB", "stocktype", "StockManagement_stocktype", null, "contains");
        ...
        cfi.AutoCompleteV2("City", "CityCode", "StockManagement_City", null, "contains");
        cfi.AutoCompleteV2("Office", "Name", "StockManagement_Office", null, "contains");
    ...
    if (getQueryStringValue("FormAction").toUpperCase() == "READ") {
        ...
        cfi.AutoCompleteV2("City", "CityCode", "StockManagement_ReadCity", ResetAutoComplete, "contains");
        cfi.AutoCompleteV2("Office", "Name", "StockManagement_ReadOffice", GetAvailableStock, "contains");
        ...
        cfi.AutoCompleteV2("IsAutoAWB", "stocktype", "StockManagement_Readstocktype", null, "contains");
    ...
    if (getQueryStringValue("FormAction").toUpperCase() == "EDIT") {
       ...
       // debugger;
        ...
        cfi.AutoCompleteV2("City", "CityCode", "StockManagement_EditCity", ResetAutoComplete, "contains");
        cfi.AutoCompleteV2("Office", "Name", "StockManagement_EditOffice", GetAvailableStock, "contains");
        cfi.AutoCompleteV2("Account", "AccountCode,Name", "Stock_Account", GetAvailableStock, "contains");
        cfi.AutoCompleteV2("IsAutoAWB", "stocktype", "StockManagement_Editstocktype", null, "contains");
...
//    $('.k-grid-header table tr th[data-field="AWBNo"]').css("width", "130px");
//    $('.k-grid-content table tr td[data-column="AWBNo"]').css("width", "130px");


//    $('.k-grid-header table tr th[data-field="LotNo"]').css("width", "130px");
//    $('.k-grid-content table tr td[data-column="LotNo"]').css("width", "130px");
    ...
    try {
        $.ajax({
            ...
            success: function (result) {
                var Data = jQuery.parseJSON(result);
                var resData = Data.Table0;
                if (resData.length > 0 && userContext.OfficeSNo == undefined ? "0" : userContext.OfficeSNo=="0" ) {
                    $('#Office').val(resData[0].officeSNo);
                    $('#Text_Office').val(resData[0].officeName);
                }
                else if (userContext.OfficeSNo == undefined ? "0" : userContext.OfficeSNo == "0") {
                    $('#Office').val("");
                    $('#Text_Office').val("");
                }
            }
...
function ExtraCondition(textId) {
    ...
    if (textId == "Text_Office") {
        try {
            ...
            cfi.setFilter(f, "AirlineCode", "eq", $("#Text_AWBPrefix").val())
    ...
    if (textId == "Text_Account") {
        try {
            ...
            if ($("#Text_IsAutoAWB").data("kendoAutoComplete").value() == "CASS") {
                ...
                cfi.setFilter(or, "OfficeSNo", "eq", $("#Office").val());
                cfi.setFilter(or, "OfficeParentID", "eq", $("#Office").val());
...
function CreateStock() {
    ...
    $.ajax({
        type: "POST",
        url: "./Services/Stock/StockManagementService.svc/CreateStock?AWBPrefix=" + AWBPrefix + "&AWBType=" + AWBType + "&IsAutoAWB=" + IsAutoAWB + "&CountryCode=" + CountryCode + "&ExpiryDate=" + ExpiryDate,
        ...
        success: function (response) {
            ...
            if (response[0].leftStock.length > 0) {
                ...
                cfi.AutoCompleteV2("City", "CityCode", "StockManagement_CreateCity", null, "contains");
                cfi.AutoCompleteV2("Office", "Name", "StockManagement_CreateOffice", null, "contains");
...
function GetMaxIssueAWB(e) {
    ...
    $("#lstTotalCreatedStock option").each(function () {
    
                $(this).attr('selected', false)
                 
     
    });
    var awbfrom = $("#AWBFROM").val().trim();
    ...
    if (TotalIssueStock > TotalCreatedStock) {
        ...
        $("#lstTotalCreatedStock option").each(function () {
            if (e.value != '') {
                ...
                if ($(this).val() >= awbfrom && $(this).index() <= ($(this).index())+(i-1)) {
                    $(this).attr('selected', 'selected')
                    
                    if ($(this).index() == ($(this).index()) + (i - 1)) {
                        $("#AWBTO").val($(this).val());
                    }
                    i = i - 1;
                }
...
function GetMaxIssuetoAgentAWB(e) {
    $("#lstIssueStockOffice option").each(function () {
...
//Started code by Shivali Thakur for awb stock start and end series filter
function keyupAWBFrom()
{
    var awbfrom = $("#AWBFROM").val().trim();
    var awbto = $("#AWBTO").val().trim();
    var awbcount = $("#IssueAWB").val();
   
        if ($('#lstTotalCreatedStock').is(':disabled') == false) {
            if ($("#lstTotalCreatedStock option").length > 0) {
                $("#lstTotalCreatedStock option").each(function () {
               
                    if (awbto != "") {
                        if ($(this).val() >= awbfrom && $(this).val() <= awbto) {
                            // alert($(this).val())   
                            if (awbfrom != "" && awbto != "") {
                                $(this).attr('selected', 'selected')
                                $("#_tempIssueAWB").val($("#lstTotalCreatedStock option:selected").length)
                            ...
                            if (awbfrom.length >= 12 && $(this).val() >= awbfrom && $(this).index() <= LeftStockArr.length) {
                            // alert($(this).val())  && $(this).index() <= LeftStockArr.length 
                            if (awbfrom != "" && awbto == "") {
                                $(this).attr('selected', 'selected')
                                $("#_tempIssueAWB").val($("#lstTotalCreatedStock option:selected").length)
                                $("#IssueAWB").val($("#lstTotalCreatedStock option:selected").length)
                                if ($(this).index() == LeftStockArr.length-1) {
                                    $("#AWBTO").val($(this).val())
                                }
                            }
        ...
        else if ($('#lstTotalCreatedStock').is(':disabled') == true) {
            if ($("#lstIssueStockOffice option").length > 0) {
                $("#lstIssueStockOffice option").each(function () {
                    if (awbto != "") {
                        if ($(this).val() >= awbfrom && $(this).val() <= awbto) {
                            // alert($(this).val())
        ...
        else if ($('#lstIssueStockOffice').is(':disabled') == true) {
            if ($("#lstIssueStockAgent option").length > 0) {
                {
                    $("#lstIssueStockAgent option").each(function () {
                        if (awbto != "") {
                            if ($(this).val() >= awbfrom && $(this).val() <= awbto) {
                                // alert($(this).val())
                                if (awbfrom != "" && awbto != "") {
                                    $(this).attr('selected', 'selected')
                                    $("#_tempIssueAWB").val($("#lstIssueStockAgent option:selected").length)
                                    $("#IssueAWB").val($("#lstIssueStockAgent option:selected").length)
                                }
                                else {
                                    $(this).prop('selected', false);
                                    $("#_tempIssueAWB").val(" ");
...
function keyupAWBTo() {
    ...
    $("#_tempIssueAWB").val(" ");
    $("#IssueAWB").val(" ");
    var awbfrom = $("#AWBFROM").val();
    var awbto = $("#AWBTO").val();
    if ($('#lstTotalCreatedStock').is(':disabled') == false && $("#lstTotalCreatedStock option").length > 0) {    
            $("#lstTotalCreatedStock option").each(function () {
                if (awbto.length >= 12 && $(this).val() >= awbfrom && $(this).val() <= awbto) {
                    // alert($(this).val())
                  
                    if (awbfrom.length != "" && awbto != "") {
                        $(this).attr('selected', 'selected')
                        $("#_tempIssueAWB").val($("#lstTotalCreatedStock option:selected").length)
                        $("#IssueAWB").val($("#lstTotalCreatedStock option:selected").length)
                    }
...
//Ended Code by Shivali Thakur for awb stock start and end series filter
...
function IssueStock() {
    ...
    //if ($('#IssueAWB').val() == '' || $('#IssueAWB').val() == '0')
    //{ ShowMessage('info', 'Need your Kind Attention!', "Please Select AWB No"); return false; }
    if($('#AWBFROM').val() == '')
    { ShowMessage('info', 'Need your Kind Attention!', "Please Select Starting Series From:AWB No"); return false;}
    else {
        var CreatedAWB = "";
        var alradyCreatedStock = "";
        var AWBPrefix = $("#Text_AWBPrefix").val();
        ...
        if ($('#lstTotalCreatedStock option:selected').length > 0) {
            ...
            s = JSON.stringify(selval);
            sadd = s.replace(/"/g, "@}");
            strData = sadd.replace(/@}-/g, "A");
        }
        else if ($('#lstTotalCreatedStock option:selected').length == 0) {
            if ($('#IssueAWB').val() != "") {
                if ($('#lstTotalCreatedStock option').length > 0) {
                    for (var i = 0; i < NoOfAWB; i++) {
                        var Element = $('#lstTotalCreatedStock option:eq(' + i + ')').val();
                        selval.push(Element.substring(Element.indexOf('-'), 20));
                    }
                    s = JSON.stringify(selval);
                    sadd = s.replace(/"/g, "@}");
                    strData = sadd.replace(/@}-/g, "A");
        ...
        if (LeftStockArr.length > 0) {
            $.ajax({
                type: "POST",
                url: "./Services/Stock/StockManagementService.svc/IssueStock?AWBPrefix=" + AWBPrefix + "&CitySNo=" + CitySNo + "&OfficeSNo=" + OfficeSNo + "&Remark=" + Remark + "&NoOfAWB=" + NoOfAWB + "&AutoRetrievalDate=" + AutoRetrievalDate,
                data: JSON.stringify({ strData: btoa(strData) }),
                dataType: "json",
                contentType: "application/json; charset=utf-8",
                success: function (response) {
                    ...
                    if (response.length > 0) {
                        ...
                        GetReIssue(StockSno);
                        //GetIssuedOfficeStock(StockSno);
                        $("#lstIssueStockOffice").closest('td').append(buttonhtml);
    ...
    if (NoOfAWB1 == '' || NoOfAWB1 == '0') {
        ...
        if ($('#lstIssueStockOffice option:selected').length > 0) {
            ...
            if (NoOfAWB != "") {
                if ($('#lstIssueStockOffice option').length > 0) {
                    for (var i = 0; i < NoOfAWB; i++) {
                        var Element = $('#lstIssueStockOffice option:eq(' + i + ')').val();
                        selval.push(Element.substring(Element.indexOf('-'), 20));
                    }
                    s = JSON.stringify(selval);
                    sadd = s.replace(/"/g, "@}");
        ...
        if (IssueStockToAgentArr.length > 0) {
            $.ajax({
                type: "POST",
                url: "./Services/Stock/StockManagementService.svc/IssueStocktoAgent?AWBPrefix=" + AWBPrefix + "&AccountSNo=" + AccountSNo + "&OfficeSNo=" + OfficeSNo + "&Remark=" + Remark + "&NoOfAWB=" + NoOfAWB + "&AutoRetrievalDate=" + AutoRetrievalDate,
                data: JSON.stringify({ strData: btoa(strData) }),                
                dataType: "json",
                contentType: "application/json; charset=utf-8",
                success: function (response) {
...
function ExportToExcel_StockOffice() {
    ...
    var OfficeCity = $('#Text_City').val();
    OfficeName = (OfficeName.indexOf('-') == 0) ? OfficeName.substring(1) : OfficeName;
    var str = "";
     str = "<html><table border=\"1px\">";
     str += "<tr ><td><strong>AWB Number</strong></td> <td><strong>Office Name</strong></td></tr> <tr><td><strong>Office City</strong></td></tr>"
    ...
    $("#lstIssueStockOffice option").each(function (i, e) {
        ...
        var AWBNO = $("#lstIssueStockOffice ").find('option').eq(i).text()
            str += "<tr><td>" + AWBNO + "</td><td>" + OfficeName + "</td><td>"+ OfficeCity + "</td></tr>"


    });

    str += "</table></html>";
    var filename = 'Stock Issued to Office_' + today
    exportToExcelNew(str, filename)
