...
var length = 0;
var table1 = "AlertEvents_recipienttype";//"recipienttype";
var column1 = "Name";
$(document).ready(function () {
    ...
    cfi.AutoCompleteV2("TransactionType", "LOOKUPNAME", "SpaceControl_LookUp", null, "contains", ",");
    cfi.AutoCompleteV2("CitySNo", "CityCode,CityName", "AlertEvents_City", null, "contains");
    cfi.AutoCompleteV2("OfficeSNo", "SNo,Name", "AlertEvents_Office", null, "contains");
    cfi.AutoCompleteV2("AirlineSNo", "CarrierCode,AirlineName", "IndividualSalesReport_Airline", null, "contains", ",");

    //cfi.AutoCompleteV2("SPHCSNo", "code", "AlertEvents_SHC", null, "contains", ",");
    cfi.AutoCompleteV2("AlertEventSNo", "EventType", "AlertEvents_AlertEventType", ChangeRequired, "contains");
    ...
    cfi.AutoCompleteV2("TriggerNameSNo", "SNo,SubProcessName", "AlertEvents_GetSubProcessName", null, "contains");
   

    cfi.AutoCompleteV2("CommoditySNo", "CommodityCode,CommodityDescription", "MultipleCommodityAlertEvent", null, "contains", ",");
    cfi.BindMultiValue("CommoditySNo", $("#Text_CommoditySNo").val(), $("#CommoditySNo").val());
    
      //ADDED BY SHIVAM
    var TriggerEventType = [{ Key: "1", Text: "PROCESS" }, { Key: "2", Text: "STATUS" }];
    cfi.AutoCompleteByDataSource("TriggerEventTypeSNo", TriggerEventType , TypeOfEvent);
    ...
    if (getQueryStringValue("FormAction").toUpperCase() == "NEW") {
        ...
        //Added by Shivam
        $('#Text_TriggerEventTypeSNo').val('PROCESS');
        $('#TriggerEventTypeSNo').val(1);



        //if (userContext.AirlineSNo != "" && userContext.AirlineCarrierCode != "" && userContext.AirlineCarrierCode != undefined && userContext.AirlineSNo != undefined) {
...
function TypeOfEvent() {
    var TriggerType = $('#Text_TriggerEventTypeSNo').val()

    if (TriggerType === "PROCESS") {
        $('#Text_TriggerNameSNo').val('');
        $('#TriggerNameSNo').val('');
        cfi.AutoCompleteV2("TriggerNameSNo", "SNo,SubProcessName", "AlertEvents_GetSubProcessName", null, "contains");
       
    }
    else if (TriggerType === "STATUS") {
        $('#Text_TriggerNameSNo').val('');
        $('#TriggerNameSNo').val('');
        var StatusType = [{ Key: "1", Text: "BKD" }, { Key: "2", Text: "EXE" }, { Key: "3", Text: "FOH" }, { Key: "4", Text: "RCS" }, { Key: "5", Text: "MAN" }, { Key: "6", Text: "DEP" }, { Key: "7", Text: "NO SHOW" }, { Key: "8", Text: "CANCEL" }, { Key: "9", Text: "VOID" }];
        cfi.AutoCompleteByDataSource("TriggerNameSNo", StatusType);
...
function BindingAutocompleteOnRadioButtonAction() {
    if (getQueryStringValue("FormAction").toUpperCase() == "EDIT" || getQueryStringValue("FormAction").toUpperCase() == "DUPLICATE") {
        ...
        if ($("input[name^=SPHCType]:checked").val() == 0) {

            var SPHCCode = $('#SPHCCode').val();
            var Text_SPHCCode = $('#Text_SPHCCode').val();
            ...
            $('#SPHCCode').val(SPHCCode);
            $('#Text_SPHCCode').val(Text_SPHCCode);

            $("#SPHCSubGroupSNo").val('');
            $("#Text_SPHCSubGroupSNo").val('');
            $('#Text_SPHCSubGroupSNo').data("kendoAutoComplete").enable(false);
            $('#Text_SPHCCode').data("kendoAutoComplete").enable(true);

            $('td[title="Select SHC Sub Group"]').html("<span style='color:red'></span> SHC Sub Group");
            $('td[title="Select SHC Code"]').html("<span style='color:red'>*</span> SHC Code");

        }
        if ($("input[name^=SPHCType]:checked").val() == 1) {
            var SPHCSubGroupSNo = $('#SPHCSubGroupSNo').val();
            var Text_SPHCSubGroupSNo = $('#Text_SPHCSubGroupSNo ').val();
            var data = GetDataSourceV2("SPHCSubGroupSNo", "SHCDocument_SPHCGroup", "SNo", "Name", ["Name"], null);
            cfi.ChangeAutoCompleteDataSource("SPHCSubGroupSNo", data, true, null, "Name", "contains");

            $('#SPHCSubGroupSNo').val(SPHCSubGroupSNo);
            $('#Text_SPHCSubGroupSNo').val(Text_SPHCSubGroupSNo);

            $("#SPHCCode").val('');
            $("#Text_SPHCCode").val('');
            $('#Text_SPHCCode').data("kendoAutoComplete").enable(false);
            $('#Text_SPHCSubGroupSNo').data("kendoAutoComplete").enable(true);
    ...
    if (getQueryStringValue("FormAction").toUpperCase() == "READ" || getQueryStringValue("FormAction").toUpperCase() == "DELETE") {
        ...
        $('td[title="Airline Code"]').html("<span style='color:red'></span> Airline Code");
        $('td[title="City Code"]').html("<span style='color:red'></span> City Code");
        $('td[title="Office Name"]').html("<span style='color:red'></span> Office Name");
        $('td[title="SHC/SHC GROUP"]').html("<span style='color:red'></span> SHC/SHC GROUP");

        $('td[title="Transaction Type"]').html("<span style='color:red'></span>Transaction Type");

        if ($("input[name^=SPHCType]").val() == 'SHC Group') {

        }
        $("input[name^='SPHCType']").attr('disabled', true);
    }

    $("input[name^=SPHCType]").live("click", function () {
        if ($("input[name^=SPHCType]:checked").val() == 0) {
            var data = GetDataSourceV2("SPHCCode", "Master_CheckListType_SHCCode");
            cfi.ChangeAutoCompleteDataSource("SPHCCode", data, true, SPHCFunction, "Code", "contains");
...
function SaveAlertEventsDetail() {
    ...
    addintext();
    $("#hdnFormData").val(btoa($('#tblRecipient').appendGrid('getStringJson')));
    $("#Email").val(btoa($("#Email").val()));
    $("#Email").attr("oldvalue", btoa($("#Email").attr("oldvalue")));
    isSubmit = true;
}
...
function ExtraCondition(textId) {
    ...
    if (textId == "Text_OfficeSNo") {
        ...
        cfi.setFilter(filterEmbargo, "AirlineSNo", "in", $("#AirlineSNo").val());
    ...
    if (textId == "Text_AirlineSNo") {
        ...
        cfi.setFilter(filterEmbargo, "SNo", "notin", $("#AirlineSNo").val())
        var OriginCityAutoCompleteFilter2 = cfi.autoCompleteFilter([filterEmbargo]);
        return OriginCityAutoCompleteFilter2;
    ...
    if (textId == "Text_TransactionType")
        return textId = cfi.getFilter("AND"), cfi.setFilter(textId, "LOOKUPTYPENAME", "eq", 'TransactionType'), cfi.setFilter(textId, "LOOKUPCODE", "notin", $('#TransactionType').val()), cfi.autoCompleteFilter(textId);
    ...
    if (textId == "Text_SPHCSubGroupSNo") {
        ...
        if ($('#SPHCCode').val() != undefined && $('#SPHCCode').val() != "" && $('#SPHCCode').val() != 0) {
...
function getGrid() {
    ...
    $('#tblRecipient').appendGrid({
        ...
        columns: [{ name: 'SNo', type: 'hidden', value: 0 },
                ...
                {
                    name: "Name", display: "Name", type: (pageType == "NEW" || pageType == "EDIT") ? "text" : "label", ctrlAttr: { controltype: pageType == "NEW" || pageType == "EDIT" ? 'autocomplete' : 'label', placeholder: "ALL" }, ctrlCss: { width: '100px', height: '20px' }, isRequired: (pageType == "NEW" || pageType == "EDIT" ? true : false), AutoCompleteName: table1, filterField: column1,  //tableName: table1, textColumn: column1, keyColumn: 'SNo',
                    filterCriteria: "contains", onChange: function (evt, rowIndex) {
                ...
                {
                    name: "MobileNo", display: "Mobile No.(Max Allowed 3)", type: (pageType == "NEW" || pageType == "EDIT") ? "text" : "label", ctrlCss: { width: '100px' }, ctrlAttr: { maxlength: 12, onfocus: "return adddivMobile(this);", oninput: "this.value = this.value.replace(/[^0-9.]/g, '');" }, isRequired: (pageType == "NEW" || pageType == "EDIT" ? true : false)
                },
        ],
