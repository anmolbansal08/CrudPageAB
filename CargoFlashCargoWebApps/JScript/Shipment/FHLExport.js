...
function InitializePage(subprocess, cntrlid, isdblclick) {
    ...
    if (subprocess.toUpperCase().trim() == "FHL PRINT".trim()) {


        BindFHLSectionPrintHouse()
    }
...
function BindCountryCodeAutoComplete(elem, mainElem) {
    $(elem).find("input[id^='CountryCode']").each(function () {
        cfi.AutoCompleteV2($(this).attr("name"), "CountryCode,CountryName", "FHLExport_CountryCode", null, "contains");
    });

    $(elem).find("input[id^='InfoType']").each(function () {
        cfi.AutoCompleteV2($(this).attr("name"), "InformationCode,Description", "FHLExport_InformationCode", null, "contains");
    });

    $(elem).find("input[id^='CSControlInfoIdentifire']").each(function () {
        cfi.AutoCompleteV2($(this).attr("name"), "CustomsCode,Description", "FHLExport_CustomsCode", null, "contains");
    });
...
function ReBindCountryCodeAutoComplete(elem, mainElem) {
    $(elem).closest("div[id$='areaTrans_import_fwbshipmentocitrans']").find("[id^='areaTrans_import_fwbshipmentocitrans']").each(function () {
        $(this).find("input[id^='CountryCode']").each(function () {
            var newDataSource = GetDataSourceV2("Text_" + $(this).attr("id"), "FHLExport_CountryCode");
            cfi.ChangeAutoCompleteDataSource($(this).attr("name"), newDataSource, false);
        });

        $(this).find("input[id^='InfoType']").each(function () {
            var newDataSource = GetDataSourceV2("Text_" + $(this).attr("id"), "FHLExport_InformationCode");
            cfi.ChangeAutoCompleteDataSource($(this).attr("name"), newDataSource, false);
        });

        $(this).find("input[id^='CSControlInfoIdentifire']").each(function () {
            var newDataSource = GetDataSourceV2("Text_" + $(this).attr("id"), "FHLExport_CustomsCode");
            cfi.ChangeAutoCompleteDataSource($(this).attr("name"), newDataSource, false);
        });
    });
...
function SaveHSCode() {
    ...
    ShowMessage('success', 'Success - HS Code', "Save Successfully", "bottom-right");
...
function CalculateHAWBTotalVolumeWeight(obj) {
    var CalculateVolumeWeight = $("#HAWBTotalVolumeWeight").val();
    $("#HAWBTotalVolumeWeight").val(GetroundValue(CalculateVolumeWeight, 1));
    $("#_tempHAWBTotalVolumeWeight").val(GetroundValue(CalculateVolumeWeight, 1));
}
...
function ModifyFHL() {
    if (userContext.SpecialRights.HAWBA == true) {
        $("#HAWBNOOfHouse").attr('disabled', false);
        $("#_tempHAWBNOOfHouse").attr('disabled', false);
        $("#HAWBTotalPieces").attr('disabled', false);
        $("#_tempHAWBTotalPieces").attr('disabled', false);
        $("#HAWBTotalGrossWeight").attr('disabled', false);
        $("#_tempHAWBTotalGrossWeight").attr('disabled', false);
        $("#HAWBTotalVolumeWeight").attr('disabled', false);
        $("#_tempHAWBTotalVolumeWeight").attr('disabled', false);

        $("#btnModifyFHL").css("display", "none");
        $("#btnUpdateModifyFHL").css("display", "");
        $("#btnSave").css("display", "none");
        $("#divDetail1").html("");
    }
}
function UpdateModifyFHL() {
    var Conditions = true;
    if (userContext.SpecialRights.HAWBA == true) {
        var HAWBNOOfHouse = $("#HAWBNOOfHouse").val();
        var HAWBTotalPieces = $("#HAWBTotalPieces").val();
        var HAWBTotalGrossWeight = $("#HAWBTotalGrossWeight").val();
        var HAWBTotalVolumeWeight = $("#HAWBTotalVolumeWeight").val();

        if (HAWBNOOfHouse > 0 && HAWBTotalPieces > 0 && HAWBTotalGrossWeight > 0 && HAWBTotalVolumeWeight > 0) {
            if (HAWBNOOfHouse < TotalHAWBNOOfHouse && TotalHAWBNOOfHouse > 0) {
                ShowMessage('warning', 'Information', "No Of House can not be less than already saved HAWB Details.");
                Conditions = false;
                return;
            }
        }
        ...
        if (Conditions == true) {
            $.ajax({
                url: "Services/Shipment/FHLExportService.svc/UpdateModifyFHL", async: false, type: "POST", dataType: "json", cache: false,
                data: JSON.stringify({ AWBSNo: currentawbsno, HAWBNOOfHouse: HAWBNOOfHouse, HAWBTotalPieces: HAWBTotalPieces, HAWBTotalGrossWeight: HAWBTotalGrossWeight, HAWBTotalVolumeWeight: HAWBTotalVolumeWeight }),
                contentType: "application/json; charset=utf-8",
                success: function (result) {
                    if (result.split('?')[0] == "0") {
...
var TotalHAWBNOOfHouse = 0;
var TotalHAWBTotalPieces = 0;
var TotalHAWBTotalGrossWeight = 0.000;
var TotalHAWBTotalVolumeWeight = 0.000;
function BindFHLSection() {
    TotalHAWBNOOfHouse = 0;
    TotalHAWBTotalPieces = 0;
    TotalHAWBTotalGrossWeight = 0.000;
    TotalHAWBTotalVolumeWeight = 0.000;
    $.ajax({
        ...
        success: function (result) {
            ...
            if (result.substring(1, 0) == "{") {
                ...
                if (myData.Table0.length > 0) {
                    var mydata = myData.Table0;
                    ISProcessed = mydata[0].ISProcessed;
                    $("input[id='FHL_HAWB_NatureofGoods']").after("<a title='HS Code' style='color: blue; margin - right: 15px; white - space:nowrap' id='ahref_HSCode' href='javascript: void (0); '><span id='HSCode'>HS Code</span></a>");
                    ...
                    table += "<td class='ui-widget-content first'>" + myData.Table0[0].AWBNo + "<input type='hidden' id='AWBSNo' value='" + myData.Table0[0].SNo + "'></input></td><td class='ui-widget-content first'><input type='text' class='' name='HAWBNOOfHouse' style='width: 50px;' id='HAWBNOOfHouse' controltype='number' maxlength='4' value=" + myData.Table0[0].NoOfHouse + " data-role='numerictextbox'><input type='hidden' id='hdnNoOfHouse' value=" + myData.Table0[0].NoOfHouse + " /></td><td class='ui-widget-content first'>" + myData.Table0[0].Origin + "</td><td class='ui-widget-content first'>" + myData.Table0[0].Destination + "</td><td class='ui-widget-content first'><input type='text' class='' name='HAWBTotalPieces' style='width: 50px;' id='HAWBTotalPieces' controltype='number' maxlength='3' value=" + myData.Table0[0].TotalPieces + " data-role='numerictextbox'><input type='hidden' id='hdnTotalPieces' value=" + myData.Table0[0].TotalPieces + " /></td><td class='ui-widget-content first'><input type='text' class='' name='HAWBTotalGrossWeight' style='width: 60px;' id='HAWBTotalGrossWeight' controltype='decimal3' maxlength='10' value=" + myData.Table0[0].TotalGrossWeight + " data-role='numerictextbox'><input type='hidden' id='hdnTotalGrossWeight' value=" + myData.Table0[0].TotalGrossWeight + " /></td><td class='ui-widget-content first'><input type='text' class='' name='HAWBTotalVolumeWeight' onblur='CalculateHAWBTotalVolumeWeight(this);' style='width: 60px;' id='HAWBTotalVolumeWeight' controltype='decimal3' maxlength='10' value=" + myData.Table0[0].TotalVolumeWeight + " data-role='numerictextbox'><input type='hidden' id='hdnTotalVolumeWeight' value=" + myData.Table0[0].TotalVolumeWeight + " /></td>";

                    if (myData.Table0[0].NoOfHouse > 0 && myData.Table1.length < myData.Table0[0].NoOfHouse) {
                        if (userContext.SpecialRights.HAWBA == true && ISProcessed != "1")
                            table += "<td class='ui-widget-content first'><button aria-disabled='false' role='button' title='Submit' type='button' id='btnSubmit' value=" + myData.Table0[0].AWBNo + " tabindex='16' class='btn btn-success' style='width:60px;' onclick='AddFHL();'><span class='ui-button-text'>Add</span></button><button aria-disabled='false' role='button' title='Submit' type='button' id='btnUpdateModifyFHL' value=" + myData.Table0[0].SNo + " tabindex='16' class='btn btn-success' style='width:60px;display:none;' onclick='UpdateModifyFHL();'><span class='ui-button-text'>Update</span></button><button aria-disabled='false' role='button' title='Submit' type='button' id='btnModifyFHL' value=" + myData.Table0[0].SNo + " tabindex='16' class='btn btn-success' style='width:60px;' onclick='ModifyFHL();'><span class='ui-button-text'>Modify</span></button></td>";
                        else
                        ...
                        if (userContext.SpecialRights.HAWBA == true && ISProcessed != "1")
                            table += "<td class='ui-widget-content first'><button aria-disabled='false' role='button' title='Submit' type='button' id='btnUpdateModifyFHL' value=" + myData.Table0[0].SNo + " tabindex='16' class='btn btn-success' style='width:60px;display:none;' onclick='UpdateModifyFHL();'><span class='ui-button-text'>Update</span></button><button aria-disabled='false' role='button' title='Submit' type='button' id='btnModifyFHL' value=" + myData.Table0[0].SNo + " tabindex='16' class='btn btn-success' style='width:60px;' onclick='ModifyFHL();'><span class='ui-button-text'>Modify</span></button></td>";
                        else
                    ...
                    if (myData.Table1.length > 0) {
                        ...
                        for (var i = 0; i < myData.Table1.length; i++) {
                            TotalHAWBNOOfHouse = TotalHAWBNOOfHouse + parseInt(1);
                            TotalHAWBTotalPieces = TotalHAWBTotalPieces + parseInt(myData.Table1[i].TotalPieces);
                            TotalHAWBTotalGrossWeight = TotalHAWBTotalGrossWeight + parseFloat(myData.Table1[i].TotalGrossWeight);
                            TotalHAWBTotalVolumeWeight = TotalHAWBTotalVolumeWeight + parseFloat(myData.Table1[i].TotalVolumeWeight);
                            ...
                            if (ISProcessed == "1" && ISProcessed != "undefined") {
                                FHLtable += "<td class='ui-widget-content first'><button aria-disabled='false' role='button' title='Submit' type='button' id='btnUpdateFHLGrid' value=" + myData.Table1[i].SNo + " tabindex='16' class='btn btn-success' style='width:50px;' disabled><span class='ui-button-text'>Edit</span></button>"
                            }
                            else {
                                FHLtable += "<td class='ui-widget-content first'><button aria-disabled='false' role='button' title='Submit' type='button' id='btnUpdateFHLGrid' value=" + myData.Table1[i].SNo + " tabindex='16' class='btn btn-success' style='width:50px;' onclick='UpdateFHL(" + myData.Table1[i].SNo + ");'><span class='ui-button-text'>Edit</span></button>"
                            }
                            FHLtable += "<button aria-disabled='false' role='button' title='Submit' type='button' id='btnViewFHLGrid' value=" + myData.Table1[i].SNo + " tabindex='16' class='btn btn-success' style='width:50px;' onclick='ViewFHL(" + myData.Table1[i].SNo + ");'><span class='ui-button-text'>View</span></button>"
                            if (ISProcessed == "1" && ISProcessed != "undefined") {
                                FHLtable += "<button aria-disabled='false' role='button' title='Submit' type='button' id='btnDeleteFHLGrid' value=" + myData.Table1[i].SNo + " tabindex='16' class='btn btn-danger' style='width:50px;' disabled><span class='ui-button-text'>Delete</span></button></td></tr>";
                            }
                            else {
                                FHLtable += "<button aria-disabled='false' role='button' title='Submit' type='button' id='btnDeleteFHLGrid' value=" + myData.Table1[i].SNo + " tabindex='16' class='btn btn-danger' style='width:50px;' onclick='DeleteFHL(" + myData.Table1[i].SNo + ");'><span class='ui-button-text'>Delete</span></button></td></tr>";
                            }
                    ...
                    $("#HAWBNOOfHouse").attr('disabled', true);
                    $("#_tempHAWBNOOfHouse").attr('disabled', true);
                    $("#HAWBTotalPieces").attr('disabled', true);
                    $("#_tempHAWBTotalPieces").attr('disabled', true);
                    $("#HAWBTotalGrossWeight").attr('disabled', true);
                    $("#_tempHAWBTotalGrossWeight").attr('disabled', true);
                    $("#HAWBTotalVolumeWeight").attr('disabled', true);
                    $("#_tempHAWBTotalVolumeWeight").attr('disabled', true);
                }
                else {
...
function BindFHLSectionPrintHouse() {
    $.ajax({
        url: "Services/Shipment/FHLExportService.svc/BindFHLSectionPrintHouse",
        async: false,
        type: "GET",
        dataType: "json",
        data: { AWBSNO: currentawbsno },
        ...
        success: function (result) {
            ...
            var myData = jQuery.parseJSON(result);
            if (myData.Table0.length > 0) {
                myData.Table0[0].MessageBox
                if (myData.Table0[0].MessageBox != "") {

                    ShowMessage('warning', 'Warning - HAWB PRINT', myData.Table0[0].MessageBox, "bottom-right");
                    return;

                } else {

                    FHLtable = "</BR><table class='appendGrid ui-widget' id='tblFHL'><thead class='ui-widget-header' style='text-align:center'><tr><td colspan='11' class='ui-widget-header'>PRINT HAWB INFORMATION</td></tr><tr><td class='ui-widget-header'>HAWB No</td><td class='ui-widget-header'>Origin City</td><td class='ui-widget-header'>Destination City</td><td class='ui-widget-header'>Pieces</td><td class='ui-widget-header'>Gross weight</td><td class='ui-widget-header'>Volume weight</td><td class='ui-widget-header'>Consignee Name</td><td class='ui-widget-header'>SHC</td><td class='ui-widget-header'>Freight Type</td><td class='ui-widget-header'>Nature of Goods</td><td class='ui-widget-header'></td></tr></thead><tbody class='ui-widget-content'>";
                    for (var i = 0; i < myData.Table0.length; i++) {
                        FHLtable += "<tr><td class='ui-widget-content first'>" + myData.Table0[i].HAWBNo
                            + "</td><td class='ui-widget-content first'>" + myData.Table0[i].Origin
                            + "</td><td class='ui-widget-content first'>" + myData.Table0[i].Destination
                            + "</td><td class='ui-widget-content first'>" + myData.Table0[i].TotalPieces
                            + "</td><td class='ui-widget-content first'>" + myData.Table0[i].TotalGrossWeight
                            + "</td><td class='ui-widget-content first'>" + myData.Table0[i].TotalVolumeWeight
                            + "</td><td class='ui-widget-content first'>" + myData.Table0[i].CustomerName.toUpperCase()
                            + "</td><td class='ui-widget-content first'>" + myData.Table0[i].SPHC.toUpperCase()
                            + "</td><td class='ui-widget-content first'>" + myData.Table0[i].FreightType
                            + "</td><td class='ui-widget-content first'>" + myData.Table0[i].NatureOfGoods.toUpperCase() + "</td>"

                        FHLtable += "<td class='ui-widget-content first'><button aria-disabled='false' role='button' title='Submit' type='button' id='btnViewFHLGrid' value=" + myData.Table0[i].SNo + " tabindex='16' class='btn btn-success' style='width:50px;' onclick='PrintViewFHL(" + myData.Table0[i].SNo + ");'><span class='ui-button-text'>Print</span></button><td>"
                    }
                    FHLtable += "</tbody></table>";
                    theDiv.innerHTML += FHLtable;
...
function PrintViewFHL(Sno) {
    var tnc = userContext.SysSetting.AWBPRINTTERMSCONDITIONS;

    if (currentawbsno > 0)
        window.open("FHLPrintA4.html?sno=" + btoa(currentawbsno) + "&pagename=" + btoa('FHLExport') + "&tnc=" + btoa(tnc) + "&HAWBNo=" + btoa(Sno));
    else
        ...
        $.ajax({
            ...
            success: function (result) {
                if (result == "0") {
                    ...
                    ShowMessage('warning', 'warning - HAWB', "HAWB has already been accepted in Dimension, can't be deleted.", "bottom-right");
                    flag = true;
                }
                else {
                    ShowMessage('warning', 'Warning - HAWB', "HAWB unable to process.", "bottom-right");
...
function ViewFHL(SNo) {
    $.ajax({
        ...
        success: function (result) {
            ...
            cfi.AutoCompleteV2("FHL_HAWB_Origin", "AirportCode", "FHLExport_AirportName", null, "contains");
            cfi.AutoCompleteV2("FHL_HAWB_Destination", "AirportCode", "FHLExport_AirportName", null, "contains");
            cfi.AutoCompleteV2("FHL_HAWB_Commodity", "CommodityCode", "FHLExport_CommodityCode", null, "contains");
            cfi.AutoCompleteV2("FHL_HAWB_SPHC", "CODE", "FHLExport_CODE", null, "contains", ",");
            cfi.AutoCompleteV2("FHL_HAWB_AWBCurrency", "CurrencyCode", "FHLExport_CurrencyCode", null, "contains");
            cfi.AutoCompleteV2("FHL_HAWB_SHI_AccountNo", "CustomerNo", "FHLExport_CustomerNo", GetShipperConsigneeDetailsFHL, "contains");
            cfi.AutoCompleteV2("FHL_HAWB_SHI_CountryCode", "CountryCode,CountryName", "FHLExport_CountryCode", ClearAutoComplete, "contains");

            cfi.AutoCompleteV2("FHL_HAWB_SHI_City", "CityCode,CityName", "FHLExport_CityName", null, "contains");
            cfi.AutoCompleteV2("FHL_HAWB_CON_AccountNo", "CustomerNo", "FHLExport_CustomerNo", GetShipperConsigneeDetailsFHL, "contains");
            cfi.AutoCompleteV2("FHL_HAWB_CON_CountryCode", "CountryCode,CountryName", "FHLExport_CountryCode", ClearAutoComplete, "contains");
            cfi.AutoCompleteV2("FHL_HAWB_CON_City", "CityCode,CityName", "FHLExport_CityName", null, "contains");
            cfi.AutoCompleteV2("FHL_HAWB_CD_Currency", "CurrencyCode", "FHLExport_CurrencyCode", null, "contains");
            ...
            cfi.AutoCompleteV2("FHL_OCI_CountryCode", "CountryCode,CountryName", "FHLExport_CountryCode", null, "contains");
            cfi.AutoCompleteV2("FHL_OCI_InfoType", "InformationCode,Description", "FHLExport_InformationCode", null, "contains");
            cfi.AutoCompleteV2("FHL_OCI_CSControlInfoIdentifire", "CustomsCode,Description", "FHLExport_CustomsCode", null, "contains");
            $("input[id='FHL_HAWB_NatureofGoods']").after("<a title='HS Code' style='color: blue; margin - right: 15px; white - space:nowrap' id='ahref_HSCode' href='javascript: void (0); '><span id='HSCode'>HS Code</span></a>");
            $("a[id^='ahref_HSCode']").unbind("click").bind("click", function () {
                var theDivHSCodeDetails = document.getElementById("divHSCodePopUp");
                theDivHSCodeDetails.innerHTML = "";
                var table = "<table border='0' cellspacing='0' cellpadding='0' style='width: 100%; margin: 0px; padding: 0px;'></table><table class='appendGrid ui-widget'><tbody class='ui-widget-content'>";
                table += "<tr><td class='ui-widget-content first' style='text-align: left;'><table class='appendGrid ui-widget'><tbody class='ui-widget-content'><tr><td><label>HS Code 1: </label><div><input type='hidden' name='HSCode1' id='HSCode1' value=''><input type='text' class='' name='Text_HSCode1' id='Text_HSCode1' controltype='autocomplete' maxlength='50' value='' placeholder='' data-role='autocomplete' autocomplete='off' style='text-transform: uppercase;'></div></td><td><label>HS Code 2: </label><div><input type='hidden' name='HSCode2' id='HSCode2' value=''><input type='text' class='' name='Text_HSCode2' id='Text_HSCode2' controltype='autocomplete' maxlength='50' value='' placeholder='' data-role='autocomplete' autocomplete='off' style='text-transform: uppercase;'></div></td><td><label>HS Code 3: </label><div><input type='hidden' name='HSCode3' id='HSCode3' value=''><input type='text' class='' name='Text_HSCode3' id='Text_HSCode3' controltype='autocomplete' maxlength='50' value='' placeholder='' data-role='autocomplete' autocomplete='off' style='text-transform: uppercase;'></div></td><td><label>HS Code 4: </label><div><input type='hidden' name='HSCode4' id='HSCode4' value=''><input type='text' class='' name='Text_HSCode4' id='Text_HSCode4' controltype='autocomplete' maxlength='50' value='' placeholder='' data-role='autocomplete' autocomplete='off' style='text-transform: uppercase;'></div></td><td></tr><tr><td><label>HS Code 5: </label><div><input type='hidden' name='HSCode5' id='HSCode5' value=''><input type='text' class='' name='Text_HSCode5' id='Text_HSCode5' controltype='autocomplete' maxlength='50' value='' placeholder='' data-role='autocomplete' autocomplete='off' style='text-transform: uppercase;'></div></td><td><label>HS Code 6: </label><div><input type='hidden' name='HSCode6' id='HSCode6' value=''><input type='text' class='' name='Text_HSCode6' id='Text_HSCode6' controltype='autocomplete' maxlength='50' value='' placeholder='' data-role='autocomplete' autocomplete='off' style='text-transform: uppercase;'></div></td><td><label>HS Code 7: </label><div><input type='hidden' name='HSCode7' id='HSCode7' value=''><input type='text' class='' name='Text_HSCode7' id='Text_HSCode7' controltype='autocomplete' maxlength='50' value='' placeholder='' data-role='autocomplete' autocomplete='off' style='text-transform: uppercase;'></div></td><td><label>HS Code 8: </label><div><input type='hidden' name='HSCode8' id='HSCode8' value=''><input type='text' class='' name='Text_HSCode8' id='Text_HSCode8' controltype='autocomplete' maxlength='50' value='' placeholder='' data-role='autocomplete' autocomplete='off' style='text-transform: uppercase;'></div></td><tr><td><label>HS Code 9: </label><div><input type='hidden' name='HSCode9' id='HSCode9' value=''><input type='text' class='' name='Text_HSCode9' id='Text_HSCode9' controltype='autocomplete' maxlength='50' value='' placeholder='' data-role='autocomplete' autocomplete='off' style='text-transform: uppercase;'></div></td><td></td><td></td><td><input type='submit' name='btnSavHSCodeData' id='btnSavHSCodeData' tabindex='21' onclick='SaveHSCode();' value='Save' style='display: none;' class='btn btn-block btn-success btn-sm'></td></tr></table></td></tr>";
                table += "</tbody></table>";
                theDivHSCodeDetails.innerHTML += table;
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            if (MainData.length > 0) {
                ...
                $("#_tempFHL_HAWB_Pieces").attr("disabled", "disabled");
                $("#_tempFHL_HAWB_GrossWeight").attr("disabled", "disabled");
                $("#_tempFHL_HAWB_VolumeWeight").attr("disabled", "disabled");
                $("#_tempFHL_HAWB_ChargeableWeight").attr("disabled", "disabled");
                ...
                if (MainData[0].HSCode != "" && MainData[0].Text_HSCode != "") {
                    HSCode = MainData[0].HSCode;
                    Text_HSCode = MainData[0].Text_HSCode;
                }
                ...
                if (shipperData.length > 0) {
                    ...
                    $("#_tempFHL_HAWB_SHI_MobileNo").attr("disabled", "disabled");
                    $("#_tempFHL_HAWB_SHI_Fax").attr("disabled", "disabled");
                ...
                if (consigneeData.length > 0) {
                    ...
                    $("#_tempFHL_HAWB_CON_MobileNo").attr("disabled", "disabled");
                    $("#_tempFHL_HAWB_CON_Fax").attr("disabled", "disabled");
            ...
            $("#_tempFHL_HAWB_CD_DecCarriageVal").attr("disabled", "disabled");
            $("#_tempFHL_HAWB_CD_DecCustomsVal").attr("disabled", "disabled");
            $("#_tempFHL_HAWB_CD_Insurance").attr("disabled", "disabled");
            $("#_tempFHL_HAWB_CD_ValuationCharge").attr("disabled", "disabled");
            ...
            $("#chkshippertaxid").attr("disabled", "disabled");
            ...
            $("#chkconsigneetaxid").attr("disabled", "disabled");
    ...
    $('#__divfhl__ table td').removeAttr("title"); //Remove Tootip for view case
}
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            if (MainData.length > 0) {
                ...
                if (MainData[0].CVDDeclareCarriageValue == "NVD") {
                    $("#FHL_HAWB_CD_DecCarriageVal").val('');
                    $("#_tempFHL_HAWB_CD_DecCarriageVal").val('');
                }
                else {
                ...
                if (MainData[0].CVDDeclareCustomValue == "NCV") {
                    $("#FHL_HAWB_CD_DecCustomsVal").val('');
                    $("#_tempFHL_HAWB_CD_DecCustomsVal").val('');
                }
                else {
                ...
                if (MainData[0].CVDDeclareInsurenceValue == "XXX") {
                    $("#FHL_HAWB_CD_Insurance").val('');
                    $("#_tempFHL_HAWB_CD_Insurance").val('');
                }
                else {
...
function UpdateFHLData(subprocess, Sno) {
    ...
    $.ajax({
        ...
        data: JSON.stringify({ AWBSNo: currentawbsno, ArrivedShipmentSNo: currentArrivedShipmentSNo, HAWBSNo: Sno, HAWBInformation: HAWBViewModel, ShipperInformation: ShipperViewModel, ConsigneeInformation: ConsigneeViewMode, ChargeDeclarationsInformation: ChargeDeclarationsViewMode, OCIInformation: OCIInfoModel, UpdatedBy: 2, ShipperSno: $("#Text_FHL_HAWB_SHI_AccountNo").data("kendoAutoComplete").key() == "" ? 0 : $("#Text_FHL_HAWB_SHI_AccountNo").data("kendoAutoComplete").key(), ConsigneeSno: $("#Text_FHL_HAWB_CON_AccountNo").data("kendoAutoComplete").key() == "" ? 0 : $("#Text_FHL_HAWB_CON_AccountNo").data("kendoAutoComplete").key(), HSCode: HSCode || "", CreateShipperTaxParticipants: CreateShipperTaxParticipants, CreateConsigneeTaxParticipants: CreateConsigneeTaxParticipants, CreateShipperTaxId: CreateShipperTaxId, CreateConsigneeTaxId: CreateConsigneeTaxId }),
        contentType: "application/json; charset=utf-8",
        success: function (result) {
            ...
            if (result == "0") {
                ...
                ShowMessage('warning', 'warning - HAWB', "HAWB has already been accepted in Dimension. No more changes are allowed.", "bottom-right");
                flag = true;
            }
            else {
                ShowMessage('warning', 'Warning - HAWB', "HAWB unable to process.", "bottom-right");
    ...
    $.ajax({
        ...
        data: JSON.stringify({ HarmonizedCommodityCode: HarmonizedCommodityCode, HAWBNo: HAWBNo, AWBSNo: AWBSNo }),
        contentType: "application/json; charset=utf-8",
        success: function (result) {
            if (result == "0") {
    ...
    $.ajax({
        ...
        data: JSON.stringify({ HawbDescription: HawbDescription, HAWBNo: HAWBNo, AWBSNo: AWBSNo }),
        contentType: "application/json; charset=utf-8",
        success: function (result) {
            if (result == "0") {
...
function SaveFHLinfo() {
    ...
    //if (CreateShipperTaxParticipants == "1")
    //    {
    //    if (CheckTaxId(CreateShipperTaxId, 'S', ShipperViewModel.ShipperCountryCode) == false)
    //{
    //    alert('asas');
    //}
    //}
    //if (CheckTaxId(CreateConsigneeTaxId, 'C', ConsigneeViewMode.ConsigneeCountryCode) == false) {
    //    alert('asa353453s');
    //}
    $.ajax({
        ...
        success: function (result) {
            if (result == "0") {
                ...
                ShowMessage('warning', 'Warning - HAWB', "All HAWB already processed.", "bottom-right");
                flag = false;
            }
            else {
                ShowMessage('warning', 'Warning - HAWB', "HAWB unable to process.", "bottom-right");
...
function GetShipperConsigneeDetails_tax(usertype) {
    ...
    if (checkTaxIdResult == true) {
        if (TaxId != "" && IsGetRecords == "1") {
            $.ajax({
                url: "Services/Shipment/FHLExportService.svc/GetShipperConsigneeDetails_TaxID?UserType=" + UserTyp + "&TaxId=" + TaxId, async: false, type: "get", dataType: "json", cache: false,
                contentType: "application/json; charset=utf-8",
                success: function (result) {
                    var Data = jQuery.parseJSON(result);
                    ...
                    if (shipperData.length > 0) {
                        if (UserTyp == "S") {
                            ...
                            $("#Text_FHL_HAWB_CON_AccountNo").data("kendoAutoComplete").setDefaultValue(shipperData[0].ConsigneeAccountNo, shipperData[0].CustomerNo);
                            ...
                            $("#FHL_HAWB_CON_Fax").val(shipperData[0].Fax);
                            $("#_tempFHL_HAWB_CON_Fax").val(shipperData[0].Fax);
...
function GetConsignee() {
    ...
    GetShipperConsigneeDetails_tax('C');
}
function GetShipper() {

    GetShipperConsigneeDetails_tax('S');
}

var YesReady = false;
function PageRightsCheckFHLEXPORT() {
    var CheckIsFalse = 0;
    $(userContext.PageRights).each(function (e, i) {
    
        if (i.Apps.toString().toUpperCase() == "FHLEXPORT") {
         
                if (i.Apps.toString().toUpperCase() == "FHLEXPORT" && i.PageRight == "New") {
                    YesReady = false;
                    CheckIsFalse = 1;
                    return
                } if (i.Apps.toString().toUpperCase() == "FHLEXPORT" && i.PageRight == "Edit") {
                    YesReady = false;
                    CheckIsFalse = 1;
                    return
                } if (i.Apps.toString().toUpperCase() == "FHLEXPORT" && i.PageRight == "Delete") {
                    YesReady = false;
                    CheckIsFalse = 1;
                    return
...
var fotter = "<div><table style='margin-left:20px;'>" +
    ...
    "<td> &nbsp; &nbsp;</td>" +
    "<td><button class='btn btn-block btn-danger btn-sm'  id='btnCancel'>Back</button></td>" +
