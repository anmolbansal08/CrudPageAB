...
$(document).ready(function () {
    ...
    cfi.AutoCompleteV2("AirlineSNo", "AirlineName", "Shipment_UCM_Airline", null, "contains");

    $("#FlightDate").find(".flightdatepicker").kendoDatePicker({ format: "yyyy-MMM-dd", value: new Date() });
    cfi.AutoCompleteV2("FlightNo", "FlightNo", "Shipment_UCM_FlightNo", null, "contains");

    var Content = [{ Key: "0", Text: "CARGO" }, { Key: "1", Text: "BAGGAGE" }, { Key: "2", Text: "EMPTY" }];
    cfi.AutoCompleteByDataSource("Content", Content, null);

    //var UCMType = [{ Key: "0", Text: "OUTBOUND" }, { Key: "1", Text: "INBOUND" }];
    var UCMType = [{ Key: "0", Text: "UCM Out" }, { Key: "1", Text: "UCM In" }];
    ...
    $("#AirlineSNo").val(userContext.AirlineCarrierCode.split('-')[0]);
    ...
    $("#Container").hide();
    origincity = userContext.AirportCode;
    ...
    $("#divUCM").append("<br/><span style='font-weight:bold;font-size: 9pt;color: #5A7570;font-family: Verdana;height:25px;text-align:left;vertical-align: middle;background-color:#F7F7F7;padding-left: 7px;' >Add ULD <input type='radio'name='Pallet' id='Pallet' value='2' >Yes     <input type='radio'  name='Pallet' id='Pallet' value='1' checked='True'>No</span><hr/><br/><table style='width:100%' border='0' cellpadding='1' cellspacing='1' id='tbPallet' style='display:block'><tr><td width=''><b>ULD Code</b> &nbsp;&nbsp;<input type='hidden' name='ULDType' id='ULDType' value=''/>        <input type='text'  name='Text_ULDType' id='Text_ULDType' tabindex='5' controltype='autocomplete' maxlength='5' /> </td><td><b>Serial No</b>&nbsp;&nbsp;<input type='text' maxlength='5' tabindex='6' name='SerialNo' id='SerialNo'  onBlur='CheckSNo(this)' onkeyup='checkInput(this)'  width='30px'/> </td><td><b>Owner Code</b>&nbsp;&nbsp;<input type='hidden' name='OwnerCode' id='OwnerCode' value=''/><input type='text'  name='Text_OwnerCode' id='Text_OwnerCode' tabindex='7' controltype='autocomplete' maxlength='5' /></td><td><div id='tbdest'><b>OffPoint</b> &nbsp;&nbsp;<input type='hidden' name='Destination' id='Destination' value=''/><input type='text'  name='Text_Destination' id='Text_Destination' tabindex='7' controltype='autocomplete' maxlength='5' /></div></td>    <td><div id='tbContent'><b>Content</b> &nbsp;&nbsp;<input type='hidden' name='hdnContentType' id='hdnContentType' value=''/><select id='DrpContentType' name='DrpContentType' style='width: 100px; height: 20px;'><option value='0'>C-CARGO</option><option value='1'>B-BAGGAGE</option><option value='2'>X-EMPTY</option><option value='3'>M-MAIL</option><option value='4'>D-CREW BAGGAGE</option><option value='5'>E-EQUIPMENT IN COMPARTMENT</option></select></div></td>     <td><label id='AddOwnerCode' onclick='AddOwerCode();'>Add Owner Code</label></td><td><input type='button' class='btn btn-block btn-info'  tabindex='8' value='Add ULD' onclick='SavePallet();' /></td></tr></table><hr/></div>");

    $("[id^=AddOwnerCode]").css('text-decoration', 'underline').css('color', 'blue').css('cursor', 'pointer').css('fontSize', 15);;

    cfi.AutoCompleteV2("Destination", "DestinationAirportCode", "Shipment_UCM_Destination", null, "contains");



    cfi.AutoCompleteV2("OwnerCode", "CarrierCode", "Shipment_UCM_Carrier", null, "contains");

    cfi.AutoCompleteV2("ULDType", "ULDName", "Shipment_UCM_ULDType", null, "contains");
    ...
    if (pageType == "EDIT") {
        ...
        // $("#Text_UCMType").data("kendoAutoComplete").enable(false);
    ...
    if (pageType == "READ") {
        $("input:radio[name='Pallet']").parent().hide();
    }
});
...
function AddOwerCode() {

    if ($("#divOwnerCode").length === 0) {
        $("#divUCM").append("<div id='divOwnerCode'></div>")
        $("#divOwnerCode").html('<center><table id="tblOwnerCode" style="width:95%;"><tr><th align="center">Add Owner Code:&nbsp;&nbsp;<input type="hidden" id="SNo" name="SNo" /><input id="txtownercode" class="k-input" maxlength="3" type="text" onkeyup="Validateownercode();" style="width:30%;text-transform:uppercase;"/></td><input type="button" class="btn btn-success" value="Add" onclick="CreateOwnerCode();"</tr></table><center>');
    }
    cfi.PopUp("tblOwnerCode", "", 500, null, null, 180);
}

function Validateownercode() {
    var element = document.getElementById('txtownercode');
    element.value = element.value.replace(/[^a-zA-Z0-9]+/, '');
}

function CreateOwnerCode() {
    if ($('#txtownercode').val() != "") {
        var NewOwnerCode = $("#txtownercode").val();
        AirlineSno = $("#AirlineSNo").val();
        $.ajax({
            url: "Services/Shipment/UCMService.svc/CreateOwnerCode", async: false, type: "POST", dataType: "json", cache: false,
            contentType: "application/json; charset=utf-8",
            data: JSON.stringify({ NewOwnerCode: NewOwnerCode, AirlineSno: AirlineSno }),
            async: false,
            type: 'post',
            cache: false,
            success: function (result) {
                var abc = result[0];
                if (abc == 1002) {
                    ShowMessage('warning', 'Information', "Owner Code already exists");

                }
                else if (abc == 0) {
                    ShowMessage('success', '', "Owner Code Added Successfully", "bottom-right");
                }
                $('#txtownercode').val('');
    ...
    $("#tblOwnerCode").data("kendoWindow").close();
            ...
            success: function (result) {
                ...
                if (dataTableobject.Table0 != undefined) {
                    if (dataTableobject.Table0.length > 0) {
                        ...
                        $("#pallet").remove();
                        var str = " ";
                        str += "<div id='pallet' style='height:100px;width:780px;border:1px solid #ccc;overflow:auto;font:15px/15px  Serif'><table width='100%' cellpadding='0' cellspacing='1' border='0'><tr><td>&nbsp;<b><u>Added ULD No</u></b><br/><br/></td></tr><tr>";
...
function GetRecipientEmail() {
    ...
    else if ($("#UCMType").val() == "UCMIN") { sub = 0; }
    else {
        sub = 1;
    }

    $.ajax({
        url: "Services/Shipment/UCMService.svc/GetRecipientEmail", async: false, type: "POST", dataType: "json", cache: false,
        data: JSON.stringify({ AirlineSNo: $("#AirlineSNo").val(), UCMType: sub }),
        contentType: "application/json; charset=utf-8",
        success: function (result) {
            ...
            if (resData.length > 0) {
                var textval = resData[0].ReceivingID;
                if (textval != "") {
                    var len = textval.split(",").length;
                    for (var j = 0; j <= len - 1; j++) {
                        $("ul#addlist1").append("<li class='k-button' style='margin-right:3px;margin-bottom:3px;'><span>" + textval.split(',')[j] + " </span><span id='" + j + "' class='k-icon k-delete remove'></span></li>");
                    }
                }
            }

            if (resData1.length > 0) {
                var textval1 = resData1[0].ReceivingID;
                if (textval1 != "") {
                    var len = textval1.split(",").length;
                    for (var j = 0; j <= len - 1; j++) {
                        $("ul#addlist").append("<li class='k-button' style='margin-right:3px;margin-bottom:3px;'><span>" + textval1.split(',')[j] + " </span><span id='" + j + "' class='k-icon k-delete remove'></span></li>");
                    }
                }
            }

            if (resData2.length > 0) {
                var textval2 = resData2[0].ReceivingID;
                if (textval2 != "") {

                    var lent = textval2.split(",").length;
                    for (var z = 0; z <= lent - 1; z++) {
                        $("ul#addlist2").append("<li class='k-button' style='margin-right:3px;margin-bottom:3px;'><span>" + textval2.split(',')[z] + " </span><span id='" + z + "' class='k-icon k-delete remove'></span></li>");
...
function InitiateSCM() {
    ...
    for (var i = 0; i < $("ul#addlist2 li span").text().split(' ').length - 1; i++) { M = M + $("ul#addlist2 li span").text().split(' ')[i] + ','; }
    ...
    $("#hdnftp").val(M.substring(0, M.length - 1));
    //if ($("#addlist2 li").length > 0)
    //    $("#txtftp").removeAttr("data-valid");
    ...
    if (($("#addlist1 li").length == 0) && ($("#addlist li").length == 0)) {
        ...
        var email = $("#hdnmail").val();
        if ($("#hdnftp").val() != "") {
            var email = $("#hdnmail").val() + ',' + $("#hdnftp").val();
        }
        else {
            var email = $("#hdnmail").val();
        }
        ...
        $.ajax({
            ...
            data: JSON.stringify({ SitaAddress: $("#hdnadd").val(), EmailAddress: $("#hdnmail").val(), SCMMessage: $("#msgSCM").val(), FlightNo: $("#Text_FlightNo").val(), FlightDate: $("#FlightDate").val(), origincity: origincity }),
            async: false,
            type: 'post',
            cache: false,
            success: function (result) {
...
function SavePallet() {
    ...
    var FlightNo = $("#Text_FlightNo").val();
    var UCMType = $("#UCMType").val();
    var Text_ContentType = $("#DrpContentType").val();
    ...
    if (AirlineSNo != "" && FlightNo != "" && FlightDate != "" && UCMType != "" && ULDType != "" && SerialNo != "" && OwnerCode != "") {
        ...
        if (counter > 0) {
            ...
            $.ajax({
                ...
                data: JSON.stringify({ AirlineSNo: AirlineSNo, FlightDate: FlightDate, FlightNo: FlightNo, UCMType: UCMType, origincity: origincity, ULDType: ULDType, SerialNo: SerialNo, OwnerCode: OwnerCode, Destination: Destination, ContentType: Text_ContentType }),
                contentType: "application/json; charset=utf-8",
                success: function (result) {
                    if (result[0] == 1003) {

                        ShowMessage('info', '', "ULD Already Exists");
                    ...
                    if (result[0] == 1007) {
                        ...
                        ShowMessage('info', '', "ULD Stock does not exist at " + userContext.AirportCode + " station");
                    ...
                    if (result[0] == 0) {
                        ...
                        ShowMessage('success', '', "ULD Added Successfully!");
                        $("#Text_ULDType").val("");
                        $("#SerialNo").val("");
                        $("#OwnerCode").val("");
                        $("#Text_OwnerCode").val("");
...
function GenerateCIMPMessage() {
    ...
    //    sub = 1;
    //}
    var CityCode = getQueryStringValue("CityCode"); //userContext.AirportCode

    $.ajax({
        url: "./Services/Common/CommonService.svc/GenerateCIMPMessage", async: false, type: "POST", dataType: "json", cache: false,
        data: JSON.stringify({ MessageType: "UCM", SerialNo: "", SubType: sub, flightNumber: fltno, flightDate: fltdt, OriginAirport: CityCode, isDoubleSignature: false, version: "", nop: "", grossWeight: "", volumeWeight: "", eventTimeStamp: "", MsgSeqNo: "" }),
        contentType: "application/json; charset=utf-8",
        success: function (result) {
        ...
        if (iKeyCode == 32) {
            ...
            if (ValidateEMail(finalValue)) {
                if ($("ul#addlist1 li").length < 10) {
                    ...
                    ShowMessage('warning', 'Warning - UCM', "Maximum 10 E-mail Addresses allowed.");
                }
                $("#txtEmail").val('');
            }
            //else {
...
function ExtraCondition(textId) {
    ...
    if (textId.indexOf("Text_FlightNo") >= 0) {
        ...
        if ($("#UCMType").val() == 0) { cfi.setFilter(filterEmbargo, "OriginAirportCode", "eq", userContext.AirportCode); }
        else { cfi.setFilter(filterEmbargo, "DestinationAirportCode", "eq", userContext.AirportCode); }
        var filterULD = cfi.autoCompleteFilter(filterEmbargo);
        return filterULD;
    }
    if (textId.indexOf("Text_ULDType") >= 0) {
        cfi.setFilter(filterEmbargo, "ULDName", "neq", "BULK");
        //cfi.setFilter(filterEmbargo, "ULDType", "eq", 1);
        cfi.setFilter(filterEmbargo, "AirlineSNo", "eq", userContext.AirlineSNo);
        var filterULD = cfi.autoCompleteFilter(filterEmbargo);
        return filterULD;
    }
    ...
    if (textId.indexOf("Text_Destination") >= 0) {
        ...
        try {
            // Text_CarrierCode
            cfi.setFilter(filterEmbargo, "FlightNo", "eq", $("#Text_FlightNo").val() == "" ? $("#FlightNo").val() : $("#Text_FlightNo").val());
    ...
    if ($("#UCMType").val() == 1) {
        ...
        if (textId == "tblattachedUCM_ULDName_" + y) {
            cfi.setFilter(filterEmbargo, "CurrentCityCode", "eq", origincity);
    ...
    if ($("#UCMType").val() == 1) {
        ...
        var y = textId.split('_')[2];
        if (textId == "tblattachedUCM_ULDName_" + y) {
            cfi.setFilter(filterEmbargo, "CurrentCityCode", "eq", "");
            var values = ""
    ...
    if ($("#UCMType").val() == 0) {
        var y = textId.split('_')[2];
        if (textId == "tblattachedUCM_BasePallet_" + y) {
            //cfi.setFilter(filterEmbargo, "CurrentCityCode", "eq", origincity);
            //cfi.setFilter(filterEmbargo, "IsAvailable", "eq", 1);

            // Commented for Data in auto complete base pallet new code starts here 

            cfi.setFilter(filterEmbargo, "City", "eq", origincity);
            cfi.setFilter(filterEmbargo, "Bound", "eq", 1);
            // End of new code
            $('input[type="text"][id^="tblattachedUCM_ULDName_"]').each(function () {
                values = values + ',' + $(this).val();
            });
...
function GetUCM() {
    ...
    var FlightNo = $("#Text_FlightNo").val();
    var UCMType = $("#UCMType").val();
    var WhereCondition = UCMType + "NAYAK" + FlightNo + "NAYAK" + FlightDate + "NAYAK" + AirlineSNo
...
function getUCMGrid(WhereCondition) {
    var hdnUCMSNo = $("#hdnUCMSNo").val();
    if (pageType != "NEW") {
        WhereCondition = null;
    }
    if (pageType == "NEW") {
    ...
    if (userContext.SpecialRights.UCM == false) {
        $("#tblUCM").appendGrid({
            ...
            getRecordServiceMethod: (pageType != "NEW") ? "GetUCMSlabRecord" : "GetUCMDefaultOutInRecord",
            ...
            {
                name: 'ContentType', display: 'Content', type: (pageType == "NEW" || pageType == "EDIT") ? "select" : "select", controltype: 'autocomplete', ctrlOptions: { 0: 'C-CARGO', 1: 'B-BAGGAGE', 2: 'X-EMPTY', 3: 'M-MAIL', 4: 'D-CREW BAGGAGE', 5: 'E-EQUIPMENT IN COMPARTMENT' }, ctrlCss: { width: '100px', height: '20px' }, isRequired: false, onChange: function (evt, rowIndex) {
            ...
            { name: "DestinationAirportCode", display: "Destination", type: "text", ctrlAttr: { maxlength: 100, controltype: "autocomplete" }, isRequired: false, AutoCompleteName: "Shipment_UCM_Airport", filterField: "AirportCode", filterCriteria: "contains" },
            ],
            isPaging: false,
            hideButtons: {
                insert: true,
    ...
    if ($("#UCMType").val() == 1) {
        ...
        if (userContext.SpecialRights.UCM == false) {
            $('#tblattachedUCM').appendGrid({
...

                {
                    ...
                    divElements:
                        [
                            {
                                divRowNo: 1,
                                name: 'ULDName', type: (pageType == "NEW" || pageType == "EDIT") ? "text" : "label", ctrlAttr: { controltype: pageType == "NEW" || pageType == "EDIT" ? 'autocomplete' : 'label', onSelect: "return CheckULDName(this);" }, ctrlCss: { width: '100px', height: '20px' }, isRequired: true, AutoCompleteName: table1, filterField: column1, filterCriteria: "contains"
                            },



                            {
                                divRowNo: 1, name: 'EULDType', display: 'ULD Code', type: 'text', ctrlAttr: { controltype: 'autocomplete', onSelect: "return CheckEULDType(this);" }, ctrlCss: { width: '50px', height: '20px' }, AutoCompleteName: 'Shipment_UCM_ULDType', filterField: 'ULDName', templateColumn: ["ULDName"]
                            },
                            {
...

                            {
                                ...
                                divRowNo: 1, name: 'EOwnerCode', display: 'Owner Code', type: 'text', ctrlAttr: { controltype: 'autocomplete', onSelect: "return CheckEOwnerCode(this);" }, ctrlCss: { width: '50px', height: '20px' }, AutoCompleteName: 'Shipment_UCM_Carrier', filterField: 'CarrierCode', templateColumn: ["CarrierCode"]
                ...
                {
                    name: "BUPType", display: "Build Type", type: (pageType == "NEW" || pageType == "EDIT") ? "select" : "select", controltype: 'autocomplete', ctrlOptions: { 0: 'Connected', 1: 'Sandwich', 2: 'Stack' }, ctrlCss: { width: '100px', height: '20px' }, isRequired: (pageType == "NEW" || pageType == "EDIT" ? true : false)

                },
                {


                    name: "BasePallet", display: "Base Pallet", type: (pageType == "NEW" || pageType == "EDIT") ? "text" : "label", ctrlAttr: { controltype: pageType == "NEW" || pageType == "EDIT" ? 'autocomplete' : 'label', onSelect: "return CheckBase(this);" }, ctrlCss: { width: '100px', height: '20px' }, isRequired: (pageType == "NEW" || pageType == "EDIT" ? true : false), AutoCompleteName: 'Shipment_UCM_BasePallet', filterField: 'ULDNo', filterCriteria: "contains", addOnFunction: function (id, text, hdnid, key) {
                ...
                { name: 'ContentType', display: 'Content', type: (pageType == "NEW" || pageType == "EDIT") ? "select" : "select", controltype: 'autocomplete', ctrlOptions: { 0: 'C-CARGO', 1: 'B-BAGGAGE', 2: 'X-EMPTY', 3: 'M-MAIL', 4: 'D-CREW BAGGAGE', 5: 'E-EQUIPMENT IN COMPARTMENT' }, ctrlCss: { width: '100px', height: '20px' }, isRequired: false },
            ...
            $('#tblattachedUCM').appendGrid({
...

                {
                    ...
                    divElements:
                        [
                            {
                                divRowNo: 1,
                                name: 'ULDName', type: (pageType == "NEW" || pageType == "EDIT") ? "text" : "label", ctrlAttr: { controltype: pageType == "NEW" || pageType == "EDIT" ? 'autocomplete' : 'label', onSelect: "return CheckULDName(this);" }, ctrlCss: { width: '100px', height: '20px' }, isRequired: true, AutoCompleteName: 'Shipment_UCM_ULDNo', filterField: column1, filterCriteria: "contains"
                            },
                ...
                afterRowAppended: function (caller, parentRowIndex, addedRowIndex) {
                    $("a:contains('Add Owner Code')").hide();
                    var i = $("tr[id^='tblattachedUCM_Row']:last").attr('id').split("_")[2];
                    $('input[id^="tblattachedUCM_HdnEOwnerCode"]').last().after('&nbsp;&nbsp;&nbsp;&nbsp;<a href="#" onclick="AddOwerCode();" style="font-size:10px; color:blue">Add Owner Code</a>');
                    // $("#tblattachedUCM_ContentType_" + i).attr("disabled", true);
                    //  CheckNEWULD(this);
                },
                isPaging: false,
        ...
        if (userContext.SpecialRights.UCM == false) {
            $('#tblattachedUCM').appendGrid({
                ...
                { name: 'ULDName', display: 'ULD Name', type: (pageType == "NEW" || pageType == "EDIT") ? "text" : "label", ctrlAttr: { controltype: pageType == "NEW" || pageType == "EDIT" ? 'autocomplete' : 'label' }, ctrlCss: { width: '100px', height: '20px' }, isRequired: (pageType == "NEW" || pageType == "EDIT" ? true : false), AutoCompleteName: table1, filterField: column1, filterCriteria: "contains" },
                ...
                },
                {
                    ...
                    name: "BasePallet", display: "Base Pallet", type: (pageType == "NEW" || pageType == "EDIT") ? "text" : "label", ctrlAttr: { controltype: pageType == "NEW" || pageType == "EDIT" ? 'autocomplete' : 'label' }, ctrlCss: { width: '100px', height: '20px' }, isRequired: (pageType == "NEW" || pageType == "EDIT" ? true : false), AutoCompleteName: 'ULDUCM_BasePallet_ULDNo', filterField: 'ULDNo', filterCriteria: "contains", addOnFunction: function (id, text, hdnid, key) {
                ...
                { name: 'ContentType', display: 'Content', type: (pageType == "NEW" || pageType == "EDIT") ? "select" : "select", controltype: 'autocomplete', ctrlOptions: { 0: 'CARGO', 1: 'BAGGAGE', 2: 'EMPTY' }, ctrlCss: { width: '100px', height: '20px' }, isRequired: false },
                    //{ name: 'ContentType', display: 'Content', type: (pageType == "NEW" || pageType == "EDIT") ? "select" : "select", controltype: 'autocomplete', ctrlOptions: { 0: 'CARGO', 1: 'BAGGAGE', 2: 'EMPTY' }, ctrlCss: { width: '100px', height: '20px', disable: 'true' }, isRequired: false },
            ...
            $('#tblattachedUCM').appendGrid({
                ...
                },
                {
                    ...
                    name: "BasePallet", display: "Base Pallet", type: (pageType == "NEW" || pageType == "EDIT") ? "text" : "label", ctrlAttr: { controltype: pageType == "NEW" || pageType == "EDIT" ? 'autocomplete' : 'label' }, ctrlCss: { width: '100px', height: '20px' }, isRequired: (pageType == "NEW" || pageType == "EDIT" ? true : false), AutoCompleteName: 'Shipment_UCM_BasePallet', filterField: 'ULDNo', filterCriteria: "contains", addOnFunction: function (id, text, hdnid, key) {
