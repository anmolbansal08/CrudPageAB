...
$(function () {
    /*Added By Brajendra on 22 July, 2017*/
    //$("input[type='radio'][name='Type']").closest('td').hide();
    //$("[id$='spnType']").closest('td').hide();

    //Added By Shivam // Date:24-06-2020  // TFS ID:16310

    document.getElementById("spnSearch").innerHTML ='<font color="red">*</font><span id="spnSearch"> Enter ULD No</span>'
    ...
    cfi.AutoCompleteV2("MainULDNo", "DailyFlightSNo,ULDNo", "ULDBreakdown_ULDNo", clearField, "contains");
    $('input:radio').change(function () {
        ...
        if ($('input:radio[name=Type]:checked').val() == 0) {
            ...
            var data = GetDataSourceV2("MainULDNo", "ULDBreakDown_MainULDNoGetDataSource");
            ...
            var data = GetDataSourceV2("MainULDNo", "ULDBreakDown_MainULDNoTransitGetDataSource");
            cfi.ChangeAutoCompleteDataSource("MainULDNo", data, true, null, "ULDNo", "contains");
        }
    ...
    $("#btnSearch").click(function () {
        if ($("#MainULDNo").val() == "") {
            ShowMessage('warning', 'Warning - ULD Breakdown Search', "ULD No can not be blank.", "bottom-right");
            return;
        }
        else {
            ...
            if ($("#hdnIsCart").val() == 1)
            { var alphabettypesExport = [ { Key: "2", Text: "ULD Breakdown (Complete)" }]; }
            else { var alphabettypesExport = [{ Key: "1", Text: "ULD Transfer" }, { Key: "2", Text: "ULD Breakdown (Complete)" }, { Key: "3", Text: "ULD Breakdown (Remove AWB)" }]; }
...
function CreateULDBreakdownTable() {
    $.ajax({
        ...
        type: "GET",
        dataType: "json",
        data: { DailyFlightSNo: $("#MainULDNo").val(), ULDNo: $("#Text_MainULDNo").val(), Type: 0 },/*$('input:radio[name=Type]:checked').val()*/
        ...
        success: function (result) {
            ...
            if (result.substring(1, 0) == "{") {
                ...
                if (myData.Table0.length > 0) {
                    ...
                    $("#hdnIsCart").val(myData.Table0[0].IsCart);
                    table += "<td class='ui-widget-content first'>" + myData.Table0[0].ULDType + "</td><td class='ui-widget-content first'>" + myData.Table0[0].ULDNumber + "</td><td class='ui-widget-content first'>" + myData.Table0[0].Location + "</td><td class='ui-widget-content first'>" + myData.Table0[0].ULDBuildWeight + "</td><td class='ui-widget-content first'>" + myData.Table0[0].TareWeight + "</td><td class='ui-widget-content first'><input name='Action' id='Action' type='hidden' value=''/><input type='text' controltype='autocomplete' id='Text_Action' name='Text_Action'/></td><td class='ui-widget-content first'><button aria-disabled='false' role='button' title='Submit' type='button' id='btnSubmit' value='1' tabindex='16' class='btn btn-success' style='width:100px;' onclick='Submit(0);'><span class='ui-button-text'>Submit</span></button></td>";
...
function Submit(type) {
    ...
    if ($("#Action").val() == "") {
        if (type == 0) {
            ShowMessage('warning', 'Warning - ULD Breakdown', "Action can not be blank.", "bottom-right");
            return;
        }
    }
        ...
        var theDiv = document.getElementById("divULDBreakdownAction");
        theDiv.innerHTML = "";
        var table = "</br></br><table class='appendGrid ui-widget' id='tblULDBreakdownAction'><thead class='ui-widget-header' style='text-align:center'><tr><td><font color='red'>*</font>ULD No&nbsp;&nbsp;&nbsp;<input name='ULDNo' id='ULDNo' type='hidden' value=''/><input type='text' controltype='autocomplete' id='Text_ULDNo' name='Text_ULDNo'/></td><td class='ui-widget-header'><font color='red'>*</font>Requested By&nbsp;&nbsp;&nbsp;<input name='RequestedBy' id='RequestedBy' type='hidden' value=''/><input type='text' controltype='autocomplete' id='Text_RequestedBy' name='Text_RequestedBy'/></td><td class='ui-widget-header'>Billed To&nbsp;&nbsp;&nbsp;<input name='BilledTo' id='BilledTo' type='hidden' value=''/><input type='text' controltype='autocomplete' id='Text_BilledTo' name='Text_BilledTo'/></td><td class='ui-widget-header'><font color='red'>*</font>Reason&nbsp;&nbsp;&nbsp;<input name='Reason' id='Reason' type='hidden' value=''/><input type='text' controltype='autocomplete' id='Text_Reason' name='Text_Reason'/></td><td class='ui-widget-header'>Remarks&nbsp;&nbsp;&nbsp;<input name='Remark' id='Remark' type='text' value=''/></td><td class='ui-widget-header'><button aria-disabled='false' role='button' title='Transfer' type='button' id='btnTransfer' value='1' tabindex='16' class='btn btn-success' style='width:100px;' onclick='Transfer();'><span class='ui-button-text'>Transfer</span></button></td></tr></thead></table>";
        theDiv.innerHTML += table;
        cfi.AutoCompleteV2("ULDNo", "ULDNo", "UldBreakDownTransferULD", null, "contains");
        cfi.AutoCompleteV2("RequestedBy", "SNo,Name", "ULDBreakdown_Name", null, "contains");
        cfi.AutoCompleteV2("BilledTo", "SNo,Name", "ULDBreakdown_Name", null, "contains");
        cfi.AutoCompleteV2("Reason", "SNo,Remarks", "ULDBreakdown_Remarks", null, "contains");
        ULDBreakdownGrid();
    }
    ...
    //                theDiv.innerHTML = "";
    //                var table = "</br></br><table class='appendGrid ui-widget' id='tblULDBreakdown'><thead class='ui-widget-header' style='text-align:center'><tr><td class='ui-widget-header'>AWB No</td><td class='ui-widget-header'>Total Pieces / Planned Pieces</td><td class='ui-widget-header'>Total Gross Wt. / Planned Gross Wt.</td><td class='ui-widget-header'>Total Volume Wt. / Planned Volume Wt.</td><td class='ui-widget-header'>Orgin</td><td class='ui-widget-header'>Dest</td><td class='ui-widget-header'></td></tr></thead><tbody class='ui-widget-content'>";
    //                if (result.substring(1, 0) == "{") {
    //                    var myData = jQuery.parseJSON(result);
    //                    if (myData.Table0.length > 0) {
...
function ULDBreakdownTransitGrid() {
    ...
    $('#tbl' + dbtableName).appendGrid({
        ...
        columns: Action == '1' ? [
              ...
              { name: 'TotalandPlannedPcs', display: 'Total Pieces / Planned Pieces', type: 'label' },
              { name: 'TotalandPlannedGrwt', display: 'Total Gross Wt. / Planned Gross Wt.', type: 'label' },
              { name: 'TotalandPlannedVolwt', display: 'Total Volume Wt. / Planned Volume Wt.', type: 'label' },
              ...
              { name: 'Origin', display: 'Orgin', type: 'label' },
              { name: 'Destination', display: 'Dest', type: 'label' },
              { name: 'WHLocation', display: 'W/H Location', type: 'text', ctrlAttr: { maxlength: 100, controltype: 'autocomplete' }, ctrlCss: { width: '80px', height: '20px' }, isRequired: true, AutoCompleteName: 'ULDBreakDown_WHLocation', filterField: 'LocationName' },
              { name: 'RequestedBy', display: 'Requested By', type: 'text', ctrlAttr: { maxlength: 100, controltype: 'autocomplete' }, ctrlCss: { width: '80px', height: '20px' }, isRequired: true, AutoCompleteName: 'ULDBreakdown_Name', filterField: 'Name' },
              { name: 'BilledTo', display: 'Billed To', type: 'text', ctrlAttr: { maxlength: 100, controltype: 'autocomplete' }, ctrlCss: { width: '80px', height: '20px' }, AutoCompleteName: 'ULDBreakdown_Name', filterField: 'Name' },
              { name: 'Remark', display: 'Remarks', type: 'text', ctrlAttr: { controltype: 'alphanumericupper', allowchar: '.,-', maxlength: 100 }, ctrlCss: { width: '150px' }, isRequired: true },
...
function RemoveUlDAWB(rowID) {
    if (validateTableDataForUldBreakDown('tblULDBreakdownGrid', rowID)) {
        Save(rowID);
    }
...
function Transfer() {
    ...
    if ($("#ULDNo").val() == "") {
        ShowMessage('warning', 'Warning - ULD Breakdown', "ULD No can not be blank.", "bottom-right");
    }
    else if ($("#RequestedBy").val() == "") {
        ShowMessage('warning', 'Warning - ULD Breakdown', "Requested By can not be blank.", "bottom-right");
    }
        //else if ($("#BilledTo").val() == "") {
        //    ShowMessage('warning', 'Warning - ULD Breakdown', "Billed To can not be blank.", "bottom-right");
        //}
    else if ($("#Reason").val() == "") {
        ShowMessage('warning', 'Warning - ULD Breakdown', "Reasons can not be blank.", "bottom-right");
    }

    else if ($("[id*='HdnWHLocation']").val() == "") {
        ShowMessage('warning', 'Warning - ULD Breakdown', "W/H Location can not be blank.", "bottom-right");
    }
    else {
        Save(0);
    }
...
function CompleteULDBreakdown() {
    ...
    $(table).find('tbody tr').each(function (index) {
        if ($("[id^='tblULDBreakdownGrid_WHLocation_" + (index + 1) + "']").val() == "") {
            isValidate = false
            return false;
        }
    });

    if (!isValidate && $("#hdnIsCart").val() == 0) ShowMessage('warning', 'Warning - ULD Breakdown', "W/H Location can not be blank.", "bottom-right");
    else if ($("#ULDLocation").val() == "" &&  $("#hdnIsCart").val().toString()=="0") {
        ShowMessage('warning', 'Warning - ULD Breakdown', "ULD Location can not be blank.", "bottom-right");
    }
    else if ($("#RequestedBy").val() == "") {
        ShowMessage('warning', 'Warning - ULD Breakdown', "Requested By can not be blank.", "bottom-right");
    }
...
function Save(row) {
    ...
    //below commented by brajendra

    //var rows = $("tr[id^='tblULDBreakdownGrid']").map(function () { return $(this).attr("id").split('_')[2]; }).get();
    //if (!validateTableData("tblULDBreakdownGrid", rows)) {
    //    return false;
    //}
                ...
                if (result.substring(1, 0) == "{") {
                    ...
                    if (myData.Table0[0].Column1 == "2000") {
                        if ($("#Action").val() == "1" || $("#Action").val() == "2") {
                            ...
                            $("#tblULDBreakdownGrid tbody tr").eq(Number(row) - 1).remove();
                        }
                        ShowMessage('success', 'Success - ULD Breakdown', "ULD Breakdown Processed  Successfully.", "bottom-right");
                        Submit(1);
                        return false;
                    }
                    else {
...
function ADDTransitReBuildData() {
    ...
    if (ULDBreakdownArray.length == 0) {
        ShowMessage('warning', 'Warning - ULD Build details', "Please check atleast one AWB No.", "bottom-right");
        return;
    }
    else {
        $.ajax({
...
function ShowULDDetails() {
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            $('#AddScaleWeight').blur(function () {
                ...
                if (ScaleWeight != "" && parseFloat(ScaleWeight) > 0) {
                    if (parseFloat(TotalULDWeight) > parseFloat(ScaleWeight)) {
                        $('#AddScaleWeight').val('');
                        ShowMessage('warning', 'Information', "Scale weight can not be less than Gross Weight ( Net Weight + Tare Weight)");
                    }
                }
            });
            ...
            cfi.AutoCompleteV2("ULDBuildUpLocation", "LocationName", "ULDBreakdown_BULocationName", null, "contains");
            cfi.AutoCompleteV2("ULDLocation", "LocationName", "ULDBreakdown_BULocationName", null, "contains");
            cfi.AutoCompleteV2("LoadCode", "ULDLoadingCode,Description", "ULDBreakdown_ULDLoadingCode", null, "contains");
            cfi.AutoCompleteV2("LoadIndicator", "ULDLoadingIndicator,Description", "ULDBreakdown_ULDLoadingIndicator", null, "contains");
            cfi.AutoCompleteV2("AbbrCode", "AbbrCode,Description", "ULDBreakdown_AbbrCode", null, "contains");
            ...
            cfi.AutoCompleteV2("_OtherPallets", "ULDNo", "ULDBreakdown_UBULDNo", null, "contains", ",");
            cfi.AutoCompleteV2("UldBupType", "Description", "ULDBreakdown_Description", null, "contains");
                ...
                success: function (result) {
                    ...
                    $("div[id$='divareaTrans_ulddetails_uldconsumables']").find("[id='areaTrans_ulddetails_uldconsumables']").each(function () {
                        $(this).find("input[id^='ConsumablesSNo']").each(function () {
                            cfi.AutoCompleteV2($(this).attr("name"), "Item", "ULDBreakdown_Item", null, "contains");
                        });
...
function ReBindConsumablesAutoComplete(elem, mainElem) {
    $(elem).closest("div[id$='areaTrans_ulddetails_uldconsumables']").find("[id^='areaTrans_ulddetails_uldconsumables']").each(function () {
        $(this).find("input[id^='ConsumablesSNo']").each(function () {
            var newDataSource = GetDataSourceV2("Text_" + $(this).attr("id"), "ULDBreakDown_ConsumablesGetDataSource");
            //Commented Old Autocomplete GetDataSource("Text_" + $(this).attr("id"), "Consumables", "SNo", "Item", ["Item"]);
...
function validateTableDataForUldBreakDown(id, uRows) {
    var upRows;
    if (!$.isArray(uRows)) {
        upRows = new Array();
        upRows[0] = uRows;
    ...
    //Updated By Amit Yadav tblULDBreakdownGrid_HdnWHLocation_1
...
function ExtraCondition(textId) {
    ...
    if (textId.indexOf("Text_MainULDNo") >= 0) {
        var filterULDNo = cfi.getFilter("AND");
        cfi.setFilter(filterULDNo, "CurrentAirportSNo", "eq", userContext.AirportSNo);
        filterFlight = cfi.autoCompleteFilter(filterULDNo);
        return filterFlight;
    }
    if (textId.indexOf("Text_ULDNo") >= 0) {
        var filterULDNo = cfi.getFilter("AND");
        cfi.setFilter(filterULDNo, "ULDNo", "notin", $("#Text_MainULDNo").val().split('-')[0]);
        cfi.setFilter(filterULDNo, "CurrentAirportSNo", "eq", userContext.AirportSNo);

        filterFlight = cfi.autoCompleteFilter(filterULDNo);
        return filterFlight;
    }
    if (textId.indexOf("tblULDBreakdownGrid_WHLocation")>=0){
        var filterULDNo = cfi.getFilter("AND");
        cfi.setFilter(filterULDNo, "AirportSNo", "eq", userContext.AirportSNo);
        filterFlight = cfi.autoCompleteFilter(filterULDNo);
        return filterFlight;
    }
