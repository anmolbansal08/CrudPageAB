$(document).ready(function () {
    ...
    // append the html to awb tracking
    $.ajax({
        ...
        success: function (result) {
            ...
            cfi.AutoCompleteV2("AirlineCode", "AirlineCode", "AuditLog_Tracking_Airline", null, "contains");
...
$(document).ready(function () {
    ...
    $("#Emailoption").hide();
    if (_awbNo) {
        $("#AWBNo").val(_awbNo.split('-')[1]);
        $("#Text_AirlineCode").val(_awbNo.split('-')[0]);
        Search();
    }
...
$(document).on('click', '#SearchAWB', function () {
    if ($("#AWBNo").val() == "") {
        ...
        $("#AWBDetails").hide();
        ShowMessage('warning', 'Need Your Kind Attention', "AWB No / Reference Number / PO Mail No Is Required");
    }
...
function Search() {
    ...
    var str1 = "";
    var str2 = "";
    if (userContext != undefined && userContext.GroupName.toUpperCase() == "AGENT") {
            ...
            if (myData.Table0.length > 0) {
                ...
                for (var i = 0; i < $.trim($("#AWBNo").val()).split(',').length; i++) {
                    ...
                    for (var j = 0; j < myData.Table0.length; j++) {
                        ...
                        if (myData.Table0[j].AWBNo == $("#Text_AirlineCode").val() + '-' + $.trim($("#AWBNo").val()).split(',')[i] || myData.Table0[j].AWBNo == $.trim($("#AWBNo").val()).split(',')[i]) {
                            if (countmain == 0) {
                                ...
                                for (var m = 0; m < myData.Table1.length; m++) {
                                    if (myData.Table1[m].AWBNo == myData.Table0[j].AWBNo || myData.Table1[m].ReferenceNumber == myData.Table0[j].AWBNo) {
                                        ...
                                        station = myData.Table1[m].Station;
                                ...
                                if (myData.Table3.length > 0) {
                                    for (var l = 0; l < myData.Table3.length; l++) {
                                        str += '<table cellpadding="12"><tr><td><span style="color:red;"><strong>LYING AT: <span style="color:blue;">' + myData.Table3[l].LyingDetail + ' </span></strong></span></td></tr></table>';
                                    }
                                }
                                    // summary
                    ...
                    //str += '</div></li>';
                    countmain = 0;
                    countforbkd = 0; countforexe = 0; countforrcs = 0; //countforcan = 0; countforapr = 0;
                    // create table for milestone plan
                    var bkdexercs = 4;
                    for (var l = 0; l < myData.Table2.length; l++) {
                        bkdexercs = 4;

                        if (myData.Table2[l].AWBNo == $.trim($("#AWBNo").val()).split(',')[i] || myData.Table2[l].ReferenceNumber == $.trim($("#AWBNo").val()).split(',')[i]) {
                            if (countmain == 0) {
                            ...
                            //if (countforcan == 0 && myData.Table2[l].Action.toUpperCase() == 'CAN') {
                            //    action = myData.Table2[l].Action.toUpperCase();
                            //    station = myData.Table2[l].Station;
                            //    countforcan = 1;
                            //    bkdexercs = 1;
                            //}
                            //if (countforapr == 0 && myData.Table2[l].Action.toUpperCase() == 'APR') {
                            //    action = myData.Table2[l].Action.toUpperCase();
                            //    station = myData.Table2[l].Station;
                            //    countforapr = 1;
                            //    bkdexercs = 1;
                    ...
                    for (var k = 0; k < myData.Table2.length; k++) {
                        if (myData.Table2[k].AWBNo == $.trim($("#AWBNo").val()).split(',')[i] || myData.Table2[k].ReferenceNumber == $.trim($("#AWBNo").val()).split(',')[i]) {
...
function SaveEmailId() {
    ...
    for (var l = 0; l < myData.Table2.length; l++) {
        if (countmain != 0) {
            MailBody3 += '<td style="text-align:center;padding:2px;"><img  src=' + Imgurl + '/images/arrowtrack.png alt="confirm order" width="30%"></td>'
        }
        countmain = 1;
        if (countnexttd % 9 == 0 && countnexttd > 0) {
            MailBody3 += '</tr><tr>'
        }
        MailBody3 = MailBody3 + '<td style="text-align:center;padding:2px;"><p ><b>' + myData.Table2[l].Station + '</b></p><img  src=' + Imgurl + '/images/' + myData.Table2[l].Action.toUpperCase() + '.png + alt="confirm order" width="30%"><p ><b>' + myData.Table2[l].Action + '</b></p>'
        MailBody3 = MailBody3 + '<p><b>' + myData.Table2[l].createdon + '</b></p><p><b>' + myData.Table2[l].Pieces + ' pcs' + '</b></p></td>'
    ...
    var obj = {
        ...
        BodyFormat: 'HTML',
        GenerateBy: userContext.UserSNo,
        GenerateAt: '',
        IsSend: 0
    }
    $.ajax({
