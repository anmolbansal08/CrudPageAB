...
$(document).ready(function () {
	...
	//$("#Text_Airport").data("kendoComboBox").setDefaultValue(userContext.AirportSNo, userContext.AirportCode + '-' + userContext.AirportName);
    ...
    $("#grid").kendoGrid({
        ...
        dataSource: new kendo.data.DataSource({
            ...
            transport: {
                read: {
                    url: "../StationWiseSummary/StationWiseSummaryGetRecord",
                    dataType: "json",
                    global: true,
                    type: 'POST',
                    method: 'POST',
        ...
        columns: [
            ...
            { field: "Station", title: "Station", filterable: true },
...
function SearchStationWiseSummary() {
    Model =
        {
            AirlineCode: $('#AirlineCode').val(),
            FromDate: $("#FromDate").val(),
            ToDate: $("#ToDate").val(),
			BookingFilter: $('input[name=Filter]:checked').val(),
			Airport: $('#Airport').val() || 0
        };

    if (Date.parse($(Model.FromDate).val()) > Date.parse($(Model.ToDate).val())) {
        ShowMessage('warning', 'warning - Station Wise Summary', "From Date can not be greater than To Date !");
        return false;;
    }
    ...
    if (Model.AirlineCode != "" && Model.ToDate != "" && Model.FromDate != "") {
		...
		$('#df #BookingFilter').val(Model.BookingFilter);
...
//function ExportToExcel_StationWiseSummary() {
...
//    window.location.href = "../StationWiseSummaryController/ExportToExcel?Model=" + Model 
