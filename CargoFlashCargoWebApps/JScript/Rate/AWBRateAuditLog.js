...
function SearchAWBRateAuditLog(Mode) {
    ...
    $('ul.tabrow li').removeClass("selected")   // remove class from each li   // by arman ali 2017-26-05
    $('ul.tabrow li:eq(0)').addClass("selected");  // add class for first li
    ...
    if (!$("#SearchAWBRateAuditLogSearchForm").data('cfValidator').validate()) {
        ...
        if (SearchBy == "1") {
            $("#Text_ReferenceNumber").attr("data-valid-msg", "Select the AWB Number");

            theMessage = "Required Message 1";
        }
        else {
            $("#Text_ReferenceNumber").attr("data-valid-msg", "Select the Reference Number");
...
$(document).ready(function () {
    ...
    cfi.AutoCompleteV2("ReferenceNumber", "ReferenceNumber", "AWB_Rate_Audit_Log_ReferenceNumber", null, "contains");
    //cfi.AutoComplete("AirlineCode", "AirlineCode", "airline", "sno", "AirlineCode", ["AirlineCode"], null, "contains");
    //cfi.AutoCompleteV2("AirlineCode", "AirlineCode", "AWB_Rate_Audit_Log_AirlineCode", null, "contains");
    //cfi.AutoCompleteV2("AirlineCode", "AirlineCode", "AWB_Rate_Audit_Log_AirlineCode", null, "contains");
    cfi.AutoCompleteV2("AirlineCode", "CarrierCode", "AWB_Rate_Audit_Log_AirlineCode", null, "contains");
    //Ends
    document.getElementById("AWBRateAuditLogSerachGridDivInfo").style.display = "block";
    event.currentTarget.className += " active";
...
function OpenTab(evt, TabName, Mode) {
    ...
    $('ul.tabrow li').removeClass("selected")   // remove class from each li   // by arman ali 2017-26-05
    $('ul.tabrow li:eq(' + Mode + ")").addClass("selected");
...
$('input[type="radio"][name="SearchBy"]').click(function () {
    if (this.value == 1) {
       ...
       cfi.ResetAutoComplete("ReferenceNumber");
        ...
        cfi.AutoCompleteV2("ReferenceNumber", "AWBNo", "AWB_Rate_Audit_Log_AWBNo", null, "contains");
        $('#Text_AirlineCode_input').val('');
        //ends
        GridBlank();
...
function ExtraCondition(textId) {
    ...
    else if (textId.indexOf("Text_ReferenceNumber") >= 0) {
       
        var filter1 = cfi.getFilter("AND");
        cfi.setFilter(filter1, "AirlineSNo", "eq", $("#AirlineCode").val()||0);
        FilterAirlineCode = cfi.autoCompleteFilter(filter1);
        return FilterAirlineCode;
    }
