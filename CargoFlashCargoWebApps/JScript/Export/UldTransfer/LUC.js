...
$(function () {
    $.ajax({
        ...
        success: function (result) {
            ...
            IssuedTo = getQueryStringValue("IssuedTo")

            if (IssuedTo == "2") {
                $('#Text_UHFNumber').val("111111");
                $('#Text_UHFCarrierCode').val("111");
            } else {

                $('#Text_UCRNumber').val("111111");
                $('#Text_UCRCarrierCode').val("111");
            }
            ...
            if (URL == "ULDOut") {
                ...
                $("#Text_ULD").data("kendoAutoComplete").enable(false);
                $("#divMultiULD span.k-delete").remove();
            }
            PageRightsCheckLuc()
        }
    });
    ...
    $.ajax({
        ...
        success: function (response) {
            if (response == "2001") {
                ...
                //Started Code By Shivali Thakur for Audit Log//
                var item = sessionStorage.getItem("uldnumber");
                KeyValue = item;


                if ($("#caspucruhf").text().indexOf("UCR") >= 0) {
                    var ucrno = $("#Text_UCRCarrierCode").val() + "-" + $("#Text_UCRNumber").val();
                    var n = { ProcessName: getQueryStringValue("Apps").toUpperCase(), SubprocessName: "LUC", ColumnName: 'UCR NUMBER', OldValue: '', NewValue: ucrno };
                    arrVal.push(n);
                }
                else if ($("#caspucruhf").text().indexOf("UHF") >= 0) {
                    var a = { ProcessName: getQueryStringValue("Apps").toUpperCase(), SubprocessName: "LUC", ColumnName: 'UHF NUMBER', OldValue: '', NewValue: ($("#Text_UHFCarrierCode").val() + '-' + $("#Text_UHFNumber").val()) };
                    arrVal.push(a);
                }

                var b = { ProcessName: getQueryStringValue("Apps").toUpperCase(), SubprocessName: "LUC", ColumnName: 'Date', OldValue: '', NewValue: $("#Text_IssuedDate").val() };
                arrVal.push(b);
                var c = { ProcessName: getQueryStringValue("Apps").toUpperCase(), SubprocessName: "LUC", ColumnName: 'Time', OldValue: '', NewValue: $("#Text_IssuedTime").val() };
                arrVal.push(c);
                var d = { ProcessName: getQueryStringValue("Apps").toUpperCase(), SubprocessName: "LUC", ColumnName: 'Loaded By', OldValue: '', NewValue: $("#Text_LoadedBy").val() };
                arrVal.push(d);
                var e = { ProcessName: getQueryStringValue("Apps").toUpperCase(), SubprocessName: "LUC", ColumnName: 'Representative Name', OldValue: '', NewValue: $("#Text_Name").val() };
                arrVal.push(e);
                var f = { ProcessName: getQueryStringValue("Apps").toUpperCase(), SubprocessName: "LUC", ColumnName: ' Representative ID Number', OldValue: '', NewValue: $("#Text_IDNumber").val() };
                arrVal.push(f);
                var g = { ProcessName: getQueryStringValue("Apps").toUpperCase(), SubprocessName: "LUC", ColumnName: 'Representative Mobile No', OldValue: '', NewValue: $("#Text_MobileNo").val() };
                arrVal.push(g);
                if ($("#Text_SI1").val() != '') {

                    var h = { ProcessName: getQueryStringValue("Apps").toUpperCase(), SubprocessName: "LUC", ColumnName: 'Supplementary Info 1', OldValue: '', NewValue: $("#Text_SI1").val() };
                    arrVal.push(h);
                }
                if ($("#Text_SI2").val() != '') {
                    var k = { ProcessName: getQueryStringValue("Apps").toUpperCase(), SubprocessName: "LUC", ColumnName: 'Supplementary Info 2', OldValue: '', NewValue: $("#Text_SI2").val() };
                    arrVal.push(k);
                }
                SaveAppendGridAuditLog("ULD Number", getQueryStringValue("ULDNumber"), "0", JSON.stringify(arrVal), "Edit", userContext.TerminalSNo, userContext.NewTerminalName);
                //Ended Code By Shivali Thakur for Audit Log//
                ShowMessage("success", "", "Saved successfully...");
                window.location.href = "//" + location.host + "/" + window.location.pathname + "?Module=Shipment&Apps=LUC&FormAction=INDEXVIEW";
            }
            else if (response == "3001") {
                ShowMessage('info', 'Need your Kind Attention!', "UHF Number already exist.");
                $("#Text_UHFNumber").val("")
                $("#ManualChek").attr("checked", true)
                // window.location.href = "//" + location.host + "/" + window.location.pathname + "?Module=Shipment&Apps=LUC&FormAction=INDEXVIEW";
                return false;
            }
                ...
                return false;
            } else if (response == "1991") {
                ShowMessage('warning', 'Warning - LUC', "Record already exist!");
                return false;
            }
        }
    });
...
function NewClick(EditSNo) {
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            $("#divLUCInformation").css("height", "400px")
            $("#Issuto").html("")
            if (IssuedTo == "2") {
                $("#Issuto").html("AIRLINE")
                $("#caspucruhf").html("UCR NUMBER")
                $("#DivUCR").css("display", "block")
                $("#Text_UHFNumber").removeAttr("required")
                $("#Text_UHFCarrierCode").removeAttr("required")


            } else {
                $("#Text_UCRNumber").removeAttr("required")
                $("#Text_UCRCarrierCode").removeAttr("required")
                $("#DivUHF").css("display", "block")
                $("#Issuto").html("FORWADER/SHIPPER")
                $("#caspucruhf").html("UHF NUMBER")
...
function OnSuccessGrid() {
    ...
    $("div[id$='divMainGrid']").find("div[class^='k-grid-content'] tbody tr").each(function (row, tr) {
        if ($(tr).find("td:eq(" + SentLUCIndex + ")").text() == "YES") {
            ...
            if (YesReady) {
                $(tr).find('input[type="button"][value="S"]').prop('disabled', true);
            } else {
...
function BindGridData(SNo, obj) {
    if ($(obj).attr("value") == "E") {
        ...
        var IssuedTo = trHRow.find("th[data-field='IssuedTo']").index();
        var IsIssuedTo = $(obj).closest('tr').find('td:eq(' + IssuedTo + ')').text();
        if (IsIssuedTo == "2") {
            BindSendLUC(SNo, 'Send');
        } else {
            ShowMessage('warning', 'Warning - LUC', "LUC can't be sent to Forwarder/Shipper!", "bottom-right");
            return
        }
...
function ExtraCondition(textId) {
    ...
    if (textId.indexOf("Text_ULD") >= 0) {
        cfi.setFilter(f, "ULDNumber", "notin", $("#Text_ULD").data("kendoAutoComplete").key());
        f = cfi.autoCompleteFilter(f);
        return f;
    }
...
var footer = '<div id="divFooter" class="divFooter" style="height: 0px; padding-bottom: 30px; display: block;"><div><table style="margin-left:20px;"><tbody><tr><td> &nbsp; &nbsp;</td><td></td><td> &nbsp; &nbsp;</td><td><input type="button" style="float:right;display:none;" onclick="SaveDetails(0)" id="btnSave"  value="Save" class="btn btn-success"></td><td> &nbsp; &nbsp;</td><td><button class="btn btn-block btn-danger btn-sm" id="btnCancel" onclick="CancelDetailsWithHREF()">Cancel</button></td></tr></tbody></table> </div></div>';
//ESSCHARGES
var flags = 0;
var weight = 0;
...
function Get_LUCAutoGenerateUcrUHf() {
    ...
    $.ajax({
        ...
        async: false, type: "GET"
        , dataType: "json",
        data: { ChklstUldStack: ChklstUldStack, AirlineSNo: userContext.AirlineSNo, IssuedTo: IssuedTo }
        , contentType: "application/json;charset=utf-8", cache: false, success: function (response) {
            if (response.length > 0) {
                ...
                if (IssuedTo == "2") {
                    ...
                    $('#Text_UHFCarrierCode').val("111");
                    setTimeout(function () {
                        CheckUCRandUHFNumber(v.Table0[0].UCRReceiptNo, 2)
                    }, 100)
                } else {
                    ...
                    $('#Text_UCRCarrierCode').val("111");
                    setTimeout(function () {
                        CheckUCRandUHFNumber(v.Table0[0].UHFReceiptNo, 3)
                    }, 100)
...
function DupicateCheckUHFumber(thisv) {
    DupicateCheckUCRUHFNumber($(thisv).val(), 2)
}
function DupicateCheckUCRUHFNumber(UCRNumber, Type) {
    $.ajax({
        url: "Services/Export/UldTransfer/LUCService.svc/DupicateCheckUCRUHFNumber",
        async: false, type: "GET"
        , dataType: "json",
        ...
        , contentType: "application/json;charset=utf-8", cache: false, success: function (result) {
            ...
            if (Data[0].CheckNumber != "") {
                debugger;
                ShowMessage('info', 'Need your Kind Attention!', "Already Exits")
                if (Type == 1) {
                    $("#Text_UCRNumber").val("")
                } else if (Type == 2) {
                    $("#Text_UHFNumber").val("")
...
function CheckUCRandUHFNumber(UCRANDUHFNO, ISSUETYPE) {
    var id = 0;
    $.ajax({
        url: "Services/Export/UldTransfer/LUCService.svc/CheckUCRandUHFNumberDuplicate",
        async: false, type: "GET"
        , dataType: "json",
        data: { UCRANDUHFNO: UCRANDUHFNO, ISSUETYPE: ISSUETYPE }
        , contentType: "application/json;charset=utf-8", cache: false, success: function (response) {
            if (response.length > 0) {
                debugger;
                var v = $.parseJSON(response);
                if (IssuedTo == "2") {
                    if (v.Table0[0].UCRReceiptNo != "") {
                        $('#Text_UCRNumber').val(v.Table0[0].UCRReceiptNo);
                        //$('#Text_UCRCarrierCode').val(v.Table0[0].airlinecode);
                        $('#Text_UHFNumber').val("111111");
                        $('#Text_UHFCarrierCode').val("111");
...
// add Sushant on 09-10-2017
function txtLoaded(evt) {
    if (/^[a-z0-9]+$/i.test(String.fromCharCode(evt.charCode)) == false) {
        evt.returnValue = false;
        return false
    }
}
function txtmobile(e) {
...
$(document).on('keypress keyup blur', '#Text_Name', function (evt) {
...
$(document).on('keypress keyup blur', '#Text_IDNumber', function (evt) {
    var keyCode = event.keyCode || event.which
    // Don't validate the input if below arrow, delete and backspace keys were pressed 
...
var YesReady = false;
function PageRightsCheckLuc() {
    var CheckIsFalse = 0;
    $(userContext.PageRights).each(function (e, i) {
        if (i.Apps.toString().toUpperCase() == "LUC") {

            if (i.Apps.toString().toUpperCase() == "LUC" && i.PageRight == "New") {
                YesReady = false;
                CheckIsFalse = 1;
                return
            } if (i.Apps.toString().toUpperCase() == "LUC" && i.PageRight == "Edit") {
                YesReady = false;
                CheckIsFalse = 1;
                return
            } if (i.Apps.toString().toUpperCase() == "LUC" && i.PageRight == "Delete") {
                YesReady = false;
                CheckIsFalse = 1;
                return
