...
function CreateMatrix(result) {
    ...
    cfi.AutoCompleteV2("WarehouseName", "WarehouseName", "Warehouse_WarehouseName", WarehouseChange, "contains", null, null, null, null);
    ...
    cfi.AutoCompleteV2("WarehouseSubLocation", "SubLocationType", "Warehouse_SubLocationType", SubLocationChange, "contains");
    cfi.AutoCompleteV2("WHTerminal", "TerminalName", "Warehouse_TerminalName", TerminalNameChange, "contains");
...
//Add By Sushant Kumar Nayak On 07-05-2018
function WarehouseTypeCheck() {

    var WarehouseName = $("#WarehouseName").val();
    var AirportName = $("#lblAirportName").text();
    var WarehouseCod = $("#lblWarehouseCode").text();

    $.ajax({
        url: "Services/Warehouse/WarehouseSetupService.svc/WarehouseTypeCheck", async: false, type: "POST", dataType: "json", cache: false,
        data: JSON.stringify({ WarehouseName: WarehouseName, AirportName: AirportName, WarehouseCod: WarehouseCod }),
        contentType: "application/json; charset=utf-8",
        success: function (result) {
            if (result.length > 0) {
                if (jQuery.parseJSON(result).Table0[0].WarehouseType != "") {
                    $("#lblWarehouseType").text(jQuery.parseJSON(result).Table0[0].WarehouseType)
                }
            }
        }
    });
...
function EditSubArea(htm) {
    ...
    var obj = { obj: { WHColumnNo: seletedTd.join(",") } }
    $.ajax({
        ...
        success: function (result) {
            ...
            if ($('#spnLocationName').length == 0)
                $(".k-window-title").closest("span").append("&nbsp;<span id='spnLocationName' style='color:red'><b>" + result.LocationName + "</b></span>");
            else
                $('#spnLocationName').html('<b>' + result.LocationName + '</b>');
...
function OpenSaveAreaWindow() {
    ...
    $("#Text_Area").val('');
    $("Area").val('');
    cfi.AutoCompleteV2("Area", "AreaName", "Warehouse_AreaName", null, "contains", null, null, null, null, AreaSelected);
}
...
function BindSubLocationControls() {
    cfi.AutoCompleteV2("Terminal", "TerminalName", "Warehouse_Terminal_Search", "SNo", "TerminalName", ["TerminalName"], null, "contains", ",");
    $("#Text_Terminal").data("kendoAutoComplete").setDefaultValue(TerminalSNo, TerminalName);
    $("#Text_Terminal").css("cursor", "not-allowed");
    $("#Text_Terminal").data("kendoAutoComplete").enable(false);

    cfi.AutoCompleteV2("Airline", "AirlineName", "Warehouse_Airline_Search", null, "contains");
    cfi.AutoCompleteV2("SHC", "Code", "Warehouse_SHC_Search", null, "contains", ",");
    cfi.AutoCompleteV2("DestCountry", "CountryCode,CountryName", "Warehouse_Country", null, "contains", ",");
    cfi.AutoCompleteV2("DestCity", "CityCode,CityName", "Warehouse_City", null, "contains", ",");
    cfi.AutoCompleteV2("AgentForwarder", "ParticipantID,Name", "Warehouse_AgentForwarder_Search", null, "contains", ",");
    cfi.AutoCompleteV2("Location", "LocationType", "Warehouse_Location_Search", null, "contains", ",", null, null, null);
    cfi.AutoCompleteV2("SubLocation", "SubLocationType", "Warehouse_SubLocationType", null, "contains", null, null, null, null, onSubLocationSelect);

    var template = '<span>#: TemplateColumn #</span><div style="height:10px;padding:5px;background-color:#: GetAutoColor(TemplateColumn) #"/></div>';
    cfi.AutoCompleteV2("LocationColorCode", "ColorName,ColorCode", "Warehouse_ColorCode", null, "contains", null, null, null, null, onColorSelect, null, template);
}
...
function MakeRoomGrid(getrecord, SNo) {
    ...
    $("#" + dbtableName).appendGrid({
        ...
        columns: [{ name: 'SNo', type: 'hidden', value: 0 },
            ...
            { name: 'SensorName', display: 'Sensor', type: 'text', ctrlAttr: { controltype: 'autocomplete', maxlength: 100 }, AutoCompleteName: 'Warehouse_TempratureSensorName', filterField: 'TempratureSensorName', filterCriteria: 'contains', separator: ',' },
...
function ExtraCondition(a) {
    ...
    if ("Text_AgentForwarder" == a)
        return a = cfi.getFilter("AND"), cfi.setFilter(a, "AccountTypeName", "eq", "FORWARDER"), cfi.setFilter(a, "SNo", "notin", $("#AgentForwarder").val()), filter = cfi.autoCompleteFilter(a);
    if ("Text_Location" == a)
    ...
    if ("Text_Terminal_Search" == a) // Added by Ankit Kumar      
        if ($('#Text_WarehouseDetails_Search').val()!="") {
           return a = cfi.getFilter("AND"), cfi.setFilter(a, "SNo", "eq", $("#WarehouseDetails_Search").val()), filter = cfi.autoCompleteFilter(a);
        }
    };
...
function SearchLocation() {
    $.ajax({
        ...
        success: function (result) {
            ...
            $("#DestCity_Search").closest('td').prev('td').html('<input type="checkbox" name="DestAsLocation"/><span style="color:red;">Destination as location</span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<b>Dest City</b>');
            BindSearchControls();
        }
    });
}
...
function BindSearchControls() {
    cfi.AutoCompleteV2("Terminal_Search", "TerminalName", "Warehouse_TerminalName", null, "contains");//Added by Ankit Kumar
    cfi.AutoCompleteV2("Airline_Search", "CarrierCode,AirlineName", "Warehouse_AirlineSearch", null, "contains");
    cfi.AutoCompleteV2("SHC_Search", "Code", "Warehouse_SHC_Search", null, "contains");
    cfi.AutoCompleteV2("DestCountry_Search", "CountryName", "Warehouse_DestCountry_Search", null, "contains");
    cfi.AutoCompleteV2("DestCity_Search", "CityCode,CityName", "Warehouse_DestCity_Search", null, "contains");
    cfi.AutoCompleteV2("AgentForwarder_Search", "ParticipantID,Name", "Warehouse_AgentForwarder_Search", null, "contains");
    cfi.AutoCompleteV2("Location_Search", "LocationType", "Warehouse_Location_Search", null, "contains");
    ...
    $("#Text_SearchAction").data("kendoAutoComplete").setDefaultValue(1, "SEARCH");
    $("#Text_SearchAction").attr("placeholder", "Search Action");
    $("#Text_SearchAction").attr("title", "Select a Search Action");
    //Show warehouse rows on search warehouse and set up page hide tr windows
    if (PageHints == 1) {
        cfi.AutoCompleteV2("WarehouseDetails_Search", "WarehouseName", "Warehouse_WarehouseNameGetList", null, "contains");
    ...
    var SearchDataSource = [{ Key: "1", Text: "AWB NUMBER" }, { Key: "2", Text: "ULD NUMBER" }, { Key: "3", Text: "MOVABLE" }, { Key: "4", Text: "LOCATION NAME" }];
...
function SearchData() {
    ...
    if (type == "") {
        ...
        ShowMessage('warning', 'Warning !!', "Please Select a Search Action.!", "bottom-right");
        return false;
    }
    if (type == "4") {
        ClearAutoComplete();
    ...
    if (type == "3") {
        ...
        obj = { pageNo: 1, pageSize: 100000, model: BindWhereCondition(), sort: "", }
    }
    else {
        obj = {
            ...
            SubLocation: $("#Search").val() || 0,//Search By dd
            SubAreaName: $("#Text_SearchBy").val(),//@SearchText textbox
            WarehouseSNo: PageHints == 1 ? WhSearchSNo : warehouseSNo, // added by purushottam
            AirportSNo: userContext.AirportSNo,
            OriginCitySNo: userContext.CitySNo,
            DestAsLocation: $('input[name="DestAsLocation"]').is(':checked') == true ? 1 : 0
        }
    }
    $.ajax({
        type: "POST",
        ...
        success: function (response) {
            if (response) {
                ...
                //if (type == "2") {
                //    BindLoactionSearchGrid2(response, type)
                //}
                if (type == "3") {
                    var str = "<table>";
                    ...
                    for (var i = 0; i < response.value.length; i++) {
                        str += "<tr><td>" + response.value[i].LocationName + "</td><td>" + response.value[i].SPHC + "</td><td>" + response.value[i].AWBNo + "</td><td>" + response.value[i].CountryName + "</td><td>" + response.value[i].CityName + "</td><td>" + response.value[i].TerminalName + "</td><td>" + response.value[i].AgentName + "</td><td>" + response.value[i].AirlineName + "</td></tr>"
                    }
                    str += "</table>";
...
function findAWBLocation(evtval, WHSetSNo) {
    if (WHSetSNo == '') {
        ShowMessage('info', 'Need your Kind Attention!', "No location has been assigned.");
        return false;
    }
    PageLoaded(WHSetSNo);
    //var obj = {
...
function FindLocation() {
    ...
    var obj = {
        Terminal: $("#Terminal_Search").val() || 0,
        Airline: $("#Airline_Search").val() || 0,
        SHC: $("#SHC_Search").val() || 0,
        DestCountry: $("#DestCountry_Search").val() || 0,
        DestCity: $("#DestCity_Search").val(),
        AgentForwarder: $("#AgentForwarder_Search").val() || 0,
        Location: $("#Location_Search").val() || 0,
...
function BindWhereCondition() {
    return {
        WHSetupSNo: $("#Text_WarehouseDetails_Search").data("kendoAutoComplete").key() || 0,
        TerminalSNo: $("#Text_Terminal_Search").data("kendoAutoComplete").key() || 0,
        AirlineSNo: $("#Text_Airline_Search").data("kendoAutoComplete").key() || 0,
        SPHCSNo: $("#Text_SHC_Search").data("kendoAutoComplete").key() || 0,
        DestinationCountrySNo: $("#Text_DestCountry_Search").data("kendoAutoComplete").key() || 0,
        DestinationCitySNo: $("#Text_DestCity_Search").data("kendoAutoComplete").key() || 0,
        AccountSno: $("#Text_AgentForwarder_Search").data("kendoAutoComplete").key() || 0,
        WHTypeSNo: $("#Text_Location_Search").data("kendoAutoComplete").key() || 0,
        SearchText: $("#Text_Search").data("kendoAutoComplete").key() || 0,
        SearchBy: $("#Text_SearchBy").val() || "",
        SearchAction: $('#SearchAction').val() || 0,
        OriginCitySNo: userContext.CitySNo,
        DestAsLocation: $('input[name="DestAsLocation"]').is(':checked') == true ? 1 : 0
    };
}
...
function bindhtml(evt, rowIndex) {
    ...
    $("#hdnConsumableSNo").val($("#tblLocationSearch_ConsumableSNo_" + evt.data.uIndex).val());
    cfi.AutoCompleteV2("LSearch", "LocationName", "Warehouse_LocationNameSearch", DropDownBindLocation, "contains");
    cfi.AutoCompleteV2("MPieceNo", "PieceNo", "Warehouse_MPieceNo", BindLiColorChange, "contains", ",");//, true, null, null, null, true, null
...
function bindhtmlMoveable(evt, rowIndex) {
    ...
    cfi.AutoCompleteV2("LSearch", "LocationName", "Warehouse_LSearch", null, "contains");
    ...
    $.ajax({
        ...
        success: function (response) {
            if (response.length > 0) {
                ...
                var str = "<html><table  style='width:90%;'><tr font-size:12pt;font-family:Arial'><td align=\"left\" colspan='9'><b>Garuda Indonesia</b></td></tr><tr font-size:12pt;font-family:Arial'><td align=\"center\"  colspan='9'><b>Warehouse Location</b></td></tr></table> "
...
$(document).ready(function () {
    ...
    $(document).on('keyup', '#Text_SearchBy', function () {
        var yourInput = $('#Text_SearchBy').val();
    var textsearch = $('#Text_Search').val();
    if (textsearch != "LOCATION NAME") {
        re = /[`~!@#$%^&*()_|+\=?;:'",.<>\{\}\[\]\\\/]/gi;
