...
$(function () {
    ...
    $("#spnIsFocConsignee").closest("tr").find("td").hide();
    ...
    cfi.AutoCompleteV2("CityCode", "CityName,CityCode,SNo", "Master_Customer_CityName", getCountrySNo, "contains");
    cfi.AutoCompleteV2("CustomerTypeSNo", "CustomerTypeName,SNo", "Master_Customer_CustomerType", EventHideShow, "contains");
    cfi.AutoCompleteV2("AccountSNo", "Name,ParticipantID,SNo", "Master_Customer_AccountName", null, "contains", null, null, null, null, CheckConsigneeAsfarwarder);
    $("#TaxID").removeAttr('data-valid');
    //==================Added by arman Date : 2017-08-05===============
    $("#spnAgreementNumber").closest('td').find('font').text('');
    $("#AgreementNumber").removeAttr("data-valid");
    //====================End=====================================
    ...
    $('#AgreementNumber').keyup(function () {
        var AgreementNumber = $(this).val()
        CheckAgreementNumber(AgreementNumber)
        ...
        $('#tbl' + dbTableName).appendGrid({
            ...
            columns: [
               ...
               // { name: 'Address', display: 'Address', type: 'text', ctrlAttr: { maxlength: 98, controltype: 'text' }, ctrlCss: { width: '100px' }, isRequired: true },
                { name: 'CountrySNo', display: 'Country Code', type: 'text', ctrlAttr: { maxlength: 100, controltype: 'autocomplete' }, ctrlCss: { width: '60px', height: '20px' }, isRequired: true, AutoCompleteName: 'Master_Customer_CountryName', filterField: 'CountryCode' },
                { name: 'CitySNo', display: 'City Name', type: 'text', ctrlAttr: { maxlength: 100, controltype: 'autocomplete' }, ctrlCss: { width: '70px', height: '20px' }, isRequired: true, AutoCompleteName: 'Master_Customer_CityName', filterField: 'CityCode,CityName' },
                ...
                { name: 'Street', display: 'Address 1', type: 'text', ctrlAttr: { maxlength: 35, minlength: 1, controltype: "alphanumericupper", allowchar: "" }, ctrlCss: { width: '135px' }, isRequired: true },
                { name: 'Address2', display: 'Address 2', type: 'text', ctrlAttr: { maxlength: 80, minlength: 1, controltype: "alphanumericupper", allowchar: "!@#$%^&*()-_=\"+[]{};:'|?/.,<>`~" }, ctrlCss: { width: '135px' } },
                ...
                { name: 'Phone', display: 'Contact No', type: 'text', ctrlAttr: { maxlength: 16, minlength: 6, controltype: "number", onblur: "return checkPhoneNumber(this.id);", oninput: "this.value = this.value.replace(/[^0-9]/g, '');" }, ctrlCss: { width: '90px' }, isRequired: true },
...
function GetAuthorizedPersonnel() {
    ...
    if (getQueryStringValue("FormAction").toUpperCase() == "NEW" || getQueryStringValue("FormAction").toUpperCase() == "DUPLICATE") {
        ...
        $('#divareaTrans_Master_AuthorizedPersonal table').find("[id^='Name']").attr('data-valid-msg', 'Name Cannot be Blank'); // changes by arman ali

        $('#divareaTrans_Master_AuthorizedPersonal table').find("[id^='IdCardNo']").attr('data-valid', 'required');
        $('#divareaTrans_Master_AuthorizedPersonal table').find("[id^='IdCardNo']").attr('data-valid-msg', 'Id Card No Cannot be Blank');  // changes by arman ali
...
function DownloadAuthorizesPERSONNEL(objId, nexctrlid) {
    ...
    var CustomerAuthorizedPERSONNELSNo = $("#" + objId).closest("tr").find("td input[id^='SNo']").val(); //$("#" + objId).closest("tr").find("td:eq(9)").text();
    var cusSnoint = parseInt(CustomerAuthorizedPERSONNELSNo);
...
function EventHideShow(valueId, value, keyId, key) {
    if (value == "CONSIGNEE" && userContext.SpecialRights.FOC == true) {
        ...
        $("#spnIsFocConsignee").closest("tr").find("td").show();
        $("#spnIsFocConsignee").show();
        $("#divRadio").show();
        $('input[name="IsConsigneeAsForwarder"][value="1"]').attr("checked", true);
        $("[name='IsConsigneeAsForwarder']").attr("disabled", "disabled")
    }
    else {
    ...
    if (value == "CONSIGNEE") {
        ...
        if ($("#AccountSNo").val() != "") {
          ...
          //  $('input[name="IsConsigneeAsForwarder"][value="1"]').attr("checked", false);
            //$('#Text_AccountSNo').attr("data-valid", "required");
            //$('#Text_AccountSNo').attr("data-valid-msg", "Forwarder (Agent) Name cannot be blank");
            //$("#spnAccountSNo").closest('td').find("font").remove();
            //$("#spnAccountSNo").before('<font color="red">*</font>');
            //$('#Text_AccountSNo').css('border-color', '#94c0d2');
            $('input[name="IsConsigneeAsForwarder"][value="1"]').attr("checked", true);
    ...
    if (value == "CONSIGNEE" || value == "SHIPPER") {
       // $('#TaxID').attr('data-valid', 'required');
      //  $('#TaxID').attr('data-valid-msg', 'Tax ID is required');
    }
    else {
       // $('#TaxID').removeAttr('data-valid');
       // $('#TaxID').removeAttr('data-valid-msg');
    }
}
...
function CheckConsigneeAsfarwarder()
{
    if (getQueryStringValue("FormAction").toUpperCase() == "NEW" || getQueryStringValue("FormAction").toUpperCase() == "DUPLICATE")
    {
        if ($('#Text_CustomerTypeSNo').val() == "CONSIGNEE")
        {
            $("[type='radio'][id='IsConsigneeAsForwarder']").removeAttr("disabled", "disabled");
            $('input[name="IsConsigneeAsForwarder"][value="0"]').attr("checked", false);
            $('input[name="IsConsigneeAsForwarder"][value="1"]').attr("checked", true);
            //$('#Text_AccountSNo').attr("data-valid", "required");
            //$('#Text_AccountSNo').attr("data-valid-msg", "Forwarder (Agent) Name cannot be blank");
...
//=============end========================
//----- Sushant
function CheckAgreementNumber(AgreementNumber) {
    if (AgreementNumber.replace(/\s+/g, '').length > 0) {
        $.ajax({
            url: "Services/Master/CustomerService.svc/CheckAgreementNumber?AgreementNumber=" + AgreementNumber, async: false, type: "get", dataType: "json", cache: false,
            contentType: "application/json; charset=utf-8",
            success: function (result) {
                var myData = jQuery.parseJSON(result)
                if (myData.Table0.length != "0") {
                    ShowMessage('info', 'Need your Kind Attention!', "Agreement number already exist !");
                    $("#AgreementNumber").val('')
                }
...
// Addeed by Arman Date : 2017-08-16
$("input[name='operation'][value='Update']").click(function ()
{
    $("#hdnCustomerName").val($('#CustomerName').val().toUpperCase() + "-" + $('#CustomerName2').val().toUpperCase())
       // validateForwader();
});
// Addeed by Devendra Date : 2018-08-14
$('input[id=IsConsigneeAsForwarder]').click(function () {
    validateForwader();
})

function validateForwader() {
    if ($('input[id="IsConsigneeAsForwarder"][value="0"]').is(':checked') == true) {
        $('#Text_AccountSNo').attr("data-valid", "required");
        $('#Text_AccountSNo').attr("data-valid-msg", "Forwarder (Agent) name can not be blank");
        $("#spnAccountSNo").closest('td').find("font").remove();
        $("#spnAccountSNo").before('<font color="red">*</font>');
    ...
    if ($('input[id="IsConsigneeAsForwarder"][value="1"]').is(':checked') == true) {
        $('#Text_AccountSNo').removeAttr("data-valid");
        $('#Text_AccountSNo').removeAttr("data-valid-msg");
        $("#spnAccountSNo").closest('td').find("font").remove();
        $('#Text_AccountSNo').closest('span').css('border-color', '');
