...
$(document).ready(function () {
    ...
    cfi.AutoCompleteV2("ULDNo", "ULDNumber", "ULDTransPortReport_ULDNumber", null, "contains");

    cfi.AutoCompleteV2("Recd", "name", "ULDTransPortReport_Recd", null, "contains");

    cfi.AutoCompleteV2("UCR", "UCRReceiptNo", "ULDTransPortReport_UCRReceiptNo", null, "contains");
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.length > 0) {
                    ...
                    var str = "<html><table  style='width:90%;'><tr><td align=\"left\" style='width:30%;' >" + userContext.AirlineCarrierCode+" CARGO</td><td></td><td align=\"center\" style='width:50%;'>ULD Transfer Report</td><td align=\"right\" style='width:20%;'>Date : " + response[0].Dt +
                        "</td></tr><tr><td colspan='2' align='LEFT'>From " + Fdt + " To " + Tdt + "</td></tr></table> "



                    str += "<br/><table style='width:90%;'  border=\"1px\">";
                    str += "<tr ><td>ULD Nbr</td><td>UCR Nbr</td><td>Transferred  By</td><td>Issued To</td> <td>Received By</td><td>Issuance Date </td><td>Destination</td><td>Remarks</td><td>Demurrage Charges</td><td>Updated By</td></tr>"
                    ...
                    for (var i = 0; i < response.length; i++) {
                        str += "<tr><td>" + response[i].ULDNo + "</td><td>" + response[i].UCRReceiptNo + "</td><td>" + response[i].TransfredBy + "</td><td>" + response[i].IssuedTo + "</td><td>" + response[i].ReceivedBy + "</td><td>'" + response[i].IssuanceDate + "</td><td>" + response[i].Destination
                            + "</td><td>" + response[i].Remarks + "</td><td>" + response[i].Amount + "</td><td>" + response[i].UserName
...
function ULDOutGrid() {
    if (cfi.IsValidSubmitSection()) {
        ...
        $('#tbl' + dbtableName).appendGrid({
            ...
            tableColume: 'ULDNo',
            masterTableSNo: 1,
            currentPage: 1, itemsPerPage: 500, model: BindWhereCondition(),
            sort: '',
            ...
            columns: [
                      ...
                      { name: 'UCRReceiptNo', display: 'UHF/UCR Nbr', type: 'label' },
                      ...
                      { name: 'Destination', display: 'Destination', type: 'label' },
                       { name: 'Remarks', display: 'Remarks', type: 'label' },
                        { name: 'Amount', display: 'Demurrage Charges', type: 'label' },
