...
function UWSInfoList() {
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            cfi.AutoCompleteV2("searchRouting", "AirportCode,AirportName", "UWS_AirportName", null, "contains");
            cfi.AutoCompleteV2("searchAirline", "CarrierCode,AirlineName", "UWS_AirlineName", null, "contains");
            cfi.AutoCompleteV2("searchProcess", "ProcessName", "UWS_ProcessName", null, "contains");          
            cfi.AutoCompleteV2("searchFlightNo", "SNo,FlightNo", "UWS_FlightNo", null, "contains");
            ...
            PagerightsCheckUWSInfo()
        }
    });
}
var PartnerCarrierCode = '';
function BindEvents(obj, e, isdblclick) {
    ...
    sliSNoIndex = 1;
    var isCartIndex = trLocked.find("th[data-field='IsCart']").index();
    ...
    PartnerCarrierCode = closestTr.find("td[data-column=PartnerAirline]").text().trim();
    ...
    IsCart = closestTr.find("td:eq(" + isCartIndex + ")").text() == "true" ? true : false;
...
function ShowProcessDetails(subprocess, isdblclick) {
    ...
    if (subprocess.toUpperCase() == "UWSPRINT") {
    ...
    else if (subprocess.toUpperCase() == "CHANGEEQUIPMENT") {
        $('#btnSave').hide();
        $('#btnSaveToNext').hide();
        InitializePage(subprocess, "divNewDetail", isdblclick);
    }
    ...
    else if (subprocess.toUpperCase() == "OFFLOADCART") {
        $('#btnSave').hide();
        $('#btnSaveToNext').hide();
        InitializePage(subprocess, "divNewDetail", isdblclick);
    }
        ...
        cfi.AutoCompleteV2("FlightNo", "FlightNo", "UWS_FlightNoN", null, "contains");
...
function OnSuccessGrid() {
    ...
    var IsCartIndex = TrHeader.find("th[data-field='IsCart']").index();
    ...
    $("div[id$='divDetail']").find("div[class^='k-grid-content'] tbody tr").each(function (row, tr) {
        ...
        if ($(tr).find("td:eq(" + IsCartIndex + ")").text() == "true" && $(tr).find('input[type="button"][process="OFFLOADCART"]').attr('process') == "OFFLOADCART") {
            $(tr).find('input[type="button"][process="OFFLOADCART"]').css("display", "block");
        }
        else {
            $(tr).find('input[type="button"][process="OFFLOADCART"]').css("display", "none");
        }
        ...
        if ($(tr).find("td:eq(" + DepartedIndex + ")").text() == "false"  && ($(tr).find("td:eq(" + ShipmentIndex + ")").text() == "false" ||$(tr).find("td:eq(" + IsCartIndex + ")").text() == "true") && ($(tr).find("td:eq(" + ProcessIndex + ")").text() == "Export") && $(tr).find("td:eq(" + FlightIndex + ")").text() != "") {
...
function ShowGridBulkData(obj) {
    ...
    if (CheckEquipmentNoOFLD_Status(UWSSNo, EquipmentNo)) {
            ...
            success: function (result) {
                ...
                if (data.Table0.length > 1) {
                    ...
                    if (data.Table1.length == 0) {
                        ...
                        //for (var i = 1; i < data.Table0.length; i++) {
                        //    table += "<table width='100%'><thead  style='text-align:center'><tr><td colspan='3'></td><td ><a href='#' onClick='showOverhangData(); return false;'><b>Overhang Details</b></a></td></tr><tr class='ui-widget-header'><td>ULD Class</td><td>ULD Loading Code </td><td> ULD Loading Indicator</td><td>ULD Contour Code</td></tr><tr><td style='text-align:center'><label id='lblULDClass'>" + data.Table0[i].ULDClass + "</label></td><td style='text-align:center'><label id='lblULDLoadingCode'>" + data.Table0[i].ULDLoadingCode + "</label></td><td><label id='lblULDLoadingIndicator' name='lblULDLoadingIndicator'>" + data.Table0[i].ULDLoadingIndicator + "</label></td><td style='text-align:center'>" + data.Table0[i].ULDContour + "</td></tr></table>";
                        //}
                        ...
                        for (var i = 1; i < data.Table0.length; i++) {
                            table += "<table width='100%'><thead  style='text-align:center'><tr><td colspan='3'></td><td ><a href='#' onClick='showOverhangData(\"" + data.Table1[i].ULDStockSNo + "\",\"" + data.Table1[i].DailyFlightSNo + "\"); return false;'><b>Overhang Details</b></a></td></tr><tr class='ui-widget-header'><td>ULD Class</td><td>ULD Loading Code </td><td> ULD Loading Indicator</td><td>ULD Contour Code</td></tr><tr><td style='text-align:center'><label id='lblULDClass'>" + data.Table0[i].ULDClass + "</label></td><td style='text-align:center'><label id='lblULDLoadingCode'>" + data.Table0[1].ULDLoadingCode + "</label></td><td><label id='lblULDLoadingIndicator' name='lblULDLoadingIndicator'>" + data.Table0[1].ULDLoadingIndicator + "</label></td><td style='text-align:center'>" + data.Table0[1].ULDContour + "</td></tr></table>";
                        }
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                if (data.Table0.length >= 1) {
                    ...
                    tbl += "<table width='100%'><thead  style='text-align:center'><tr class='ui-widget-header'><td>AWB No</td><td>Pieces </td></tr>";
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            if (data.Table1.length >= 1) {
                ...
                Waybill += "<tr ><td colspan='3'><label id='lblCutOffHeight'>" + data.Table0[0].CutOffHeight + "</label></td><td colspan='3'><label id='lblCutOffMesUnit'>" + data.Table0[0].text_CutOffMesUnit + "</label></td></tr>";
                theDiv.innerHTML += Waybilltable;
                theDiv.innerHTML += Waybill;
                cfi.PopUp("divULDOverhang", "ULD Overhang", "800");
...
function GetChangeEquipment() {
    var UserSNo = userContext.UserSNo;
    var HTMLResult = "";
    HTMLResult = HTMLResult + "</table><table class='WebFormTable' id='tblSave'  border=\"0\" cellpadding='0' cellspacing='0' align=\"center\"><tr><td style='height:30px;text-align:right;font-size:12px; font-family: Verdana'>New Equipment No:</td><td class='formthreeInputcolumn'><input type='hidden' name='ChangeEquipmentNo' id='ChangeEquipmentNo' value=''/> <input type='text'  name='Text_ChangeEquipmentNo' id='Text_ChangeEquipmentNo' tabindex='2' controltype='autocomplete' maxlength='20' /></td><td><input type='button' tabindex='3' class='btn btn-block btn-success btn-sm' name='btnSaveChangeEquipment id='btnSaveChangeEquipment' style='width:90px;' value='Save' onclick='SaveChangeEquipment();'></td></tr></table>";

    $("#divChangeEquipmentNo").html(HTMLResult)

   // cfi.AutoComplete("ChangeEquipmentNo", "EquipmentNo", "vwAllEquipment", "EquipmentSNo", "EquipmentNo", ["EquipmentNo"], null, "contains");
    cfi.AutoCompleteV2("ChangeEquipmentNo", "EquipmentNo", "UWS_ChangeEquipmentNo", null, "contains");


    cfi.PopUp("divChangeEquipmentNo", "Change Equipment", 500);
...
function SaveChangeEquipment() {
    var UserSNo = userContext.UserSNo;
    var NewEquipmentSNo = $("#ChangeEquipmentNo").val() == "" ? 0 : $("#ChangeEquipmentNo").val();
    if (NewEquipmentSNo == 0) {
        ShowMessage('warning', 'Warning - UWS', "New Equipment is mandatory", "bottom-right");
        return false;
    }
    else {
        $.ajax({
            url: "Services/Shipment/UWSInfoService.svc/SaveChangeEquipment", async: false, type: "POST", dataType: "json", cache: false,
            data: JSON.stringify({ UWSSNo: currentUWSSNo, EquipmentNo: CurrentEquipmentNo, NewEquipmentSNo: NewEquipmentSNo, UserSNo: UserSNo }),
            contentType: "application/json; charset=utf-8",
            success: function (result) {
                if (result == "1") {
                    ShowMessage('success', 'Success - UWS', "Equipment Changed from " + CurrentEquipmentNo + " to " + $("#Text_ChangeEquipmentNo").val() + " successfully.", "bottom-right");
                    cfi.ShowIndexView("divDetail", "Services/Shipment/UWSInfoService.svc/GetUWSGridData/" + _CURR_PRO_ + "/UWS/Booking/0/0/0/0/1222", "Scripts/maketrans.js?" + Math.random());
                    $("#divChangeEquipmentNo").data("kendoWindow").close();
                    $("#divFlightAssign").data("kendoWindow").close();
                    $("div").remove(".k-overlay");
                }
                else if (result == "2") {
                    ShowMessage('warning', 'Warning -UWS', " Equipment Already Utilized,Please eneter another Equipment. ", "bottom-right");
                }
...
function CheckEquipmentNoOFLD_Status(UWSSNo, EquipmentNo) {
    var flag = true;
    $.ajax({
            url: "Services/Shipment/UWSInfoService.svc/CheckEquipmentNoOFLD_Status", async: false, type: "POST", dataType: "json", cache: false,
        data: JSON.stringify({ UWSSNo: UWSSNo, EquipmentNumber: EquipmentNo}),
            contentType: "application/json; charset=utf-8",
            success: function (result) {
                ...
                if (ResultStatus == "1") {
                    ShowMessage('warning', 'Warning -UWS', ResultValue, "bottom-right");
                    flag = false;
                }
            }  
    });
...
function GetUWSEquipmentPrint() {
    ...
    $.ajax({
        url: "Services/Shipment/UWSInfoService.svc/GetUWSEquipmentPrint?UWSSNo=" + currentUWSSNo + "&EquipmentNo=" + EquipmentNo + "&UserSNo=" + UserSNo,
        contentType: "application/json; charset=utf-8",
        async: false,
        type: 'get',
        cache: false,
        success: function (result) {
            ...
            if (IsDGR == "false") {
                AllResult = AllResult + '<tr><td>' + $("#divULDTag").html() + '</td></tr>';
            }
            else {
                AllResult = AllResult + '<tr><td style="background: url(./Images/DangrousGoodsFront.png) ;-webkit-print-color-adjust:exact;">' + $("#divULDTag").html() + '</td></tr>';
            }
            // }
            AllResult = AllResult + "</table>";
            $("#divNewDetail").html(AllResult);
...
function OffloadCart() {
    var r = jConfirm("Do you wish to Off-Load this Cart ?", "", function (r) {
        if (r == true) {
            $.ajax({
                url: "Services/Shipment/UWSInfoService.svc/OffloadCart", async: false, type: "POST", dataType: "json", cache: false,
                data: JSON.stringify({ UWSSNo: currentUWSSNo, EquipmentNumber: CurrentEquipmentNo }),
                contentType: "application/json; charset=utf-8",
                success: function (result) {
                    var ResultStatus = result.split('?')[0];
                    var ResultValue = result.split('?')[1];
                    if (ResultStatus == "0") {
                        ShowMessage('success', 'Success -UWS', ResultValue, "bottom-right");
                        UWSInfoList();
                    }
...
function GetUWSRemoveBulk() {
    ...
    $.ajax({
        ...
        success: function (result) {
            var ResultData = jQuery.parseJSON(result);
            var FinalStatus = ResultData.Table0;
            var FinalData = ResultData.Table1;
            if (FinalStatus[0].FlightStatus == 'BUILD UP') {
                ShowMessage('warning', 'Warning -UWS', "Flight is in Build Up stage. Amendments restricted ", "bottom-right");
            }
            else
            {
                $("#divULDTag").html('');
                ...
                $("#divReWeiging").html("");
                var totalScaleWeight = 0;
                //<input type='checkbox' id='chkAllAWB'>
                //$("#divBulkRemove").html
                HTMLResult = "<table class='WebFormTable' ><thead><tr><td class='ui-widget-header' style='text-align:center'>Select</td><td class='ui-widget-header' style='text-align:center'>No<input type='hidden' name='hdnNewScaleWeightUWSSNo' id='hdnNewScaleWeightUWSSNo' value='" + currentUWSSNo + "'/><input type='hidden' name='hdnRemoveBulkEquipmentNo' id='hdnRemoveBulkEquipmentNo' value='" + EquipmentNo + "'/></td><td class='ui-widget-header' style='text-align:center'>AWB No</td><td class='ui-widget-header' style='text-align:center'>Equipment No</td><td class='ui-widget-header' style='text-align:center'>Total Pieces</td><td class='ui-widget-header' style='text-align:center'>Remove Pieces</td><td class='ui-widget-header' style='text-align:center'>Gross Weight</td><td class='ui-widget-header' style='text-align:center'>Average Gross Weight</td></tr></thead><tbody id='tblBDYRemoveBulk'>"
                for (var i = 0; i < FinalData.length; i++) {
                    HTMLResult = HTMLResult + "<tr><td  class='formthreelabel' style='text-align:center'><input type='checkbox' id='chkAWB_" + i + "'></td><td class='formthreelabel' style='text-align:center'>" + FinalData[i].No + "</td><td class='formthreelabel' style='text-align:center'><label id='lblAWBNo" + i + "'>" + FinalData[i].AWBNo + "</label></td><td class='formthreelabel' style='text-align:center'><label id='lblEquipmentNo" + i + "'>" + EquipmentNo + "</label></td><td class='formthreelabel' style='text-align:center'><label id='lblPieces" + i + "'>" + FinalData[i].Pieces + "</label></td><td class='formthreelabel' style='text-align:center'><input type='text' id='txtRemovedpieces" + i + "' controltype='number' data-role='numerictextbox' onblur='fnCalScaleWeight(this)' style='width:70px' maxlength='5' name='txtRemovedpieces" + i + "'/></td><td class='formthreelabel' style='text-align:center'><input type='text' id='txtRemovedGrossWeight" + i + "' controltype='number' onblur='fnCalScaleWeight(this)'  data-role='numerictextbox' style='width:70px' maxlength='5' name='txtRemovedGrossWeight" + i + "'/></td><input type='hidden' name='SNo" + i + "' id='SNo" + i + "' value='" + FinalData[i].SNo + "'/></td><td class='formthreelabel' style='text-align:center'><label id='lblGrossWeight" + i + "' style='display:none;' >" + FinalData[i].GrossWeight + "</label><label id='lblAverageGrossWeight" + i + "' style='display:none;'>" + FinalData[i].GrossWeight + "</label></td></tr>";
                    totalScaleWeight = totalScaleWeight + parseFloat(FinalData[i].GrossWeight);
                }
                HTMLResult = HTMLResult + "</tbody><tfoot><tr><td colspan='8'></td></tr>";
                if (currentGrossWeight > 0) {
                    HTMLResult = HTMLResult + "<tr><td colspan='5'></td><td class='formthreelabel' style='font-size-bold'>New Scale Weight</td><td><input type='text' id='txtNewScaleWeight' controltype='number' data-role='numerictextbox' style='width:70px' maxlength='10' name='txtNewScaleWeight'/></td><td class='formthreelabel' style='text-align:right'><input type='button' class='btn btn-block btn-success btn-sm' id='btnRemovePieces' value='Save' onClick='SaveRemovepieces(this)'/></td></tr></tfoot>";
                ...
                $('#txtNewScaleWeight').val(parseFloat(parseFloat(CurrentTareWeight) + totalScaleWeight).toFixed(2));
                ...
                $('[id^=txtRemovedpieces]').bind("keydown", function () {
                    ...
                    var posi = this.id.replace('txtRemovedpieces', ''); //this.id.charAt((this.id.length) - 1);
                    ...
                    $("#txtRemovedGrossWeight" + posi).val(AverageGrossWeight.toFixed(2));
                    if (Pieces > Totalpieces) {
                        $("#" + this.id).val('');
                        $("#lblAverageGrossWeight" + posi).text(totalGrossWeight.toFixed(2));
                        $("#txtRemovedGrossWeight" + posi).val(AverageGrossWeight.toFixed(2));
                    }
                });

            $('[id^=txtRemovedGrossWeight]').bind("keydown", function () {
                if (event.shiftKey == true) {
                    event.preventDefault();
                }
                if (!((event.keyCode >= 48 && event.keyCode <= 57)) && !((event.keyCode >= 96 && event.keyCode <= 105)) && !(event.keyCode == 8) && !(event.keyCode == 37) && !(event.keyCode == 46) && !(event.keyCode == 110)) {
                    //not a number key or period so prevent
                    event.preventDefault();
                }
            }).keyup(function () {
                var GrossWeight = parseFloat($("#" + this.id).val() == "" ? 0 : $("#" + this.id).val());
                var posi = this.id.replace('txtRemovedGrossWeight', ''); //this.id.charAt((this.id.length) - 1);
                var totalGrossWeight = parseFloat($("#lblGrossWeight" + posi).text());
                if (GrossWeight > totalGrossWeight) {
                    $("#" + this.id).val('');
                    $("#txtRemovedGrossWeight" + posi).val($("#lblAverageGrossWeight" + posi).text());
                }
            });
...
function fnCalScaleWeight(obj) {

    if ($('#txtRemovedGrossWeight' + $(obj).closest('tr').index()).val() == '')
        return

    var totalAvgWeight = 0;
    $('#lblAverageGrossWeight' + $(obj).closest('tr').index()).show()
    if ($(obj).attr('id') == 'txtRemovedpieces' + $(obj).closest('tr').index()) {
        $('#txtRemovedGrossWeight' + $(obj).closest('tr').index()).val($('#lblAverageGrossWeight' + $(obj).closest('tr').index()).text())
    }
    $('#tblBDYRemoveBulk tr').each(function (row, tr) {
        totalAvgWeight = totalAvgWeight + parseFloat($('#txtRemovedGrossWeight' + row).val() == '' ? 0 : $('#txtRemovedGrossWeight' + row).val());
       
    });
    $(obj).css('border-color', '');
    $("#txtNewScaleWeight").val(parseFloat(parseFloat(currentGrossWeight) - totalAvgWeight).toFixed(2));
}
...
function fn_CheckValidation() {
    ...
    var totalPlanGrossWeight=0;
    $('#tblBDYRemoveBulk tr').each(function (row, tr) {
        TotalRowCount = row+1;
        if ($('#chkAWB_' + row).prop('checked')) {
            //RowCount = RowCount + 1;
            var totalPieces = parseInt($('#lblPieces' + row).text());
            var removePieces = !Number.isNaN(parseInt($('#txtRemovedpieces' + row).val())) ? parseInt($('#txtRemovedpieces' + row).val()) : 0;
            var totalGrossWeight = parseFloat($('#lblGrossWeight' + row).text());
            var removeGrossWeight = !Number.isNaN(parseFloat($('#txtRemovedGrossWeight' + row).val())) ? parseFloat($('#txtRemovedGrossWeight' + row).val()) : 0;
            totalPlanGrossWeight = totalPlanGrossWeight + parseFloat($('#lblAverageGrossWeight' + row).text());
            if (removePieces > totalPieces) {
                ShowMessage('warning', 'Warning - UWS', "Please enter correct pieces.", "bottom-right");
                $('#txtRemovedpieces' + row).focus().css('border-color', 'red');
                chkFlag = false;
                return false;
            }
            else if (removePieces == 0 || removePieces == "") {
                ShowMessage('warning', 'Warning - UWS', "Please enter pieces.", "bottom-right");
                $('#txtRemovedpieces' + row).focus().css('border-color', 'red');
                chkFlag = false;
                return false;
            }
            else if (removeGrossWeight > totalGrossWeight) {
                ShowMessage('warning', 'Warning - UWS', "Please enter correct Gross Weight.", "bottom-right");
                $('#txtRemovedGrossWeight' + row).focus().css('border-color', 'red');
                chkFlag = false;
                return false;
            }
            else if (removeGrossWeight == 0 || removeGrossWeight == "") {
                ShowMessage('warning', 'Warning - UWS', "Please enter Gross Weight.", "bottom-right");
                $('#txtRemovedGrossWeight' + row).focus().css('border-color', 'red');
                chkFlag = false;
                return false;
            }
    ...
    if ($("#txtNewScaleWeight").val() != undefined && $("#txtNewScaleWeight").val() == "") {
        ShowMessage('warning', 'Warning - UWS', "Please enter New Scale Weight.", "bottom-right");
        $('#txtNewScaleWeight').focus().css('border-color', 'red');
        chkFlag = false;
        return false;
    }
    else if (parseFloat($("#txtNewScaleWeight").val()) <= parseFloat(CurrentTareWeight) && $("#txtNewScaleWeight").val() != undefined && (currentGrossWeight - totalPlanGrossWeight) != parseFloat(CurrentTareWeight) ) {
        //ShowMessage('warning', 'Warning - UWS', "Scale Weight should be greater than Tare Weight.", "bottom-right");
        //ShowMessage('warning', 'Information', "Scale weight should be greater than the total of ULD Tare weight & Equipment Tare weight.[Total Tare weight=" + CurrentTareWeight + "]");
        ShowMessage('warning', 'Information', "Scale weight should be greater than the Total of Shipment Weight & Equipment Tare Weight(" + CurrentTareWeight + ")");
        $('#txtNewScaleWeight').focus().css('border-color', 'red');
        chkFlag = false;
        return false;
    }
    ...
    if (fn_CheckValidation()) {
        $('#tblBDYRemoveBulk tr').each(function (row, tr) {
            if ($('#chkAWB_' + row).prop('checked')) {
                ...
                UWSArrayList.push({
                    SNo: $('#SNo' + row).val(),
                    AWBNo: $('#lblAWBNo' + row).text(),
                    RemovePieces: $('#txtRemovedpieces' + row).val(),
                    TotalPieces: $('#lblPieces' + row).text(),
                    GrossWeight: $('#txtRemovedGrossWeight' + row).val()
                });
            }
        });
        if (SaveFlag) {
            $.ajax({
                ...
                data: JSON.stringify({ UWSDataList: UWSArrayList, NewScaleWeight: parseFloat($("#txtNewScaleWeight").val()), IsCart: IsCart }),
                // JSON.stringify({ SNo: SNo, AWBNo: AWBNo, Removepieces: Pieces, Totalpieces: Totalpieces, NewScaleWeight: NewScaleWeight }),
                contentType: "application/json; charset=utf-8",
                success: function (result) {
                    if (result == "1") {
                        ShowMessage('warning', 'Warning - UWS', "Please enter correct pieces.", "bottom-right");
                        ...
                        ShowMessage('success', 'Success -UWS', "Deleted successfully.", "bottom-right");
                    }
                    else if (result == "3") {
                        // $("#lblPieces" + posi).text(Totalpieces - Pieces);
                        //$("#lblGrossWeight" + posi).text(RemainGrossWeight.toFixed(2));
                        ShowMessage('success', 'Success -UWS', "Piece(s) removed successfully ", "bottom-right");
            ...
            ShowMessage('warning', 'Warning - UWS', "Please select shipment", "bottom-right");
        }
    }
...
function SaveRemovepieces_old(obj) {
...
function OnScaleWeightChange() {
    if ($("#ScaleWeight").val() != "" && $("#EquipmentNo").val() != "") {
        ...
        if (tareWeight >= ScaleWeight) {
            // ShowMessage('info', 'Need your Kind Attention!', "Scale Weight should be greater than Tare Weight.", "bottom-left");
            ShowMessage('warning', 'Information', "Scale weight should be greater than the total of ULD Tare weight & Equipment Tare weight.[Total Tare weight=" + tareWeight + "]");
            ...
            success: function (result) {
                ...
                if (data.Table0[0].EquipmentSNo == 0) {
                    ...
                    if (data.Table0[0].TypeofShipment == "False") {
                        ...
                        tbl += "<tr><td style='text-align:center'><label id='lblBTRNo'>" + $('#EquipmentNo').val().toUpperCase() + "</label><input name='hdnEquipmentSNo' id='hdnEquipmentSNo' type='hidden' value=" + data.Table0[0].EquipmentSNo + "><input name='hdnUWSSNo' id='hdnUWSSNo' type='hidden' value=" + data.Table0[0].UWSSNo + "><input name='hdnAssignEquipment' id='hdnAssignEquipment' type='hidden' value=" + data.Table0[0].AssignEquipment + "><input name='hdnTotalWeight' id='hdnTotalWeight' type='hidden' value=" + data.Table0[0].TotalWeight + "></td><td style='text-align:center'><label id='lbltareweight'>" + data.Table0[0].TareWeight + "</label></td><td><label id='lblNetweight' name='lblNetweight'>" + data.Table0[0].TotalWeight + "</label></td><td style='text-align:center'><a href='#' onClick='showShipment(" + data.Table0[0].EquipmentSNo + "," + data.Table0[0].UWSSNo + ",this,0); return false;'>" + shipment[0] + "</a></td></tr>";
                        ...
                        var tbl = "";
                        tbl += "<table width='100%'><thead  style='text-align:center'><tr class='ui-widget-header'><td>BTR No.</td><td>Tare Weight </td><td>Net Weight </td><td>Total AWB No.</td></tr>";
                        var shipment = data.Table0[0].Shipment.split(',');
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                if (resultStatus == "1") {
                    ...
                    ShowMessage('warning', 'Warning - UWS', resultVal, "bottom-right");
                }
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            $("input[id^='Location']").each(function (index, value) {
                cfi.AutoCompleteV2("Location" + index, "LocationName", "UWS_LocationName", null, "contains");
            });
            
            cfi.AutoCompleteV2("UnAssignEquipmentNo", "EquipmentNo", "UWS_EquipmentNo", null, "contains");
...
function BindUWSAWB() {
    ...
    cfi.AutoCompleteV2("AirlineSNo", "CarrierCode,AirlineName", "UWS_AirlineNameNew", SetAWBPrefixCode, "contains");
...
function ExtraCondition(textId) {
    ...
    else if (textId.indexOf("Text_ChangeEquipmentNo") >= 0) {
        var filterEChange = cfi.getFilter("AND");
        cfi.setFilter(filterEChange, "IsAvailable", "eq", "1");
        cfi.setFilter(filterEChange, "CarrierCode", "in", GetPartnerCarrierCode(PartnerCarrierCode));
        filterNewEquipment = cfi.autoCompleteFilter(filterEChange);
        return filterNewEquipment;
    }
...
function PagerightsCheckUWSInfo() {
    var CheckIsFalse = 0;

    $(userContext.PageRights).each(function (e, i) {

        if (i.Apps.toString().toUpperCase() == "UWSINFO") {
      
                if (i.Apps.toString().toUpperCase() == "UWSINFO" && i.PageRight == "New") {
                    RightsCheck = false;
                    CheckIsFalse = 1;
                    return
                } if (i.Apps.toString().toUpperCase() == "UWSINFO" && i.PageRight == "Edit") {
                    RightsCheck = false;
                    CheckIsFalse = 1;
                    return
                } if (i.Apps.toString().toUpperCase() == "UWSINFO" && i.PageRight == "Delete") {
                    RightsCheck = false;
                    CheckIsFalse = 1;
                    return
    ...
    if (RightsCheck) {
        ...
        $("#btnPrint").hide()
        $("#btnAddWaybill").hide()
        $("#divDetail").find('input[type="button"][value="F"]').prop('disabled', true);
        $("#divDetail").find('input[type="button"][value="RW"]').prop('disabled', true);
        $("#divDetail").find('input[type="button"][value="CE"]').prop('disabled', true);
        $("#divDetail").find('input[type="button"][value="O"]').prop('disabled', true);
        $("#divDetail").find('input[type="button"][value="R"]').prop('disabled', true);
