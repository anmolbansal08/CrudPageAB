...
$(document).ready(function () {
    ...
    $('#ULDSerialNo').focusout(function () {
        if (userContext.SysSetting.UldserialnumberSize == 'True') {
            if ($('#ULDSerialNo').val().length >= 5 || $('#ULDSerialNo').val() == '') {
                return true;
            }
            else if ($('#ULDSerialNo').val().length <= 3) {
                ShowMessage('warning', 'Need your Kind Attention!', "ULD Serial No must have minimum 4 & 5 characters.");
                $('#ULDSerialNo').val('');
            }
        } else {
            if ($('#ULDSerialNo').val().length >= 5 || $('#ULDSerialNo').val() == '') {
                ...
                ShowMessage('warning', 'Need your Kind Attention!', "ULD Serial No must have minimum 5 characters.");
                $('#ULDSerialNo').val('');
            }
        }
    });
    ...
    $("input[name='operation']").click(function () {
        ...
        if (operation.trim() == "Update".trim()) {
            ...
            if (ret == "NO") {
                ShowMessage('warning', 'Need your Kind Attention!', "ULD Stock not available !!!");
                return false;
            }
        }
    ...
    var ULDALL = "";
    var ULDOWN = "";
    OWNULD = userContext.SpecialRights.OWNULD;
    ...
    function onselect() {
        ...
        var AirlineCode = $("#Text_AirlineSNo").val()
        $("#OwnerCode").val(AirlineCode.split("-")[0])
...
//    });
//    if (isView) {
//       // $(".incompleteprocess").attr("disabled", true);
//        $('.k-grid-content').find('.incompleteprocess').removeAttr('onclick');
//    }
//    else if (IsBlocked) {
//        $('.k-grid-content').find('.incompleteprocess').hide();
//      //  $(".incompleteprocess").hide();
//    }
//    else if (IsEdit) {
//        // $(".incompleteprocess").show();
//        $('.k-grid-content').find('.incompleteprocess').show();
//    }
//});
...
function Inventorymessage(id, IvId) {


    $("#DivGetInventory").dialog({
        modal: true,
        draggable: true,
        resizable: true,
        position: ['center', 'top'],
        ...
        buttons: {
            ...
            "OK": function () {
                var moveok = InventoryMove(IvId)


                $(this).dialog("close");
                ShowMessage('success', '', "ULD Stocks moved successfully !!!");
                navigateUrl('Default.cshtml?Module=ULD&Apps=UldStock&FormAction=INDEXVIEW');
...
var moveuld = 0;
function InventoryMove(IvId) {

    $.ajax({
        url: "Services/ULD/ULDStockService.svc/Inventorymessage", async: false, type: "POST", dataType: "json", cache: false,
...
$(document).on('click', '#IsServiceable', function () {
    //ShowMessage('warning', 'Warning - ULD Stock', "Don't have permission to update ", "bottom-right");
    return false;
...
$(document).on('click', '#IsDamaged', function () {
    ShowMessage('warning', 'Warning - ULD Stock', "Please use ULD Inventory page to mark ULD as damage.", "bottom-right");
    return false;
...
function ExtraCondition(textId) {
    ...
    //Added by Shivali Thakur to remove duplicacy of SHC dropdown.
    if (textId == "Text_SHCSNo") {
        cfi.setFilter(filter, "SNo", "notin", $("#Text_SHCSNo").data("kendoAutoComplete").key());
        var RegionAutoCompleteFilter = cfi.autoCompleteFilter(filter);
        return RegionAutoCompleteFilter;
    }
    if (textId == "Text_AirlineSNo") {
        if (userContext.SpecialRights.OWNULD != true) {
            cfi.setFilter(filter, "CarrierCode", "neq", userContext.AirlineCarrierCode.split("-")[0].trim());
            var RegionAutoCompleteFilter = cfi.autoCompleteFilter(filter);
            return RegionAutoCompleteFilter;
        }
...
$("#divFooter").append("<div id='divGeneratedInventory'></div>")
$("#divGeneratedInventory").html("<center><div style='display:none;' id='DivGetInventory'><h3> Are you sure to move ULD Stock to Non-Inventory . </h3></div><center>");
