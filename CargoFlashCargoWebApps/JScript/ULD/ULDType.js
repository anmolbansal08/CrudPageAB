$(document).ready(function () {
    ...
    cfi.AutoCompleteV2("AirlineSno", "CarrierCode,AirlineName", "ULD_AirlineSno", null, "contains")


    //cfi.AutoComplete("RateClassSno", "RateClassCode", "vwRateClass", "SNo", "RateClassCode", ["RateClassCode"], null, "contains");

    cfi.AutoCompleteV2("RateClassSno", "RateClassCode", "ULD_RateClassSno", null, "contains");
    ...
    /// Add By Sushant 
    $(document).on('change', '#ULDCode', function () {
...
var checkPwd = function () {
    var str = document.getElementById('ULDCode').value;
    if (str.search(/[a-zA-Z]/) == -1) {
        ShowMessage('info', 'ULD Code!', "Please enter at least one letter!", "bottom-right");
        $("#ULDCode").val("");
    }
}

$(document).on('change', '#EmptyWeight', function () {
    var GrossWeight = $("#GrossWeight").val() == "" ? parseFloat($("#_tempGrossWeight").val()).toFixed(2) : parseFloat($("#GrossWeight").val()).toFixed(2)
    var EmptyWeight = parseFloat($("#EmptyWeight").val()).toFixed(2)
    if (parseFloat(EmptyWeight) > parseFloat(GrossWeight)) {
        ShowMessage('info', 'ULD CODE', "TARE WEIGHT SHOULD NOT BE GREATER THEN GROSS WEIGHT", "bottom-right");
        $("#EmptyWeight").val("")
    }
});

$(document).on('change', '#GrossWeight', function () {

    var EmptyWeight = $("#EmptyWeight").val() == "" ? parseFloat($("#_tempEmptyWeight").val()).toFixed(2) : parseFloat($("#EmptyWeight").val()).toFixed(2)

    if (parseFloat(EmptyWeight) > parseFloat($("#GrossWeight").val()).toFixed(2)) {
        ShowMessage('info', 'ULD CODE', "TARE WEIGHT SHOULD NOT BE GREATER THEN GROSS WEIGHT", "bottom-right");
        $("#GrossWeight").val("")
    }
});
