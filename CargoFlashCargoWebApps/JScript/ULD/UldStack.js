...
$(document).ready(function () {
    ...
    cfi.AutoCompleteV2("StackUld", "ULDNo", "ULD_StackUld", BuildStack, "contains", ",", null, null, null, BuildStackRestriction, true);
    ...
    cfi.AutoCompleteV2("ChoosenAirline", "CarrierCode,AirlineName", "ULD_ChoosenAirline", null, "contains");
    ...
    $('input[name="operation"]').attr("disabled", "disabled")
    //  $("input:button[id$='MasterSaveAndNew']").
    ...
    $('input[name="operation"]').click(function (e) {
        if (getQueryStringValue("FormAction").toUpperCase() == "EDIT" || getQueryStringValue("FormAction").toUpperCase() == "NEW") {
            ...
            uldStackOject.push({
                ...
                FlightDate: $("#txtFlightDate").val() == "" ? null : $("#txtFlightDate").val(),
            ...
            /*
            var multistackuld = $('#divMultiStackUld ul li:visible').length;
            var noofstack = 5;
            if (multistackuld > noofstack)
            {
                ShowMessage('warning', 'Warning', "Maximum 5 Uld Allowed.", "bottom-right");
                return false;
            }
           // else {
           */
    ...
    $("input:button[id$='ValidateBaseUld']").click(function () {
        ...
        if ($("#ChoosenAirline").val() == "") {
            ShowMessage('warning', 'Warning', "Please Select Airline!", "bottom-right");
            return false;
        }

        if ($("#Text_BaseUldNumber").val().length < 9) {
            ShowMessage('warning', 'Warning', "ULD number Should be Minimum 10 & Maximum 11", "bottom-right");
            return false;
        }
        ...
        $.ajax({
            ...
            type: "GET",
            dataType: "json",
            data: { UldNo: $("#Text_BaseUldNumber").val(), AirlineCode: $("#ChoosenAirline").val(), CityCode: userContext.CityCode, AirportSNo: userContext.AirportSNo },
            contentType: "application/json; charset=utf-8",
            cache: false,
            success: function (result) {
                if (JSON.parse(result)[0].ULDType == '00') {
                    ShowMessage('warning', 'Warning', "ULD Number Is Not Available", "bottom-right");
                    return false;
                }

                else
                    if (JSON.parse(result)[0].ULDType == '02') {
                        ShowMessage('warning', 'Warning', "ULD Number Is Damage", "bottom-right");
                        return false;
                    }
                    else
                            ...
                            if (parseInt(JSON.parse(result)[0].ULDSNo) != 0) {
                                ...
                                $.ajax({
                                    ...
                                    success: function (result) {
                                        ...
                                        if (myData.length > 0) {
                                            $('input[name="operation"]').attr("disabled", false)
                                            if ($("#ChoosenAirline").val() != myData[0].SNo)
                                ...
                                //alert("Incorrect ULD Number or May be duplicate ULD Number or Damaged ULD");
                                ShowMessage('warning', 'Warning', "Incorrect ULD Number or May be duplicate ULD Number or Damaged ULD", "bottom-right");
                                return false;
                            }
                        }
            }
    ...
    if (getQueryStringValue("FormAction").toUpperCase() == "READ" || getQueryStringValue("FormAction").toUpperCase() == "DELETE") {
        ...
        cfi.AutoCompleteV2("txtAirline", "CarrierCode,AirlineName", "ULD_txtAirline", GetFlight, "contains");
        ...
        cfi.AutoCompleteV2("txtFlightNo", "FlightNo", "ULD_txtFlightNo", GetFlightRoute, "contains")
    ...
    $("span.k-delete").live("click", function () {
       // alert($(this).siblings().text());
        ...
        var udno= $(this).siblings().text().trim();
        $('#tblUldStack tr > td:contains(' + udno + ') ').closest("tr").remove();
...
function BuildStackRestriction(e) {
    ...
    if (BaseUld.trim() == e.item[0].innerText.trim()) {
        e.preventDefault();
        ShowMessage('warning', 'Warning - ULD Stack !!', "Selected ULD is Already Used as Base ULD", "bottom-right");
        return;
    }
    var nos = $('#divMultiStackUld ul li:visible').length

    if (nos == 19)
    {
        e.preventDefault();
          ShowMessage('warning', 'Warning - ULD Stack !', "Maximum 19 ULDs can be added", "bottom-right");
        //ShowMessage('warning', 'Warning - LyingList', "Please Add Valid Email.");
       // $('#divMultiStackUld ul li:visible').last("tr").remove();
       // $('#tblUldStack tbody tr').last("tr").remove();
        return;
    }
    //var finalValue = $("#StackUld").val() + ',' + e.item[0].innerText
    var finalValue = $("#StackUld").val() + ',' + $("#hdnUldSNo").val()
    var myData;
    $.ajax({
            ...
            if (myData == "False") {
                ...
                $.ajax({
                    ...
                    success: function (result) {
                        ...
                        // ValudateCheck(Uld)
                        var myData = jQuery.parseJSON(result);
                        var pj = e.item[0].innerText.trim();

                        var tj = $('#tblUldStack tr > td:contains(' + pj + ') ').text().trim();

                        if (pj == tj) {
                            e.preventDefault();
                            ShowMessage('info', 'Info - ULD Stack !!', "Selected  ULD  Already  Exists ", "bottom-right");
                            return;
                        }
...
function GetAppendGrid() {
    $('#tblConsumable').appendGrid({
        ...
        columns: [
         ...
         { name: "ConsumableType", display: "Consumable Type", type: "text", ctrlAttr: { controltype: pageType == "NEW" || pageType == "EDIT" ? "autocomplete" : "text" }, ctrlCss: { width: "100px" }, isRequired: pageType == "NEW" || pageType == "EDIT" ? true : false, AutoCompleteName: 'ULD_Item', filterField: 'Item' },
...
function assignFlightdateNFlightNo(obj) {
    ...
    cfi.AutoCompleteV2("txtAirlinePopUp", "CarrierCode,AirlineName", "ULDStack_AirlineName", GetFlightpopUp, "contains");
    cfi.AutoCompleteV2("txtFlightNoPop", "CarrierCode,FlightNumber", "ULDStack_FlightNumber", GetFlightRoutePopUp, "contains");
...
function assignFlightNo_FlightDate(uldStackSNo, IsassignOrOffload) {
    if ($("#Text_txtAirlinePopUp").val() == '') {
        ShowMessage('warning', 'Warning', "Please Select Airline Name", "bottom-right");
        return false;
    }
    if ($("#txtFlightDt").val() == '') {
        ShowMessage('warning', 'Warning', "Please Select Flight Date", "bottom-right");
        return false;
    }
    if ($("#Text_txtAirlinePopUp").val() == '') {
        ShowMessage('warning', 'Warning', "Please Select Flight No.", "bottom-right");
        return false;
    }
...
function ExtraCondition(textId) {
    ...
    var AutoCompleteFilter = cfi.getFilter("AND");
    //var filter = cfi.getFilter("AND");
    ...
    if (textId.indexOf("Text_StackUld") >= 0) {
        ...
        cfi.setFilter(filters, "CurrentAirportSNo", "eq", userContext.AirportSNo);
        ...
        return textId = cfi.getFilter("AND"),
            cfi.setFilter(textId, "FlightDate", "eq", $("#txtFlightDate").data("kendoDatePicker").element[0].value),
            cfi.getFilter("AND"), cfi.setFilter(textId, "SNo", "eq", $("#txtAirline").val()),
            filter = cfi.autoCompleteFilter(textId);
    ...
    if ("Text_txtAirline" == textId)
        ...
        if ($("#txtAirlinePopUp").val() != '' && $("#txtAirlinePopUp").val() != '0') {

            var Fliter = cfi.getFilter("AND");
            cfi.setFilter(Fliter, "FlightDate", "eq", $("#txtFlightDt").data("kendoDatePicker").element[0].value),
            cfi.setFilter(Fliter, "OriginAirportCode", "eq", userContext.AirportCode),
            //cfi.setFilter(Fliter, "SNo", "eq", $("#txtAirlinePopUp").val()),
           AutoCompleteFilter = cfi.autoCompleteFilter(Fliter);
            return AutoCompleteFilter;

            //return textId = cfi.getFilter("AND"),
            //cfi.setFilter(textId, "FlightDate", "eq", $("#txtFlightDt").data("kendoDatePicker").element[0].value),
            //cfi.getFilter("AND"),
            //cfi.setFilter(textId, "SNo", "eq", $("#txtAirlinePopUp").val()),
            //cfi.getFilter("AND"),
            //cfi.setFilter(textId, "OriginAirportCode", "eq", userContext.AirportCode), filter = cfi.autoCompleteFilter(textId);
        } else {


            var Fliter2 = cfi.getFilter("AND");
            cfi.setFilter(Fliter2, "FlightDate", "eq", $("#txtFlightDt").data("kendoDatePicker").element[0].value),
            cfi.setFilter(Fliter2, "OriginAirportCode", "eq", userContext.AirportCode),
            AutoCompleteFilter = cfi.autoCompleteFilter(Fliter2);
            return AutoCompleteFilter;
