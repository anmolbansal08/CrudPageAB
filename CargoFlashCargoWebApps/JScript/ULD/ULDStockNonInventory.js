...
function onGridDataBound(e) {
    var isView = false, IsBlocked = false, IsEdit = false;

    $(userContext.ProcessRights).each(function (i, e) {
        if (e.SubProcessSNo == 6012) {
            isView = e.IsView;
                        ...
                        buttons: {
                            ...
                            "OK": function () {
                                var moveok = InventoryMove(IvId)                                                   
                                    $(this).dialog("close");
                                    ShowMessage('success', '', "Non-Inventory ULD Stocks moved successfully !!!");
                                    navigateUrl('Default.cshtml?Module=ULD&Apps=UldStockNonInventory&FormAction=INDEXVIEW');
...
function InventoryMove(IvId)
{
        ...
        $.ajax({
            url: "Services/ULD/ULDStockNonInventoryService.svc/Inventorymessage", async: false, type: "POST", dataType: "json", cache: false,
...
$("#divGeneratedInventory").html("<center><div style='display:none;' id='DivGetInventory'><h3> Are you sure to move Non-Inventory to ULD Stock. </h3></div><center>");
