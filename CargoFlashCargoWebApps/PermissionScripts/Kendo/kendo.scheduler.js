...
*/
(function(f, define){
    define([ "./kendo.dropdownlist", "./kendo.editable", "./kendo.multiselect", "./kendo.window", "./kendo.ToWCFDatepicker", "./kendo.scheduler.recurrence", "./kendo.scheduler.view", "./kendo.scheduler.dayview", "./kendo.scheduler.agendaview", "./kendo.scheduler.monthview", "./kendo.mobile.actionsheet", "./kendo.mobile.pane" ], f);
})(function(){
//kendo.editable,kendo.multiselect,kendo.window
/*jshint eqnull: true */
(function($, undefined) {
    var kendo = window.kendo,
        date = kendo.ToWCFDate,
        input_support = kendo.support.input,
        MS_PER_DAY = date.MS_PER_DAY,
        getDate = date.getDate,
        getMilliseconds = kendo.ToWCFDate.getMilliseconds,
        ...
        TODAY = kendo.ToWCFDate(new Date()),
    ...
    var SchedulerEvent = kendo.data.Model.define({
        ...
        _time: function(field) {
            ...
            if (this[fieldTime]) {
                return this[fieldTime] - kendo.ToWCFDate.toUtcTime(kendo.ToWCFDate.getDate(date));
            }
        ...
        _date: function(field) {
            ...
            return kendo.ToWCFDate.getDate(this[field]);
        },
        ...
        duration: function() {
            ...
            var offset = (end.getTimezoneOffset() - start.getTimezoneOffset()) * kendo.ToWCFDate.MS_PER_MINUTE;
        ...
        update: function(eventInfo) {
            ...
            if (this.startTime) {
                this.set("startTime", kendo.ToWCFDate.toUtcTime(this.start));
            }

            if (this.endTime) {
                this.set("endTime", kendo.ToWCFDate.toUtcTime(this.end));
            }
        },

        isMultiDay: function() {
            return this.isAllDay || this.duration() >= kendo.ToWCFDate.MS_PER_DAY;
        },
        ...
        set: function(key, value) {
            ...
            if (key == "isAllDay" && value != isAllDay) {
                var start = kendo.ToWCFDate.getDate(this.start);
                var end = new Date(this.end);
                var milliseconds = kendo.ToWCFDate.getMilliseconds(end);
                ...
                if (value === true) {
                    kendo.ToWCFDate.setTime(end, -milliseconds);

                    if (end < start) {
                        end = start;
                    }
                } else {
                    kendo.ToWCFDate.setTime(end, MS_PER_DAY - milliseconds);
                }
    ...
    function moveEventRange(event, distance) {
        ...
        kendo.ToWCFDate.setTime(start, distance);

        var end = new Date(start.getTime());

        kendo.ToWCFDate.setTime(end, duration, true);
    ...
    var Scheduler = Widget.extend({
                ...
                if (selection.events.length) {
                    ...
                    if (selection.isAllDay) {
                        selection = $.extend({}, selection, {
                            end: kendo.ToWCFDate.addDays(selection.end, -1)
                        });
                    }
            ...
            if (dataItem && selection) {
                if (this._shiftKey && selection.start && selection.end) {
                    ...
                    if (backward && view._timeSlotInterval) {
                        kendo.ToWCFDate.setTime(selection.end, -view._timeSlotInterval());
                    }
                } else {
        ...
        _resizable: function() {
            ...
            that._resizeDraggable = new kendo.ui.Draggable(that.element, {
                ...
                dragstart: function(e) {
                    ...
                    startTime = kendo.ToWCFDate.toUtcTime(event.start);

                    endTime = kendo.ToWCFDate.toUtcTime(event.end);
                },
                drag: function(e) {
                    if (!slot) {
                        return;
                    ...
                    if (dir == "south") {
                        if (!slot.isDaySlot && slot.end - kendo.ToWCFDate.toUtcTime(event.start) >= view._timeSlotInterval()) {
                            if (event.isAllDay) {
                                endTime = slot.startOffset(e.x.location, e.y.location, that.options.snap);
                            } else {
                        ...
                        if (!slot.isDaySlot && kendo.ToWCFDate.toUtcTime(event.end) - slot.start >= view._timeSlotInterval()) {
                            startTime = slot.startOffset(e.x.location, e.y.location, that.options.snap);
                        }
                    } else if (dir == "east") {
                        if (slot.isDaySlot && kendo.ToWCFDate.toUtcTime(kendo.ToWCFDate.getDate(slot.endDate())) >= kendo.ToWCFDate.toUtcTime(kendo.ToWCFDate.getDate(event.start))) {
                            if (event.isAllDay) {
                                endTime = slot.startOffset(e.x.location, e.y.location, that.options.snap);
                            } else {
                                endTime = slot.endOffset(e.x.location, e.y.location, that.options.snap);
                        ...
                        if (slot.isDaySlot && kendo.ToWCFDate.toUtcTime(kendo.ToWCFDate.getDate(event.end)) >= kendo.ToWCFDate.toUtcTime(kendo.ToWCFDate.getDate(slot.startDate()))) {
                            startTime = slot.startOffset(e.x.location, e.y.location, that.options.snap);
                        }
                    }
                ...
                dragend: function(e) {
                    ...
                    if (dir == "south") {
                        ...
                        end = kendo.ToWCFDate.getDate(kendo.timezone.toLocalDate(endTime));
                    } else if (dir == "west") {
        ...
        _updateEvent: function(dir, event, eventInfo) {
            ...
            var updateSeries = function() {
                ...
                if (dir == "south" || dir == "north") {
                    if (eventInfo.start) {
                        var start = kendo.ToWCFDate.getDate(head.start);
                        kendo.ToWCFDate.setTime(start, getMilliseconds(eventInfo.start));
                        eventInfo.start = start;
                    }
                    if (eventInfo.end) {
                        var end = kendo.ToWCFDate.getDate(head.end);
                        kendo.ToWCFDate.setTime(end, getMilliseconds(eventInfo.end));
                        eventInfo.end = end;
                    }
                }
       ...
       saveEvent: function() {
        ...
        _adjustSelectedDate: function() {
            ...
            if (start && !kendo.ToWCFDate.isInDateRange(date, getDate(start), getDate(selection.end))) {
