/*!
* jQuery cfValidator plugin
*
* http://code.google.com/p/cfValidator/
*
...
*
* $Id: jquery.cfValidator.js 101 2012-09-30 23:51:36Z bmauser $
*/

(function ($) {

    // constructor
    $.fn.cfValidator = function (overrideOptions, instanceName) {
        return this.each(function () {
            new cfValidator($(this), overrideOptions, instanceName);
        });
    };

    // cfValidator class
    cfValidator = function (mainElement, overrideOptions, instanceName) {
        ...
        var options = {
            ...
            // css class names
            classNamePrefix: 'valid_', // prefix for css class names
            ...
            errorMessageAttr: 'data-valid-msg', // name of the attribute for overridden error message
            validateActionsAttr: 'data-valid', // name of the attribute which stores info what validation actions to do
            forceValidAttr: 'data-valid-forcevalid', // name of the attribute which which makes validator to act like the field is valid
            modifyActionsAttr: 'data-valid-modifier',
            ...
            otherErrorValidateOn: 'blur', 	// null, 'change', 'blur', 'keyup'
            ...
            errorMessages: {
                en: {
                    'default': 'Correct this value.',
                    'equalto': 'Enter the same value again.',
                    'differs': 'Enter a different value.',
                    'minlength': 'Minimum {0} characters required',
                    'maxlength': 'Maximum {0} characters allowed',
                    'rangelength': 'Length must be between {0} and {1}',
                    'min': 'Enter a number greater than or equal to {0}.',
                    'max': 'Enter a number less than or equal to {0}.',
                    'between': 'Enter a number between {0} and {1}.',
                    'required': 'This field is required.',
                    'alpha': 'Enter alphabetic characters only.',
                    'alphanum': 'Enter alphanumeric characters only.',
                    'digit': 'Enter only digits.',
                    'number': 'Enter a valid number.',
                    'email': 'Enter a valid email address.',
                    'image': 'This field should only contain image types',
                    'url': 'Enter a valid URL.',
                    'ip4': 'Enter a valid IPv4 address.',
                    'ip6': 'Enter a valid IPv6 address.',
                    'awbno': 'Enter valid AWB No.',
                    'hawbno': 'Enter valid HAWB No.',
                    'date': 'Enter a valid date in format {0}.'
                }
            }
        },
		...
		_showMsg = function (element, messages) {
		    ...
		    tooltip.css({ visibility: 'visible', position: 'absolute', top: pos.top, left: pos.left, 'z-index': 99999 }).fadeIn(options.showErrMsgSpeed);
		    ...
		    //Added by Amit Yadav
		    if ($(element).attr("data-role") == "autocomplete" || $(element).attr("data-role") == "combobox" || $(element).attr("controltype") == "datetype") {
		        $(element).closest("span").css('border-color', 'red');
		    }
		    setTimeout(function () { $(".valid_errmsg").fadeOut("slow"); }, 2000);
		...
		_removeMsg = function (element) {
		    ...
		    if (existingMsg) {
		        ...
		        if ($(element).parent('.k-state-default')) //Added by Braj
		            $(element).parent('.k-state-default').removeAttr('style');
		    }
		},
		...
		_getErrMsgPosition = function (input, tooltip) {
		    ...
		    var tempinputId = inputId;

		    if ($(input).attr("data-role") == "numerictextbox" || $(input).attr("data-role") == "alphabettextbox") {
		        tempinputId = "_temp" + inputId;
		        if ($("#" + tempinputId).length == 0) {
		            tempinputId = inputId;
		        }
		    }
		    var width = $("#" + inputId).outerWidth();
		    if ($(input).attr("type") == "checkbox" || $(input).attr("id") == "radio") {
		        width = 0;
		    ...
		    var tooltipContainer = input.data("errMsg.bV" + instanceName),
		         top = -((tooltipContainer.offset().top - ($("#" + tempinputId).is(":visible") == true ? $("#" + tempinputId).offset().top : $("#" + inputId).offset().top)) + tooltip.outerHeight() - options.offset.y),
		         left = (($("#" + tempinputId).is(":visible") == true ? $("#" + tempinputId).offset().left : $("#" + inputId).offset().left) + width) - tooltipContainer.offset().left + options.offset.x,
		...
		_chkboxGroup = function (chkbox) {
		    var name = chkbox.attr('validatename');
		    if (name && /^[^\[\]]+\[.*\]$/.test(name)) {
		        return $('input:checkbox').filter(function () {
		            var r = new RegExp(name.match(/^[^\[\]]+/)[0] + '\\[.*\\]$');
		            return $(this).attr("validatename").match(r);
		        });
		    }
		    return chkbox;
		},
		...
		_getValue = function (element) {
		    ...
		    if (element.is('input:checkbox')) {
		        ...
		        if (element.attr("data-role") == "autocomplete") {

		            if ($("#" + element.attr("id").replace("Text_", "")).attr("explicitValid") == "1") {
		                ret['value'] = element.val() || $("#" + element.attr("id").replace("Text_", "")).val();

		            }
		            else if ($("#" + element.attr("id")).closest("td").find("div[id^='divMulti']").length == 1) {
		                ret['value'] = $("#" + element.attr("id").replace(element.attr("id").split('_')[1], "Hdn" + element.attr("id").split('_')[1])).val();
		            }
		            else
		                ret['value'] = element.val();
		        }
		...
		_callModifier = function (action, el) {
		    ...
		    if (typeof modifier[action.name] == 'function')
		        ...
		        window.console.warn('[cfValidator] unknown modifier: ' + action.name);
		},
		...
		_callValidator = function (action, el, value) {
		    ...
		    if (window.console.warn)
		        window.console.warn('[cfValidator] unknown validator: ' + action.name);
		},
		...
		validator = {
		    ...
		    awbno: function (v) {
		        //return validator.regex(v, /^[0-9][0-9][0-9]+(-[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9])+$/);
		        return validator.regex(v, /^[A-Z0-9]{3}-[0-9]{8}$/);
		    },

		    hawbno: function (v) {
		        return validator.regex(v, /^[a-zA-Z\d\-]+(-*)+([0-9]*)+$/);
		    },
		    ...
		    number: function (v) {
		        //Added by Amit Yadav for  allow dot at zero index
		        v.value = v.value.indexOf('.') == 0 ? "0" + v.value : v.value;
        ...
        // global options
        if (window['cfValidatorOptions'])
            $.extend(true, options, window['cfValidatorOptions']);
        ...
        var allInstances = mainElement.data("cfValidators");
        if (!allInstances) {
            allInstances = {};
            mainElement.data("cfValidators", allInstances);
        }

        // if there is already first instance
        if (mainElement.data("cfValidator")) {
            if (!instanceName)
                return mainElement.data("cfValidator"); // return existing instance
            ...
            if (!instanceName)
                instanceName = 'first';
            mainElement.data("cfValidator", this);
        }
        ...
        this.validate = function (doNotshowMessages, elementsOverride, forceAjaxSync, ajaxResponse, onlyIsValidCheck) {
            ...
            if (typeof ajaxResponse !== 'undefined' || _callBack('onBeforeAllValidations', elementsl) !== false) {
                ...
                elementsl.each(function () {
                    ...
                    // call modifiers
                    if (modifiers_exp) {
                        for (var j = 0; j < modifiers_exp.length; j++) {

                            action = _parseAction(modifiers_exp[j]);
                        ...
                        if (errorMessages.length) {
                            ...
                            if (!chk_rad) {
                                ...
                                if (options.errorClass) {
                                    ...
                                    if ($("#" + inputId).attr("data-role") == "numerictextbox" || $("#" + inputId).attr("data-role") == "alphabettextbox") {
                                        tempinputId = "_temp" + inputId;
                                        if ($("#" + tempinputId).length == 0) {
                                            tempinputId = inputId;
                                        }
                                        if ($("#" + tempinputId).is(":visible"))
                                            $("#" + tempinputId).addClass(options.classNamePrefix + options.errorClass);
                                        else
                                            $("#" + inputId).addClass(options.classNamePrefix + options.errorClass);
                                    } else {
                                        $("#" + inputId).addClass(options.classNamePrefix + options.errorClass);
                                    }
                            ...
                            if (options.errorValidateOn) {
                                ...
                                if (chk_rad) {
                                    ...
                                    $(group).unbind(evt + '.bVerror' + instanceName);
                                    ...
                                    $(this).unbind(evt + '.bVerror' + instanceName);
                            ...
                            if (!chk_rad) {
                                ...
                                if ($(this).attr("data-role") == "numerictextbox") {
                                    inputId = "_temp" + inputId;
                                }
                                $("#" + inputId).removeClass(options.classNamePrefix + options.errorClass);
                                if (options.validClass) {
                                    var inputId = $(this).attr("id");
                                    if ($(this).attr("data-role") == "numerictextbox") {
                                        inputId = "_temp" + inputId;
                                    }
                                    $("#" + inputId).addClass(options.classNamePrefix + options.validClass);
                                }
                            }
        ...
        this.reset = function () {
            ...
            elements.each(function () {
                ...
                $("#" + inputId).removeClass(options.classNamePrefix + options.errorClass);
                $("#" + inputId).removeClass(options.classNamePrefix + options.validClass);
        ...
        this.destroy = function () {
            ...
            mainElement.removeData("cfValidator");
            mainElement.removeData("cfValidators");
        }
    }
