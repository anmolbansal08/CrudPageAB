...
var timeoutvalue = 0;
var userContext = JSON.parse(parent.document.getElementById("hdnUserContext") ? parent.document.getElementById("hdnUserContext").value : "[]");
...
function CargoFlashApplication() {
	...
	this.K_Options = function (extra_ConditionId, autoCompleteName) {
				...
				transport: {
					...
					parameterMap: function (options) {
						...
						if (options.filter != undefined) {
							var filter = _ExtraCondition(extra_ConditionId);
							if (filter == undefined) {
    ...
    this.ShowPopUp = function (title, url, data, width, OnClose) {
...
function StartTimers() {
    var timoutNow = userContext.SysSetting.SessionTimeout || 900000;
    //  var timoutNow = 5000;
...
//        xhr.setRequestHeader('Token', userContext.Signature);
//    }
//});
...
function ConfirmAlert(message) {
    ...
    var result = false;
    var template = '<div class="popupMessage"></div></br><div class="dialog_buttons"><input type="button" class="confirm_yes k-button" value="Ok"  /><input type="button" class="confirm_no k-button" value="Cancel"  /></div>';
    ...
    $("<div id='popupWindow'></div>")
        ...
        }).data('kendoWindow').content(template).center().open();

    $('.popupMessage').html(message);

    //$('#popupWindow .confirm_yes').val('OK');
    //$('#popupWindow .confirm_no').val('Cancel');
    $('#popupWindow').parent().find('.k-header').addClass('popupTitle');
