        ...
        var f = {
            ...
            getValue: function ($input) {
                ...
                if ($($input).attr("type") == "radio") {
                    ...
                    value = $input.val() == "on" ? "1" : $input.val();
                } else {
                    value = $input.val();
                }
            ...
            serializer: function (selector) {
                ...
                //Edit By Amit Yadav
                var maindiv;
                $(selector.selector).find("div[cfi-aria-trans='trans']").each(function (index, div) {
                    maindiv = div;
                    $(this).find("tr[id^='areaTrans']").each(function (innerIndex, InnerObj) {
                        $(this).find("input[type!='button']").each(function (i, txt) {
                            this.name = this.id;
                            this.name = this.name.replace(txt.name, $(maindiv).attr("id").split('_')[2] + "[" + (innerIndex) + "]." + txt.name.split('_')[0]);
                        });
                    });
                });


                var formAsArray = $(selector.selector + " :input").serializeArray(selector.selector);
                ...
                $.each(formAsArray, function (i, item) {
                    ...
                    //Updated By Amit Yadav to get chekboxes is checked.
