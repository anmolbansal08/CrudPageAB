...
{
    [ServiceContract]
    public interface IULDSupportAssignedService
    {
        [OperationContract]
        ...
        string SaveUSRDetais(int USRSNo, ULDSRequest ULDSRequest, List<USRULDType> USRULDType, List<USRConsumableType> USRConsumableType);
        ...
        [OperationContract]
        [WebInvoke(BodyStyle = WebMessageBodyStyle.WrappedRequest, ResponseFormat = WebMessageFormat.Json, RequestFormat = WebMessageFormat.Json)]
        KeyValuePair<string, List<CheckULDAvailability>> GetULDAvailability(string recordID, int page, int pageSize, WhereConditionAssign model, string sort);

        [OperationContract]
        [WebInvoke(Method = "POST", UriTemplate = "DeleteULDSupportRequestAssigned?recid={SNo}", BodyStyle = WebMessageBodyStyle.WrappedRequest, ResponseFormat = WebMessageFormat.Json, RequestFormat = WebMessageFormat.Json)]
        List<string> DeleteULDSupportRequestAssigned(string SNo);
    }
}
