...
{
    [ServiceContract]
    public interface IULDTrackingService
    {
        [OperationContract]
        ...
        [OperationContract]
        [WebInvoke(Method = "GET", UriTemplate = "GetULDSearchRecord?recid={RecordID}&pageNo={page}&pageSize={pageSize}&whereCondition={whereCondition}&sort={sort}", BodyStyle = WebMessageBodyStyle.WrappedRequest, ResponseFormat = WebMessageFormat.Json, RequestFormat = WebMessageFormat.Json)]
        KeyValuePair<string, List<ULDGridData>> GetULDSearchRecord(string recordID, int page, int pageSize, string whereCondition, string sort);
        ...
        string GetULD(String ULDNo, String StartDate,String EndDate);
    }
}
