...
namespace CargoFlash.Cargo.DataService.Stock
{
    ...
    public class ReturnToOfficeService : SignatureAuthenticate, IReturnToOfficeService
    {
        public DataSourceResult GetGridData(int skip, int take, int page, int pageSize, List<GridSort> sort, GridFilter filter)
        {
            string sorts = GridSort.ProcessSorting(sort);
            string filters = GridFilter.ProcessFilters<StockManagement>(filter);
            //string Env = (((CargoFlash.Cargo.Model.UserLogin)(System.Web.HttpContext.Current.Session["UserDetail"])).AirlineCarrierCode.ToString()).Substring(0,1);
            ...
            try
            {
              ...
              new System.Data.SqlClient.SqlParameter("@OfficeSNo", ((CargoFlash.Cargo.Model.UserLogin)(System.Web.HttpContext.Current.Session["UserDetail"])).UserSNo.ToString())};
                ds = SqlHelper.ExecuteDataset(DMLConnectionString.WebConfigConnectionString, CommandType.StoredProcedure, "GetReturnToOfficeStockList", Parameters);
        ...
        public string GetIssuedOfficeStock(int StockSNo)
        {
            ...
            try
            {
                ...
                BaseBusiness baseBusiness = new BaseBusiness();
                DataSet ds = new DataSet();
                SqlParameter[] Parameters = { new SqlParameter("@StockSNo", StockSNo), new SqlParameter("@usersno", ((CargoFlash.Cargo.Model.UserLogin)(System.Web.HttpContext.Current.Session["UserDetail"])).UserSNo.ToString()) };
