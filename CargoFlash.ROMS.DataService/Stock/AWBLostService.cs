...
namespace CargoFlash.Cargo.DataService.Stock
{
    ...
    public class AWBLostService : SignatureAuthenticate, IAWBLostService
    {
        public DataSourceResult GetGridData(int skip, int take, int page, int pageSize, List<GridSort> sort, GridFilter filter)
        {
            try
            {
                ...
                DataSet ds = SqlHelper.ExecuteDataset(DMLConnectionString.WebConfigConnectionString, CommandType.StoredProcedure, "GetListAWBLost", Parameters);
        ...
        public List<string> SaveAWBLost(List<AWBLost> AWBLost)
        {
            try
            {
                ...
                if (!baseBusiness.ValidateBaseBusiness("AWBLost", dtCreateAWBLost, "SAVE"))
                {
                ...
                int ret = (int)SqlHelper.ExecuteScalar(DMLConnectionString.WebConfigConnectionString, "SaveAWBLost", Parameters);
                if (ret > 0)
                {
                    if (ret > 1000)
                    {
                        //For Customised Validation Messages like 'Record Already Exists' etc
                        string serverErrorMessage = baseBusiness.ReadServerErrorMessages(ret, "AWBLost");
